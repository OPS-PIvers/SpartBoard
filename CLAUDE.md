...
- **Non-widget features:** add an id to `GlobalFeature` (`types.ts`) and a `FEATURE_DEFAULTS` entry in `config/featureDefaults.ts` with `defaultAccessLevel: 'admin'`, `defaultEnabled: true`, `missingDocPublic: false`, `stage: 'preview'`, plus the row's `label`, `icon`, `description` and `afterLaunch` (`'retire'` for a redesign whose gate is deleted once public, `'keep'` for a lasting capability; a `keep` flag also names its `widget` or `category`). Admins get a preview flag before any doc exists; teachers don't. Add the id to `functions/src/featureMissingDoc.ts` too (a test fails if it drifts). Gate every entry point with `canAccessFeature('<id>')`.
...
- **Org-wide rollout switches** (`admin_settings/*`, listed in `config/rolloutSwitches.ts` and shown on the Previews tab) have no per-user targeting; when one is needed, AND it with a `global_permissions` gate, as the Quiz widget does with `paperSheetsRollout.enabled && canAccessFeature('paper-answer-sheets')`.
...
- **The PR description must say** which flag gates the feature, its starting access level, and the admin path to open it: Admin Settings > Access > Previews (or Widgets for a widget) > set to Public. A `stage: 'preview'` flag that is saved Public shows "Ready to retire" or "Ready to graduate" there; Paul asks an agent to retire it (delete the gate and the registry entry, add the changelog note) or to graduate it (`stage: 'permanent'`). Paul flips it after testing in prod; agents never open a flag to Public on prod themselves.
...
- **Scroll panels keep bottom padding.** Put the end padding on the element that scrolls (`overflow-y-auto pb-*`), or on a child that grows with its content. Never pad a box pinned to `h-full` inside a scroller: its padding lands at the viewport edge and long content runs flush to the bottom (shipped three times, last in Admin Settings > Features). An Admin Settings tab that owns its own scroll opts in with `fillHeight: true`. `tests/e2e/scroll-end-padding.spec.ts` measures every Admin Settings tab, Profile & Settings section and the settings drawer; add any new tabbed panel surface to it.
