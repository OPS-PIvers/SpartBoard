...
**Last updated:** 2026-05-29 by build session (Paul + Claude) — **real-Quiz pipe shipped + verified live; ClassLink-alias identity bridge designed, blocked on a real ClassLink course**; see [Progress Log](#progress-log) top entry.

**Active phase:** Thinnest real-Quiz pipe — ✅ **shipped + verified live.** Teacher attaches a real quiz from their library (in-iframe SpartBoard sign-in) → student takes the real `QuizStudentApp` inside Classroom on the proven studentRole session → answers submit, PII-free. (Earlier de-risk GO still holds: session survives the partitioned iframe, no CHIPS needed.) **Now building the ClassLink-alias identity bridge** so the teacher monitor shows real names (PII-free) instead of generic "Student": `course alias → class sourcedId` + `student email → OneRoster sourcedId` → mint `HMAC(sourcedId)` (same as ClassLink SSO). Feasibility confirmed (server already reads OneRoster via district creds). **⚠️ BLOCKED:** needs a real ClassLink-provisioned course (Joel is setting one up: Paul as teacher, `sstudent25@orono.k12.mn.us` as student) to confirm the alias format before wiring.
