...
**Last updated:** 2026-05-29 by build session (Paul + Claude) — **ClassLink identity bridge PROVEN live (PII-free named attribution); checkpoint → handoff to a fresh agent for 3 tasks**; see [Progress Log](#progress-log) top entry.

**Active phase:** ✅ **Real-Quiz pipe + ClassLink identity bridge both proven live.** A teacher attaches a real quiz; a ClassLink student opens it and is resolved (via a teacher-declared course→sourcedId link + OneRoster) to the SAME `HMAC(sid:sourcedId)` identity as their ClassLink SSO login, so the teacher monitor shows their **real name** — fully PII-free. Verified end-to-end in the real course `866479643796` with `sstudent25`. **The course→sourcedId link was hand-seeded for the test** (real ClassLink aliases are `p:`-scoped and unreadable by SpartBoard, so the mapping is teacher-declared and stored at `/classroom_course_links/{courseId}`).

**Active agent(s):** _none — handing off to a fresh agent (see HANDOFF in the [Progress Log](#progress-log) top entry)._

**HANDOFF — 3 tasks for the next agent:** (1) **"Link to Google Classroom" button** in the roster/classes UI (writes `/classroom_course_links` via `courses.list` picker — removes the manual seed; the last functional gap). (2) **Trim the spike scaffolding** (`[DEBUG-addonctx]`, the alias-probe button, the `classIds=` log, and the hand-seeded link doc). (3) **Grade passback (Phase 4)** (`studentWorkReviewUri` + `maxPoints`; persist `submissionId`; `pushClassroomGrade` CF with offline teacher creds). Full detail + file:line pointers in the Progress Log handoff entry.
...
**Next action for the next agent:** When Joel's **real ClassLink course** is live: (1) open the add-on in it → run the **"Check course alias (debug)"** probe → record the real alias + format (e.g. `d:<sourcedId>`); (2) confirm `sstudent25@orono.k12.mn.us` resolves in that class's OneRoster; (3) wire the **identity bridge** in `classroomAddonLoginV1` (course alias → class `sourcedId` for `classIds`; student email → OneRoster `sourcedId` → mint `HMAC(sourcedId)`) and make the teacher attach use the real `sourcedId` as `classId`. The name-resolution side (`getPseudonymsForAssignmentV1`) then needs no change. Reuse the OneRoster read pattern from [functions/src/index.ts:3620](../functions/src/index.ts). Independently, Phase 1A (types/rules/CSP) can still proceed.

**Blockers / open items:** ⚠️ **Identity bridge blocked on Joel's real ClassLink course** (alias-format confirmation). Open follow-ups: (1) trim the `[DEBUG-addonctx]` instrumentation **and** the temporary alias probe when the bridge lands; (2) district data-privacy review of the draft legal pages before any non-pilot rollout; (3) Phase 4 grade passback still needs `studentWorkReviewUri` + non-zero `maxPoints` (the attachment is minimal — no grade sync yet); (4) teacher attach currently mints `classIds:["classroom:<courseId>"]` — switch to the real ClassLink `sourcedId` once the alias is confirmed.
...
- [ ] **Trust anchor — call `getAddOnContext` server-side** with the user's access token (this replaces the old JWKS step):
  - [ ] `GET /v1/courses/{courseId}/{itemType}/{itemId}/addOnContext` (path segment is `addOnContext`, NOT `getAddOnContext` — see [§ Verified API facts](#-verified-google-classroom-add-ons-api-facts-2026-05-28)) (include `addOnToken` if present).
...
### 2026-05-29 — build session (Paul + Claude) — ClassLink identity bridge PROVEN live; PII-free named attribution works [VERIFICATION / IMPLEMENTATION] — HANDOFF

- **GO — named, PII-free attribution works end-to-end.** In the REAL ClassLink course (Google `courseId 866479643796` ↔ ClassLink class `sourcedId 241232135123123`): teacher attached a quiz → `sstudent25@orono.k12.mn.us` opened it in a fresh incognito window → the bridge resolved them via OneRoster to their `sourcedId`, minted `HMAC("sid:"+sourcedId)` (the SAME uid as ClassLink SSO), the class-gate authorized the join, and the teacher monitor shows their REAL NAME. No student PII persisted.
- **What's live (commits `68a2936d`, `0a0ecd78`, `661a080c`):** `classroomAddonLoginV1` bridge — reads `/classroom_course_links/{courseId}` → OneRoster `classes/{sourcedId}/students` → matches the student's verified email → mints `HMAC(sid:sourcedId)` + `classIds:[sourcedId]`. Teacher-attach (`TeacherDiscoveryRoute`) reads the same link and sets the assignment `classIds` to the sourcedId. `/classroom_course_links` rules + `ClassRosterMeta.googleClassroomCourseId` type. **Graceful fallback**: unlinked / email-unmatched / any error → Google-sub pseudonym + `classroom:<courseId>` (works, nameless). 20/20 CF tests.
- **Bug fixed en route:** the student route rendered the quiz on ANY persisted studentRole session → a stale session (other student/course) mounted with mismatched `classIds` → Firestore `Missing or insufficient permissions` on join. Fixed (`661a080c`): render only after a FRESH handshake this page load; also logs the minted `classIds` for diagnostics.
- **⚠️ TEST SCAFFOLDING (manual, must be removed):** the link doc `classroom_course_links/866479643796` was **seeded by hand** (Firebase MCP) to test before the link UI exists: `{classlinkClassId: "241232135123123", classlinkOrgId: "orono", teacherUid: "SDdYHGQ4PZNzt05gjlT0grvRTHG3", rosterId: "4UYk7BFFX0WDMYTa90R8"}`.
- **Why the link is teacher-declared, not alias-read:** real ClassLink Google Classroom courses carry the sourcedId in a **`p:`-scoped course alias** owned by ClassLink's project → SpartBoard's project cannot read it (`courses.aliases.list` returns `{}`). So the mapping must be declared by the teacher and stored (Task 1).

> **HANDOFF — 3 tasks for the next (fresh) agent:**
>
> 1. **"Link to Google Classroom" button (roster/classes UI)** — the last functional gap; removes the manual link-doc seed. Add per-ClassLink-roster in [components/layout/sidebar/SidebarClasses.tsx](../components/layout/sidebar/SidebarClasses.tsx) (~line 307, beside the RefreshCw sync button, inside the `classLinkEnabled` branch). Open a modal ([components/common/Modal.tsx](../components/common/Modal.tsx)) that: requests `classroom.courses.readonly` via GIS (reuse `ensureGis`/`requestAccessToken` from [components/classroomAddon/gisOAuth.ts](../components/classroomAddon/gisOAuth.ts) — consider moving it to `utils/gisOAuth.ts` since permanent code now uses it), calls `GET https://classroom.googleapis.com/v1/courses?teacherId=me&courseStates=ACTIVE`, lets the teacher pick, then writes `/classroom_course_links/{googleCourseId}` = `{classlinkClassId, classlinkOrgId, teacherUid, rosterId, createdAt}` AND `updateRoster(rosterId, {googleClassroomCourseId})` ([hooks/useRosters.ts:654](../hooks/useRosters.ts)). Rules already allow the teacher-claimed write; the scope is already on the consent screen.
> 2. **Trim spike scaffolding** (now that it works): the `[DEBUG-addonctx]` instrumentation in [functions/src/classroomAddonAuth.ts](../functions/src/classroomAddonAuth.ts); the "Check course alias (debug)" button + `COURSE_READONLY_SCOPES` in `TeacherDiscoveryRoute.tsx`; the `classIds=` diagnostic log in `StudentSpikeRoute.tsx`; and DELETE the hand-seeded `classroom_course_links/866479643796` once a real link replaces it.
> 3. **Grade passback (Phase 4):** make the attachment a grade-sync attachment (`studentWorkReviewUri` + non-zero `maxPoints` in `createClassroomAttachment`); persist each student's `submissionId` (the bridge already receives it from `getAddOnContext` but doesn't store it); add a `pushClassroomGrade` CF (`PATCH .../addOnAttachments/{attachmentId}/studentSubmissions/{submissionId}?updateMask=pointsEarned`) using stored OFFLINE teacher creds ([functions/src/googleOAuth.ts](../functions/src/googleOAuth.ts)) so the DRAFT grade auto-populates. See [§ Verified API facts](#-verified-google-classroom-add-ons-api-facts-2026-05-28).

### 2026-05-29 — build session (Paul + Claude) — real-Quiz pipe shipped + verified live; ClassLink-alias identity bridge designed [IMPLEMENTATION / DESIGN]

- **Real-Quiz pipe works end-to-end (verified live).** Teacher signs into SpartBoard in the discovery iframe (AuthProvider), picks a quiz, and attaches it; `createClassroomAttachment` now takes `quizCode` + `title` and builds `studentViewUri=.../student?code=<code>`. Teacher attach: `useQuiz` picker → `loadQuizData` → `createAssignment(quiz, {sessionMode:'student'}, {classIds:["classroom:<courseId>"]})` → join code. Student view renders the real `<QuizStudentApp/>` (SSO auto-join by `?code=`) once the studentRole token's classIds cover the course. Commit `64a9964d`. Inspected Firestore: response keyed by the studentRole pseudonym, `classId` gated, **no name/PII** — PII-free model confirmed working.
- **"Generic Student" in the monitor is NOT a bug — it's the PII-free design.** SpartBoard stores no names; the Classroom pseudonym `HMAC("classroom-sub:"+googleSub)` is a different namespace than ClassLink's `HMAC(sourcedId)`, so the monitor's name map misses.
- **ClassLink-alias identity bridge (Paul's direction — the real fix).** Plan: `course alias → ClassLink class sourcedId` (used as `classId`/`classIds`), and `student verified email → that class's OneRoster roster → sourcedId`, minting `HMAC(sourcedId)` — i.e. the SAME uid as ClassLink SSO. Then `getPseudonymsForAssignmentV1` resolves the name unchanged, PII-free. **Feasibility CONFIRMED:** `getPseudonymsForAssignmentV1` ([functions/src/index.ts:3620](../functions/src/index.ts)) already reads OneRoster server-side via district secrets (`CLASSLINK_CLIENT_ID/SECRET/TENANT_URL`) with no live teacher — `classroomAddonLoginV1` can reuse that at login time.
- **Course-alias probe added** (`chore` commit `ad62384f`): a "Check course alias (debug)" button in teacher discovery calls `courses.aliases.list` with `classroom.courses.readonly`. On the hand-made test course `793938994099` it returned `200 {}` (no alias — expected; ClassLink never provisioned it). Requires `classroom.courses.readonly` declared on the consent screen (done).
- **BLOCKED on a real ClassLink course.** Joel (Workspace super-admin) is provisioning a real ClassLink course with Paul as teacher + `sstudent25@orono.k12.mn.us` as student. Next step when live: run the alias probe IN that course to capture the real alias + format, confirm `sstudent25` is in OneRoster, then wire the bridge. Remove the `[DEBUG-addonctx]` + alias-probe scaffolding when the bridge lands.
