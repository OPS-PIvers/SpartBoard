...
**Totals (verified):** ~17 genuinely-open items below · **9 found already-shipped** (removed) · **7 narrowed** · **35 completed docs deleted** (24 first pass + 11 verified-complete plans). **2026-06-24 burndown:** LO1 + Q2 shipped; M4 partially shipped (bulk Resend invite) — see §6. **2026-06-25:** Q1 + Q3 verified already-done against live prod (§5); L2 announcements-backfill closed as not-needed.
...
| Q2  | ✅ 🔒 **Announcements query scoped by `orgId` — SHIPPED 2026-06-24** (§6). Server-side org filter (`where('orgId','==',orgId)`, single-field index — no composite/deploy gate); org-less users unchanged. No backfill needed (Paul, 2026-06-25) — the scoped query already keeps legacy unstamped docs out of any 2nd org's results, and orono doesn't need old docs to resurface. | `components/announcements/AnnouncementOverlay.tsx:391`    |  High  |  S  |
...
| L2  | ✅ **Announcements backfill — NOT NEEDED** (Paul, 2026-06-25). Listener scope shipped (§6); migration unnecessary — Q2's `where('orgId','==',orgId)` query already excludes orono's legacy unstamped docs from a 2nd org's results, and orono doesn't need old docs to resurface.             | `docs/external-availability-journal.md`            |  S  |
...
| Q2  | Announcements org-scoping      | Listener queries `where('orgId','==',orgId)` when orgId is set (single-field index, no composite/deploy gate) + filters `isActive` client-side; org-less/loading users keep the legacy `where('isActive','==',true)` query. 🔒 data-isolation. Backfill of legacy docs deemed not needed (Paul, 2026-06-25). |
