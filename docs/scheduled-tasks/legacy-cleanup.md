...
- **Detected:** 2026-09-20
- **File:** utils/paperSheetMarker.ts:156
- **Detail:** `export function markerFitsGrid(): boolean { return TOTAL_BITS === MARKER_CELL_COUNT; }` is a compile-time invariant check ("True when the grid is sized to hold exactly one encoded payload") added this week as part of the paper-answer-sheets marker-encoding work. It has zero references anywhere outside its own file except `utils/paperSheetMarker.test.ts:9,26`, which calls it once as a sanity assertion — never wired into any production path. Same shape as the existing `videoActivityDriveService.ts` open item below (tested but never wired into a real code path).
- **Fix:** Either delete it (the invariant it checks is static — `TOTAL_BITS`/`MARKER_CELL_COUNT` are both compile-time constants, so a runtime assertion buys little), or move the check into a `console.assert`/dev-only guard at module load if the intent was to catch a future constant-mismatch regression, and drop the standalone exported function.

### LOW `utils/plcNoteCrdt.ts:59` — `isNoteDocEmpty` dead export (test-only, no production call site)

- **Detected:** 2026-09-20
- **File:** utils/plcNoteCrdt.ts:59
- **Detail:** `export function isNoteDocEmpty(doc: Y.Doc): boolean` (checks whether a PLC collaborative-note CRDT doc has empty title/body/action-items) has zero references within its own file, in `hooks/usePlcNoteCrdt.ts`, or anywhere else in production. Its only other reference is `tests/utils/plcNoteCrdt.test.ts:10,378,380`. Part of this week's new PLC collaborative-notes CRDT groundwork, so plausibly intended for an "is this note worth showing/saving" check that hasn't been wired into the notes UI/hook yet, rather than orphaned cruft.
- **Fix:** Either wire it into `usePlcNoteCrdt`/the notes list UI (e.g. to skip rendering/saving empty notes) or delete it if that behavior isn't planned.

_2026-09-20 audit notes (Sunday, weekly D2): Read the full journal plus diffed `9513cee..HEAD` for `utils/`/`hooks/`/`components/`/`context/` since the 2026-09-13 audit (130 commits — a very active week shipping Flashcards, Projects, and Paper Answer Sheets/OCR, plus quiz-translation increments). Checked every named export in all 60 net-new (non-test) `utils/`+`hooks/` files for outside-file/outside-own-test references; found two genuinely dead ones (above), both test-only. All previously tracked items re-verified still reproducing, none resolved: `migrateLocalStorageToFirestore` still called unconditionally at `context/DashboardContext.tsx:2547` (line drifted from :2529, same guard logic); `utils/periodCompat.ts:28` `buildPeriodFields` still zero call sites; root `scripts/*.js` bucket unchanged (two new one-shot scripts this week — `backfill-quiz-join-codes.mjs`, `build-standards.mjs` — fit the existing pattern, not wired into CI, no new item; `checkRulesSize.mjs`/`stripRulesComments.mjs`/`mergeVitestReports.mjs` are new but all wired into `package.json`/CI, ongoing not legacy); `hooks/useScaledFont.ts` still dead; `utils/imageProcessing.ts:109` console.warn-on-success still present; `utils/migrateProportionalLayout.ts` `dashboardNeedsProportionalMigration` still dead; `hooks/usePlcPresence.ts` `usePlcStandalonePresence` still dead; `utils/learningTargets.ts` `EMPTY_LEARNING_TARGET_LIST` still dead; `utils/standardsCatalog.ts` `isStandardTagId` still dead; `utils/videoActivityDriveService.ts` still only referenced from its own test. `scripts/tools/` confirmed still absent. Legacy type strings `'timer'`/`'stopwatch'`/`'workSymbols'` confirmed to appear only in `utils/migration.ts`. Zero `console.log()` found in components/, context/, hooks/, utils/. Commented-out-code sweep (91 raw candidates from a code-likeness heuristic, none in newly added files) — all manually-inspected survivors are legitimate architecture/rationale prose, zero real disabled-code blocks. **Net this cycle: 2 new LOW issues, 0 resolved.**_
