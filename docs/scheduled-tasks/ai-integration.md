...
_2026-07-10 (action): Resolved the MEDIUM `guided-learning` AI stale-entry item — highest-priority Open across today's Friday reading list (no HIGH anywhere; the three daily journals widget-registry/css-scaling/typescript-eslint had no HIGH/MEDIUM open items, so the daily-before-weekly tiebreak left this ai-integration MEDIUM as top; it precedes the RevealGrid MEDIUM in document order). File-recency check passed: `functions/src/aiGeneration.ts` (where `generateGuidedLearning` actually lives — the journal's `index.ts:1823` refs were stale since the AI functions were extracted to `aiGeneration.ts`) last touched at 37f57855, far outside the last 5 branch commits. Chose the journal's documented option (b): DOCUMENT the design intent rather than add an admin rate limit. Rationale: `generateGuidedLearning` is admin-ONLY (non-admins get `permission-denied` before any model call), and admins are exempt from the daily `ai_usage` cap in EVERY Gemini Cloud Function (`generateWithAI`, `generateVideoActivity`, `transcribeVideoWithGemini` all short-circuit for admins — see the `isExternalCaller` docblock). Adding a per-admin cap only here would be inconsistent with that uniform policy and would throttle the intended admin authoring workflow — i.e. option (a) would introduce a NEW inconsistency, not fix one. Added a thorough docblock above `generateGuidedLearning` explaining the admin-only gate and the deliberate no-rate-limit design, and noting the `canAccessFeature('guided-learning-ai')` client-gate path as the correct future extension if per-building disablement is ever needed (keeps the admin-exempt policy uniform). Comment-only source change; `tsc --noEmit` (exit 0), `eslint` (exit 0), `prettier --check` (clean). Journal table row updated. PR opened to dev-paul._
