...
- **File:** firestore.rules:574-578, components/admin/GlobalPermissionsManager.tsx:572
...
- **Resolution:** Added a `match /admin_audit_log/{logId}` block in `firestore.rules` adjacent to `admin_settings`, restricting both read and write to admins via the existing `isAdmin()` helper (matches the access model of `/admin_settings/{document=**}`). Added a short comment explaining the consequence of omitting the rule so future audits don't re-flag or accidentally remove it. Will require a `firebase deploy --only firestore:rules` to take effect in production.
