...
- Comments processed: **all 19 inline review threads across the 21 PRs were already resolved** (17 on #2395, one each on #2509 and #2511), so as on the previous run the actionable surface was the PR-level structured reviews, which have no thread and therefore no resolved state to check. Twelve reviews were posted between 06:33 and 06:45 today; six carried an actionable item.
  ...
  - #2508 (`mt-4` → `marginTop: '1em'` not px-equivalent) — **already answered** 2026-08-20 06:28. No re-reply, per the frugality directive.
  - #2506 (`*LabelId` applied as control ids) — **already fixed** in `1d132c4` with a correction comment at 06:39. No action.
  - #2507 (`Dock.tsx:1586` unresolved 33+ runs), #2505 (merge sequencing), #2395 (deploy gate) — **replied explaining inaction**, see below.
  - #2517–#2520, #2522, #2523, #2513, #2514, #2515, #2504, #2509–#2512 — reviews found no code issues, or the PR is a superseded ancestor whose fix belongs on its superset. No reply.
- Fixes pushed: **2.**
  - **#2521 / `nightly/build-tooling-2026-08-21` / `97fb6c9`** — `fix(rules): pin pollVotes writes to exactly +1 per write`. The shape-only check (`count is int && count >= 0`) still admitted any non-negative integer, so an authed client could `setDoc({count: 0})` to wipe a live tally or `{count: 999999}` to inflate one in a single write. Split `create, update` into separate rules: a create must land on `1`, an update on `resource.data.count + 1`, with explicit `is int` guards on both sides so a non-int legacy `count` fails closed on a comparison rather than an arithmetic error. Comment above the block rewritten — the old one claimed the arbitrary-value protection this change is only now actually providing.
  - **#2516 / `nightly/unify-d3-settings-labels-2026-08-21` / `415f62f`** — `test(a11y): pin TalkingTool stems group accessible name`. Two cases in a new `tests/components/admin/TalkingToolConfigurationPanel.labels.test.tsx`, following the existing `TimeToolConfigurationPanel.labels.test.tsx` template in the same directory.
- Verification before each push (Node 22; CI on Node 24 authoritative):
  - #2521 — **the load-bearing assumption was tested, not assumed.** The rule only works if `request.resource.data.count` holds the *resolved* value of `increment(1)` rather than the unresolved sentinel; if it held the sentinel the rule would deny every legitimate vote, i.e. a total outage of announcement poll voting rather than a degradation. Pinned by the two pre-existing control tests — `setDoc({count: increment(1)}, {merge: true})` still succeeds against both an existing tally and a fresh option — which pass, so transforms are resolved before rules evaluate. Six new tests, fail-before/pass-after confirmed by running them against the pre-fix rule: 5 fail (reset-to-zero, inflate-to-999999, `increment(-1)`, `increment(5)`, create-at-42), 12 pass. Full `pnpm run test:rules` on the emulator → **48 files / 1162 tests** (was 1156); `scripts/test-count-baseline.json` `rules` bumped 1156 → 1162. `type-check` ✓ · `lint --max-warnings 0` ✓ · `format:check` ✓. **All 7 CI checks green on `97fb6c9`, including Firestore Rules Tests.**
  - #2516 — fail-before confirmed against the pre-fix panel on `dev-paul`: both cases fail with `Unable to find an accessible element with the role "group" and name "Sentence Stems"`; both pass on this branch. `type-check` ✓ · `eslint --max-warnings 0` ✓ · `format:check` ✓. No baseline bump — `checkTestCounts.mjs` fails only on a drop, so growth needs none, and editing it would add a needless conflict against the other branches open on `dev-paul`.
- New findings this run:
  - **#2521: the deliberate hole in the tightened rule, pinned as a test rather than left implicit.** The rule authorises the *value*, not the transform, so a hand-rolled `{count: 6}` over a tally of 5 is indistinguishable from a real vote and stays allowed. Asserted explicitly so it reads as intended behavior rather than as a gap someone later "fixes" by trying to detect the transform — which rules cannot do.
  - **#2521: `PollWidget.vote()` remains the sole write path**, checked rather than assumed. The only other `pollVotes` consumer, `PollResponsesPanel` (`components/admin/Announcements/Widget.tsx:554`), is `onSnapshot` + CSV export, read-only. The documented legacy-doc `hasOnly` caveat is unaffected by this change.
  - **#2516: a single-group assertion would have missed the likelier regression.** This panel renders one group per category in a `.map()`, so the plausible future break isn't a dropped attribute — it's someone replacing the `cat.id` interpolation with a constant. That still yields a group with the right accessible name, so a one-case test stays green while every group silently points at the first category's label. The second test asserts distinct label ids per group and that each resolves inside its own subtree.
  - **#2505: the review's collapse arithmetic is off by one.** It says landing the five superset PRs and the log-stack tip "collapses 19 down to 11." Counting the actual containment relationships — #2509⊂#2517, #2510⊂#2518, #2511⊂#2520, #2512⊂#2521, #2513⊂#2522, #2504⊂#2514⊂#2515 — seven PRs close automatically, not eight, because #2515 is the stack tip and gets merged rather than auto-closed. The queue lands at **12**. Recommendation unaffected.
  - **#2395: one item on the review's pre-merge checklist is already closed.** The 2026-08-17 review flagged the in-code `verify global-endpoint support before merge` comment on the `gemini-2.5-*` picker entries as a genuine blocker sitting in a comment. On the current head `GlobalPermissionsManager.tsx:237` now reads `2.5-* kept — Google's Vertex locations doc lists both as global-endpoint models` — provisional wording gone, entries kept on a stated basis. No open code item remains; the blockers are the two GCP console actions plus a live model-resolution smoke test.
- Inaction explained rather than left silent (3 replies):
  - **#2507 / `Dock.tsx:1586`.** The review argues an item surviving 33 re-confirmations should either be filed as a catalogued exception or given an owner, citing run 63's structural fix of the D3 dialog item (#2519) as precedent. The two are different shapes: the dialog item was *unreached* (a correct fix, `key={currentDialog.id}`, was available the whole time — nobody had to decide anything), whereas this one is *undecided* — the analysis is complete and the decision is what's missing. Re-verified the row's load-bearing claim rather than trusting 33 prior confirmations: the FAB is `rgba(45, 63, 137, ${globalStyle.dockTransparency + 0.4})`, a computed alpha, so it needs an RGB **triplet** var; `--spart-primary` is set as a full color (`DashboardView.tsx:1311,1342`); repo-wide grep for `--spart-primary-rgb` still returns **zero hits**. Converting the FAB alone would make it diverge from the two sibling Dock icons using static `bg-brand-blue-primary` the moment an admin customizes a theme — moving the inconsistency, not fixing it. Filing it as an exception would resolve it by relabelling, which the row itself argues against. Noted that `GROUP_BRAND_RGB` (run 56) is now the same shape and both close on one product question — whether admin themes should reach alpha-blended chrome — so they're worth pairing into a single decision rather than two nightly rows.
  - **#2505.** `dev-paul` is this PR's head branch and outside this task's write scope; the review says so itself. Its only open item is merge sequencing, which no commit on this PR can express. Replied with the arithmetic correction above and a note that #2521 and #2516 both moved tonight.
  - **#2395.** All four remaining blockers are operational and invisible to CI by construction (two GCP console actions, live model-ID resolution on `location: 'global'`, YouTube `fileData.fileUri` behavior on Vertex). Writing tests against them would mock the thing under test. Replied to record that the in-code checklist item is closed, since that shortens the human's pre-merge list.
- Reviews posted: **0 new structured reviews.** Twelve had been posted against unchanged heads earlier the same day; re-reviewing the same diffs would have buried this run's replies and fixes.
- Notes:
  - Only two of the six actionable review items were still open when this run started — the other four had been fixed and replied to by an earlier pass today (#2508 `2fff1ab`, #2506 `1d132c4`). Checked each PR's existing comments before replying rather than re-answering settled items.
  - The `pollVotes` tightening was the item the 06:34 review explicitly classified as "a deliberate scope call, not a defect." It was still worth doing: the reviewer's own suggested rule was concrete and correct, and the gap it closes — a one-write tally reset — is destructive rather than merely permissive. Scope is unchanged (still not per-voter dedup); what changes is the worst case per write, from arbitrary to +1.
  - This log commit is on the designated `claude/inspiring-cannon-volvys` branch, **stacked on #2515's head** (`55e9c37`), continuing the established precedent — `pr-review-log.md` is append-only with a nightly writer, so branching fresh from `origin/dev-paul` would guarantee a trailing-line conflict while #2515 stays open. The stack is now four deep (#2504 ⊂ #2514 ⊂ #2515 ⊂ this); merging the tip lands all four in order. Also continues keeping this log **off** `scheduled-tasks`, still the head of open PR #2508.
  - Branch safety: no push to `main` or any `dev-*` branch. Both fixes went to `nightly/*` PR heads whose PRs are open and draft.
  - Tooling: GitHub via the MCP server (no `gh` CLI); all PR list/read/comment operations used `mcp__github__*` equivalents. Firestore emulator ran locally for the rules verification (Java 21 present).
  - Verification env runs Node 22 (repo pins 24, "Unsupported engine" warning); CI on Node 24 remains the authoritative gate.
