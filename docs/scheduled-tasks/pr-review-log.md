...
- PRs reviewed: 10 (all open PRs; every head branch eligible — none is `main` or `dev-*`, and #2493's head `dev-paul` is read-only for pushes but reviewable)
  - #2501 — docs(routines): log nightly debugger run 44 (head `nightly/debugger-log-2026-08-19` → `dev-paul`) — `docs/routines/debugger.md` only.
  - #2500 — fix(css-scaling): scale RevealGrid header/grid spacing with cqmin (head `scheduled-tasks` → `dev-paul`) — `RevealGrid/Widget.tsx` + 3 Wednesday audit-log docs.
  - #2499 — fix(library): stop Escape leaking from overflow menus to the global widget handler (head `nightly/dashboard-layout-2026-08-19` → `dev-paul`).
  - #2498 — fix(stations): key Send-Groups-to-Stations assignments by roster id, not name (head `nightly/widgets-2026-08-19` → `dev-paul`).
  - #2497 — fix(admin,widgets): poll CSV export filenames use local date, not UTC (head `nightly/admin-config-2026-08-19` → `dev-paul`).
  - #2496 — fix(pii): scrub name-keyed assignments and jigsaw groups from Firestore (head `nightly/state-data-2026-08-19` → `dev-paul`).
  - #2495 — docs(routines): log nightly unifier run 61 (head `nightly/unifier-log-2026-08-19` → `dev-paul`) — `docs/routines/unifier.md` only.
  - #2494 — fix(a11y): pair 3 more orphaned SettingsLabel controls with htmlFor/id (head `nightly/unify-settings-labels-2026-08-19` → `dev-paul`).
  - #2493 — Audit and test coverage improvements with dependency fixes (head `dev-paul` → `main`) — 20-commit rollup of #2475-#2492.
  - #2395 — feat(ai): move Gemini to Vertex AI, update model IDs (head `claude/quirky-ritchie-wghdl3` → `dev-paul`). Draft, still gated on preview-deploy smoke test.
- Comments processed: 3 total — 0 fixed, 1 explained + resolved, 2 deliberately left open with no reply. Eight of the ten PRs (#2501, #2500, #2499, #2497, #2496, #2495, #2494, #2493) carried **zero** open review threads between them.
  - #2498 thread `PRRT_kwDOQr0RBM6aW1Sl` (`nexus.ts` doc-comment placement): NO FIX NEEDED — already corrected on the branch by `24ec261`, which landed 20 minutes after the comment was written. Verified on the current head that `shouldResolveRosterNames` carries its own one-liner and the `@param` block sits above `buildStationsFromRandomGroups`. Replied with that evidence and resolved the thread.
  - #2395 threads `PRRT_kwDOQr0RBM6Wr0D7` / `PRRT_kwDOQr0RBM6Wr0Mb` (Vertex YouTube constraints on `generateVideoActivity` / `transcribeVideoWithGemini`): NO FIX POSSIBLE IN CODE — both already carry a substantive owner reply, and the open item is an operational preview-deploy gate, not a diff defect. No duplicate reply posted (frugality directive); left unresolved deliberately so the gate stays visible, and restated the reason in the PR review instead. This reverses the prior three runs' standing recommendation to bulk-resolve them: with the count now down to 2 and both tracking the same single blocker, they read as an accurate open-work marker rather than as noise.
- Fixes pushed: 0 — no comment on any open PR identified a defect in current code. Every actionable finding this run was surfaced through review rather than a push, because each one is a judgement call the author should make (see below), not a mechanical correction.
- Reviews posted: 10 — one structured automated review per open PR (each carries the automated-review disclaimer + Claude Code attribution footer). Merge-readiness calls:
  - Ready: #2501, #2495 (docs-only, both cross-checked against the code they describe), #2497, #2494.
  - Ready with minor notes: #2500, #2499, #2498, #2496, #2493.
  - Needs changes: #2395 — blocked on the preview-deploy smoke test, nothing outstanding in the diff.
- Verification performed per branch before reviewing (not merely asserted): `pnpm run type-check` exit 0 on all six code-bearing branches; targeted `eslint --max-warnings 0` exit 0 on every changed source file; targeted `vitest run` green on each PR's own suites — #2498 17/17, #2496 20/20 (both test files), #2499 65/65, #2497 17/17, #2500 1/1, #2494 19/19, #2395 41/41 root + 30/30 functions with `functions/` `tsc --noEmit` exit 0.
- Substantive review findings this run:
  - **#2498 (highest-value):** the `?? rosters[0]` fallback at `RandomSettings.tsx:114` reopens, by a different route, the exact misattribution `shouldResolveRosterNames` was added in `a0ed201` to close. It is inconsistent with the same file's own `activeRoster` lookup at `:141-143`, which has no fallback — so when `activeRosterId` is stale and the teacher has multiple rosters, the name→id map is built from an arbitrary *other* class and a name collision keys the assignment to an unrelated student's id. One-line fix proposed (drop the fallback, degrade to name keys). Ruled out two adjacent theories first: `StationsConfig` has no per-widget `rosterId` (both widgets read the same dashboard-level `activeRosterId`, so the predicate's premise does hold), and `rosterMode` defaults to `'class'` in `widgetDefaults.ts`, so treating `undefined` as non-custom is correct rather than accidentally permissive.
  - **#2499:** verified the fix actually works rather than assuming — the global handler is `window`-bubble (`DashboardView.tsx:1070`), the new handlers are `document`-bubble, and `document` precedes `window` in the bubble path, so `stopPropagation()` genuinely intercepts. Checked the two capture-phase neighbours too: `DraggableWindow.tsx:417` is gated on `showMaxMenu` (no live conflict) and `Modal.tsx`' `captureEscape` prop has **zero** call sites repo-wide (latent only). The real finding is reuse: this is now the third independent implementation of one pattern (`Modal.tsx` `captureEscape`, `primitives.tsx` `useCaptureEscape` from #2488, and two fresh inline copies here), with two different contracts (capture + `stopImmediatePropagation` vs bubble + `stopPropagation`). `useCaptureEscape` is module-private, which is *why* this PR couldn't reuse it — recommended promoting it to `hooks/useCaptureEscape.ts`.
  - **#2496:** traced the full round-trip through `DashboardContext.tsx` to confirm all three call sites were updated consistently (`scrubDashboardPII` `:1276`/`:1290`, `extractDashboardPII` `:1245`, `dashboardHasPII` `:1244`) and that `mergeDashboardPII` is field-agnostic so the restore path needs no change. Flagged the consequence the PR body does not state: the supplement save is gated on `driveService`, so for teachers without Drive this newly extends silent data loss to hand-arranged custom-mode station assignments — a more visible loss than the name lists already covered. Also noted the module has two parallel test files and only the colocated one received the new cases.
  - **#2497:** the sweep is correct but pattern-scoped — three UTC-derived user-visible date strings remain (`MiniApp/Widget.tsx:881` uses `.slice(0, 10)` instead of `.split('T')[0]`, which is precisely why a `split('T')` grep misses it; `Dock.tsx:252`; `DomainsView.tsx:299`).
  - **#2500:** the header this PR rebalanced is now internally inconsistent — `Widget.tsx:157`'s "Vocabulary Review" title has no font size at all while the "Show Answers" label beside it was scaled to `min(12px, 4.5cqmin)`, so small widgets read worse than before the change. Confirmed the container context is real before advising: `containerType: 'size'` sits on the `DraggableWindow` root (`:2183`), not the content area, so header `cqmin` resolves against the same container as the grid.
  - **#2493:** re-checked rather than re-derived — head unchanged since the 2026-08-18 review, so that LGTM stands. One new finding: `gcPlcOrphans.ts` carries an 18-line and a 28-line comment block (the latter mostly a `TRADE-OFF:` essay), the same shape fixed nine times on #2395 and again in `e45682d`. This PR is the gate to `main`, so it is the last chance before that becomes the in-repo precedent. Also noted `.offset(N)` still bills the skipped reads — the change bounds memory, not read cost (immaterial at `VERSION_HISTORY_LIMIT = 10`, but the comment implies otherwise).
  - **#2395:** singled out `geminiModelDeprecationParity.test.ts` as a pattern worth reusing — it imports both the client mirror and the server predicate, asserts they agree across 20 well-formed ids, and adds a second assertion that the corpus contains both accepted and rejected ids so parity cannot pass vacuously. That is the right guard for a duplication that genuinely cannot be removed.
- Notes:
  - Branch-safety: no push to `main` or any `dev-*` branch this run. Zero code pushes anywhere.
  - Cross-PR coordination flagged on #2493 and #2496: three PRs (#2486 merged, #2498, #2496) touch the Stations `assignments` key space. They are complementary — #2486/#2498 make class-mode keys opaque ids, #2496 removes the custom-mode name keys that cannot be made opaque — but merge order matters and whichever lands last should re-run the Stations suites against the combined state.
  - **Log placement deviates from the literal POST-TASK instruction, for the same reason as the 2026-08-12 and 2026-08-13 entries:** `scheduled-tasks` is currently the head branch of actively-open PR #2500, and its copy of this log is byte-identical to `dev-paul` (verified: `git diff origin/dev-paul origin/scheduled-tasks -- docs/scheduled-tasks/pr-review-log.md` empty). Committing here would inject an unrelated doc into a PR under review. Logged instead on the designated `claude/pensive-bell-otg6s3` branch, rebuilt from the latest `origin/dev-paul` (the prior branch head contained only already-merged history, 25 commits behind).
  - Tooling: this environment exposes GitHub via the MCP server (no `gh` CLI); all PR list/diff/comment/review operations used `mcp__github__*` equivalents of the prescribed `gh` commands.
  - Verification env runs Node 22 (repo pins 24, "Unsupported engine" warning); CI on Node 24 remains the authoritative gate.
