...
- PRs reviewed: **8** (every open PR). All eight target `dev-paul`, all eight are drafts, and every head branch was a `nightly/*` or `claude/*` branch — none was `main`- or `dev-*`-headed, so all were writable.
  - **#3183** fix(talking-tool): remove dead fontFamily/fontColor picker (`claude/zen-mccarthy-gemed7`)
  - **#3182** docs(debugger): log nightly run 76 — 5 fixes shipped (1 SECURITY) (`nightly/debugger-log-2026-09-20`)
  - **#3181** fix(rules): SECURITY — `help_resources` read no longer bypasses org scope via bare `isAdmin()` (`nightly/build-tooling-2026-09-20`)
  - **#3180** fix(admin): add accessible label to Video Activity's AI-mode toggle (`nightly/admin-config-2026-09-20`)
  - **#3179** fix(auth): `isBetaUser` no longer matches a blank email against a blank list entry (`nightly/state-data-2026-09-20`)
  - **#3178** fix(boardsModal): Escape in Move-to-Collection popover no longer closes the whole modal (`nightly/dashboard-layout-2026-09-20`)
  - **#3177** fix(quiz): surface a retry banner when recording-quiz submit fails (`nightly/widgets-2026-09-20`)
  - **#3176** audit(sunday): daily + weekly scheduled-task journal updates (`claude/fervent-curie-c6qwlg`)
- Comments processed: **6 — 0 fixed, 0 explained, 6 no action.** All eight PRs returned `totalCount: 0` inline review threads. Six carried a single PR-level `claude[bot]` review apiece (#3183, #3181, #3180, #3179, #3178, #3177), every one of them an approval with no change request; #3182 and #3176 had no reviews at all. The only thing resembling a finding was #3177's explicitly non-blocking note that `'Retry Submit'` is hardcoded rather than routed through `t(...)` — verified against the file and it is correct not to act on: **all four** pre-existing `saveError` button labels (`QuizStudentApp.tsx:3400`, `:3497`, `:3698`, `:3943`) are hardcoded identically, so translating only the fifth would reduce consistency. No reply was posted to any of the six, since replying "no fix needed" to an unqualified approval is noise. **Eighth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect. The one actionable finding produced by independent review (#3181's predicate ordering, below) is a one-line change on another agent's open PR, so it was raised in the review for its author rather than pushed.
- Reviews posted: **8** (one structured review per open PR). Findings that came from verifying claims against the codebase rather than restating the diff:
  - **#3181 — the security fix is right, but it silently made the common read path more expensive.** The core change is correct and I confirmed the premise directly: `isAdmin()` (`firestore.rules:13`) is a bare-existence check on `/admins/{email}`, which org-scoped roles like `building_admin` also hold, so `helpReaderOk()` really did let an org admin read another org's `help_resources`; `isSuperAdmin()` (`:120`) is the correct gate. **But the new clause reads `orgId == null || isSuperAdmin() || isOrgMember(orgId)`, and `firestore.rules:263` documents the opposite ordering in as many words** — *"Ordering: `isOrgMember()` (one `exists()`) precedes `isSuperAdmin()` (`exists()`+`get()`) so the common teacher path stays cheap."* Checked both definitions: `isOrgMember()` (`:229-233`) is a single `exists()`, while `isSuperAdmin()` fans out through `isLegacySuperAdmin()` to a `get()` on `admin_settings/user_roles`. So every org teacher reading an org-scoped help item now pays a document read that the old rule never charged. Semantics are identical either way, making the swap free. Also confirmed two things that look like gaps but aren't: `helpScopeOk()` (`:3988-3991`) is immune because its branches are mutually exclusive on `orgId`, and the bare `isAdmin()` still in `allow update` (`:4019-4023`) is already org-scoped by its `helpScopeOk(resource.data)` conjunct.
  - **#3183 / #3176 — a merge conflict that is also a factual contradiction, found only by diffing the two PRs against each other.** Both edit `docs/scheduled-tasks/admin-settings-alignment.md` and both insert immediately after the `## Open` heading, so the second to merge conflicts mechanically. The substantive half is worse: #3183 *deletes* the `MEDIUM TalkingTool: fontFamily AND fontColor pickers…` block because it fixes it, while #3176's new 2026-09-20 audit note asserts that same item *"still reproduce[s] unchanged, untouched by this week's commits."* True when the audit ran, false the moment #3183 lands. Resolving the conflict by taking either side wholesale would either drop this cycle's audit note or silently resurrect a completed item into Open — the exact failure mode CLAUDE.md's "Editing a plan" rule describes. Recommended on both PRs: merge #3183 first, rebase #3176, amend the sentence to reference only Clock `dateColor`.
  - **#3182 — spot-checked the journal's most consequential new claim instead of accepting the self-report, and it holds.** The newly filed `announcements/{announcementId}` finding is real: `firestore.rules:3904-3913` carries the identical trailing `|| isAdmin()` disjunct that #3181 is removing from `helpReaderOk()`, with the same cross-org read consequence for a `building_admin`. Added a caveat for whoever fixes it — the in-file comment ("or any admin (the manager UI subscribes to the whole collection)") suggests the bypass was deliberate for the manager UI, so that fix likely needs to preserve super-admin collection-wide reads rather than being the same one-line swap. Also confirmed the strikethrough of the prior `help_resources` entry is accurate.
  - **#3179 — checked the reordering for the regression it could easily have introduced, and it is clean.** Hoisting `roleId === 'super_admin'` from the last disjunct of the `||` chain to an early `return true` is semantically inert, and critically it sits *before* the new `if (!lowerEmail) return false`, so a genuine super admin with no email still passes — the naive version of this fix breaks exactly there, and the third added test pins it. Also traced the `email === ''` (empty string, not nullish) input, which the description doesn't mention: previously `lowerEmail` was `''` and matched blank entries, now `''` is falsy and short-circuits. Same bug class, also closed.
  - **#3178 — verified the handler-ordering claim against both files rather than trusting the comment.** `BoardsModal.tsx:167` registers its `keydown` listener with no capture flag, so it is bubble-phase; the new listener passes `true`. Capture precedes bubble on the same target, and `stopImmediatePropagation()` prevents the bubble pass entirely — the mechanism works as described. Cleanup correctly repeats the `true` flag, which is the usual place this pattern breaks. Flagged one low-probability edge: `stopImmediatePropagation()` on a *document-level capture* listener suppresses every later-registered document Escape handler app-wide while the menu is mounted, so if this popover and `CollectionColorPicker` (same directory, same pattern) could ever be open together, whichever mounted first would win and the other would be keyboard-unreachable. Looks unreachable given the `BoardsModal` structure — noted because the pattern is now used twice in one directory and a third user would make it real.
  - **#3177 — confirmed scope and reuse, which is where this kind of patch usually goes wrong.** `saveError`/`setSaveError` are declared at `QuizStudentApp.tsx:2040`, inside the same `ActiveQuiz` component as both the modified handler and the modified JSX, so no scope error. The error copy is byte-identical to the existing message at `:2820`. `setSaveError(null)` is placed *after* the `submitBlocked` early return, so a blocked submit cannot clear a real error. Banner gating (`currentIndex >= effectiveTotalQuestions - 1`) matches the only branch that sets `saveError`, so a mid-quiz "Next" can't render a stale banner.
  - **#3180 — verified both ends of the accessibility claim.** `Toggle` accepts `label` and renders it as `aria-label` (`components/common/Toggle.tsx:37`, `:71`), and the visible heading beside the switch is exactly `Enable AI Mode` (`VideoActivityConfigurationModal.tsx:256`), so the accessible name matches the visible text rather than drifting from it. Confirmed placement is already correct for admin-config standards — `aiEnabled` is a global Gemini gate ("hidden from non-admins if disabled"), belongs in the Feature-Permissions modal, and this PR doesn't move it. Noted that `aria-labelledby` would be drift-proof but is a `Toggle` API question, not a per-call-site one, and deviating here would break consistency with the eight sibling fixes already shipped.
- Cross-PR checks run this review:
  - **One merge conflict found, between #3183 and #3176** (both at the `## Open` heading of `docs/scheduled-tasks/admin-settings-alignment.md`) — see above. All other file sets are disjoint: #3182 `docs/routines/debugger.md`; #3181 `firestore.rules` + `tests/rules/`; #3180 `components/admin/`; #3179 `utils/betaAccess.*`; #3178 `components/boardsModal/`; #3177 `components/quiz/`. Merge order is otherwise unconstrained.
  - Base freshness: all eight head branches sit on `dev-paul`'s head `099d069`. `dev-paul` did not move during the run.
  - CI: **six of eight green across all 14 checks** on their current heads, including Claude Code Review. #3182 and #3176 have **zero check runs** on their heads — consistent with a docs-only path filter, but it means neither carries a green signal to read, which was noted on both as information for the merger rather than a defect.
  - **Two of eight PRs carry no production code at all** (#3182, #3176 — both journal appends). Of the six code-touching PRs, every one shipped tests with it, and the tests are well targeted rather than perfunctory: #3178's middle test registers an outer bubble-phase listener *before* mounting to pin the ordering guarantee itself, and #3181's pair covers both the vulnerable case and the over-tightening guard. Coverage was judged adequate on all six; no test-absence gaps were filed this run.
  - Convention drift worth one pass: three of the six code PRs (#3181's test file, #3179's `utils/betaAccess.ts` and its test) carry 4–6 line comment blocks where CLAUDE.md caps diff comments at one short line, with the stated exception scoped to `firestore.rules` itself. Raised as a nit on each, not a blocker — the rationale in all three is good, it just belongs in the PR description.

## 2026-09-18

- PRs reviewed: **4** (every open PR). All four target `dev-paul`, all four are drafts, and every head branch was a `nightly/*` or `scheduled-tasks` branch — none was `main`- or `dev-*`-headed, so all were writable.
  - **#3104** Friday scheduled tasks — journal audits + AI permission enforcement fix (`scheduled-tasks`)
  - **#3103** docs(unifier): log run 90 — 2 shipped (D1, D3), 3 aligned, 0 collisions (`nightly/unifier-log-2026-09-18`)
  - **#3102** fix(a11y): convert RevealGrid settings groups to radiogroup (D3) (`nightly/unify-d3-settings-labels-2026-09-18`)
  - **#3101** fix(a11y): use ScaledEmptyState in Flashcards library (D1) (`nightly/unify-d1-empty-states-2026-09-18`)
- Comments processed: **2 — 0 fixed, 0 explained, 2 no action.** Two of four PRs (#3102, #3103) returned `totalCount: 0` inline review threads and zero PR-level reviews. The two threads that existed were both already **resolved**, each with the author's fix commit and a reply beneath it: #3101's `ScaledEmptyState` no-match-branch catch (fixed in `7fb4410`) and #3104's missing-regression-test finding (fixed in `41f5248`). Nothing was left unaddressed for this run to act on. **Sixth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect. The one genuine defect found during independent review (#3104's vacuous negative assertions, below) is a test-only change on a PR still open for its author, so it was raised in the review rather than pushed.
- Reviews posted: **4** (one structured review per open PR). Findings that came from verifying claims against the codebase rather than restating the diff:
  - **#3104 — read `HttpsError`'s actual constructor instead of trusting the assertion's wording, and the two negative tests turn out to test nothing.** The production fix is correct on every property that matters: it sits inside the pre-existing `if (!isAdmin)` guard and the `if (specPermDoc.exists)` branch (so admins are unaffected and a missing doc still defaults public), it runs *before* both the per-feature limit check and the usage-counter `transaction.set` calls (so a rejected request doesn't burn quota, and throwing inside `db.runTransaction` aborts the overall counter too), and its `enabled → admin → beta/betaUsers` ordering mirrors the global `gemini-functions` block including the `email.toLowerCase()` normalization. The three *positive* tests assert on the thrown message, which is unique per branch, so they genuinely pin the new behavior. But both *negative* tests (`index.test.ts:3549`, `:3562`) close with `expect(err?.name).not.toBe('permission-denied')` — and `HttpsError extends Error` assigns `this.code`, `this.details` and `this.httpErrorCode` in its constructor while **never touching `name`**, so `err.name` is `'Error'` for every error the handler can throw. The assertion is vacuously true: both tests would still pass if the fix wrongly rejected a whitelisted beta user, or wrongly rejected a feature with no permission doc at all. Paired with `toBeInstanceOf(Error)` — satisfied by the mocked Gemini rejection these tests rely on reaching — neither assertion can currently fail. The fix is `expect((err as HttpsError)?.code).not.toBe('permission-denied')`. Inherited, not invented here: the `generateVideoActivity` precedent at `:3362` that this block was modelled on carries the identical flaw, so both want correcting in one pass; the two genuinely-passing assertions in the same file (`:1760`, `:1858`) compare `jsonCall?.error`, which *is* the code. Also flagged the deploy-time flip side — any `global_permissions` doc sitting at `accessLevel: 'admin'` whose feature non-admins were silently reaching via `generateWithAI` starts returning `permission-denied`, which is the intended outcome but worth a pass over the live docs so it doesn't surprise a teacher mid-class.
  - **#3102 — checked the one property this retrofit pattern can actually get wrong, for all three groups.** The failure mode for roving tabindex is a group whose current value falls outside its option list, leaving every button at `tabIndex={-1}` and the group unreachable by Tab. None of the three has it, and each for a different reason: Columns resolves via `const columns = config.columns ?? 3` (L35) into `[2,3,4,5]`; Game Mode is an exhaustive boolean whose `!config.isMemoryMode` / `config.isMemoryMode` pair partitions every state including `undefined`; Reveal Mode compares `(config.revealMode ?? 'flip')` against a closed two-member union. So exactly one `tabIndex={0}` in every reachable config state. `as const` on each option array also lets `handleRadioGroupKeyDown<O>` infer properly narrowed setter params at all three call sites. Test coverage judged **adequate rather than a gap** — the keyboard behavior lives in the shared helper, which `components/common/radioGroupKeyNav.test.tsx` covers centrally, and the regenerated `legacySnapshots.test.tsx` `reveal-grid` entry pins this file's ARIA wiring with a diff confined to the new `role`/`aria-checked`/`tabindex` attributes, no class or text drift. Noted selection-follows-focus write amplification (arrowing Columns 2 → 5 fires three `updateWidget` calls and persists two throwaway configs) as **inherent to the helper across all 23 call sites, not introduced here** — Columns is just the widest group yet at 4 options.
  - **#3101 — read `ScaledEmptyState`'s own JSX and found two visual deltas past the one the description discloses.** The post-review fix is real: both the true-empty and filtered-no-match branches now route through one component with a ternary title/subtitle, restoring the shared treatment the first push had split. The description honestly calls out the uppercase/`tracking-widest` delta, but the shared component also (a) renders its title as `<p className="font-black uppercase tracking-widest">` rather than the `<h3>` this file had — so a PR tagged `fix(a11y)` actually removes a heading from the accessibility tree, consistent with every prior conversion in the series but a canon-level question rather than a this-PR one — and (b) changes the type scale and drops a padding cap: title `min(18px, 5.5cqmin)` → `min(14px, 4cqmin)`, subtitle `min(12px, 3.8cqmin)` → `min(12px, 3cqmin)`, padding `min(36px, 9cqmin)` → an uncapped `4cqmin` that grows past the old 36px ceiling at large widget sizes. "Icon, colors, and copy are unchanged" is accurate as written but doesn't cover type scale. **Test gap worth noting:** `FlashcardLibrary.test.tsx` asserts `getByText('Nothing assigned yet')` (L123) and nothing at all for the set-library grid's `emptyState` slot — neither branch of the ternary. Since the no-match branch is precisely what regressed on the first push and was caught only by review, a two-line assertion there would make the fix self-guarding.
  - **#3103 — read the journal against the PRs it describes rather than accepting its self-report.** The D3 backlog row correctly removes `RevealGrid` and retains `MaterialsWidget` plus the three admin panels, matching #3102's actual scope (all three groups in the one file); the D1 row's account of the review catch matches #3101's real history (the initial push *did* split the no-match branch into a hardcoded `text-sm` fallback, and `7fb4410` *does* route both back through one `ScaledEmptyState`); run count 89 → 90 and the `_Last run_` date are consistent with the two dated Run Log rows added. One inconsistency worth a glance: the new run-90 D3 backlog row is marked "Not re-verified live this run" while the D1 row and the Notes section both describe a repo-wide sweep — an accurate self-report of differing rigor per dimension rather than a contradiction, but the row's own suggestion (a fresh grep re-pass next D3 run) is worth honoring so the admin-panel candidates don't drift silently.
- Cross-PR checks run this review:
  - **No merge conflicts between the open PRs.** File sets are disjoint: #3101 `components/widgets/Flashcards/FlashcardLibrary.tsx`; #3102 `components/widgets/RevealGrid/Settings.tsx` + its legacy snapshot; #3103 `docs/routines/unifier.md`; #3104 `functions/src/aiGeneration.ts` + `functions/src/index.test.ts` + `docs/scheduled-tasks/*`. Merge order is unconstrained.
  - Base freshness: all four head branches sit on `dev-paul`'s head `5e59ee0`. `dev-paul` did not move during the run.
  - CI: all four PRs green across all 14 checks on their current heads, including Claude Code Review, at the time of review.
  - **Only one of four PRs carried a production code change** (#3104, +28 in `functions/src/aiGeneration.ts`); the other three are a settings-panel ARIA retrofit, a presentational component swap, and a docs log. Every code-touching PR shipped tests with it — the coverage gaps found this run are in the *quality* of those tests (#3104's vacuous negative assertions, #3101's untested ternary), not their absence.

## 2026-09-17

- PRs reviewed: **8** (every open PR). All eight target `dev-paul`, all eight are drafts, and every head branch was a `nightly/*` or `scheduled-tasks` branch — none was `main`- or `dev-*`-headed, so all were writable.
  - **#3091** fix(security): require verified email before ClassLink pseudonym disclosure (`nightly/build-tooling-2026-09-17`)
  - **#3090** fix(admin): canonicalize building id in `NoteConfigurationPanel` (`nightly/admin-config-2026-09-17`)
  - **#3089** fix(dashboard): stable-stringify `updateWidget`'s config version-bump check (`nightly/state-data-2026-09-17`)
  - **#3088** fix(board-nav): keep orphaned collections reachable in nav menus (`nightly/dashboard-layout-2026-09-17`)
  - **#3087** fix(quiz): dedupe live scoreboard denominator on `arrayUnion` race (`nightly/widgets-2026-09-17`)
  - **#3086** Thursday scheduled tasks — journal audits + `AssignTargetingSection` cqmin fix (`scheduled-tasks`)
  - **#3085** docs(unifier): log run 89 — 1 shipped (D3), 4 aligned (`nightly/unifier-log-2026-09-17`)
  - **#3084** fix(a11y): retrofit HotspotImage Popover Theme picker to radiogroup + roving tabindex (D3) (`nightly/unify-d3-settings-labels-2026-09-17`)
- Comments processed: **9 — 0 fixed, 1 explained, 8 no action.** Seven of eight PRs returned `totalCount: 0` inline review threads; #3088 had one, already **resolved** with the author's fix (`93be75a`) and reply beneath it. Of the eight PR-level bot reviews, six concluded "no issues found." The two that carried real findings had both already been acted on before this run read them: #3088's deeper `effectiveParentId` bug (fixed in `4b409c6`, then re-reviewed clean by a second `claude[bot]` pass), and #3086's "the description says docs-only but the diff has source changes" — **verified stale**: the PR body now carries an accurate title and a dedicated "Code fix — css-scaling MEDIUM" section, so the wording that comment reacted to is gone. Explained that inline in #3086's review rather than posting a separate comment. **Fifth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect, and nothing found during independent review was both a confident mechanical fix and in scope for the PR that surfaced it. The one genuinely exploitable bug found this run (see #3091 below) lives in a file no open PR touches — pushing it into #3091 would have buried a one-line security change under an unrelated diff, so it was raised in the review with a recommendation to open its own PR.
- Reviews posted: **8** (one structured review per open PR). Findings that came from verifying claims against the codebase rather than restating the diff:
  - **#3091 — followed the "out of scope, future PR" note to the file it names and confirmed the hole is real and open today.** The PR correctly gates `getPseudonymsForAssignmentV1` on `email_verified !== true`, and the gate is placed above the `data` destructure so it covers **both** consumers of `teacherEmail` — the ClassLink `filter='email=...'` re-verification (~L825–832) *and* the `organizations/{orgId}/members/{teacherEmailLower}` lookup in the test-class branch (~L732–734). A narrower gate in front of only the ClassLink call, which the description says was considered, would have left the test-class path fully exploitable. But `functions/src/classlinkRoster.ts`'s `getClassLinkRosterV1` reads `request.auth.token.email` (L44), checks only that it is non-empty (L45–50), and feeds it into the *same* OneRoster email filter (L69–70) with **no `email_verified` check anywhere in the handler** — same attack, and the payload is `studentsByClass`, which is less redacted than the pseudonyms this PR just protected. `isSafeEmailForOneRosterFilter` guards injection, not identity. Flagged as merge-this-then-immediately-open-the-counterpart rather than as a backlog item: this rail is only as strong as the weakest function reading the same unverified claim.
  - **#3086 — traced where `cqScaled` stops and named the visual consequence the diff can't show.** The opt-in prop design is right and avoids the obvious trap (the five non-CQ consumers render outside any `container-type` ancestor, where `cqmin` computes to `0`, so a blanket conversion breaks all of them). But `cqScaled` reaches only this file's own JSX — `CollapsibleSection`, `AssignStudentPicker` and `OverrideEditorRow`, all rendered inside the same modal, keep fixed Tailwind sizing. So as the MiniApp widget shrinks, this file's headings scale down (`min(14px, 5.5cqmin)` → ~11px at a 200px container) while the imported chrome stays put, and body text ends up rendering **larger than the headings above it** — hierarchy inversion, against CLAUDE.md's "type hierarchy is the primary tool for visual organization" and principle 3. The deferral is documented honestly, but the intermediate state is visibly worse than either endpoint at small sizes. Also: **the entire `cqScaled={true}` branch is untested** — the existing suite passes precisely *because* the default is `false`, so all ~15 ternaries could have their branches swapped with CI still green.
  - **#3088 — checked whether the reorder affordance still tells the truth for the rows the fix newly surfaces.** The `effectiveParentId` follow-up is the half that makes the fix real, and the added test asserts a rendered `menuitem` rather than array membership, which is the right level. But with `effectiveParentId = null` a surfaced orphan lands in the `null` bucket alongside real roots, so Alt+Arrow calls `onReorder(null, ids)` and persists an interleaved ordering — while `flattenCollections` builds output in two passes (walk real roots sorted by `order`, *then* append orphan roots sorted by `order`), so on re-render the orphan snaps back to the bottom regardless of what was just written. The keypress writes to Firestore and the row doesn't move. Minor, and orphans are a transient error state, but the affordance is currently dishonest. Confirmed the mutual-parent cycle gap is genuinely pre-existing and now non-hanging thanks to the new `visited` guard, and that `MoveBoardMenu.tsx` destructures `{ c, depth }` only — so it needed no change and correctly got none.
  - **#3090 — named the two write-side properties the three-line diff inherits from the shared helper.** The canonicalization is correct and matches `TrafficLightConfigurationPanel` exactly, and leaving `BUILDINGS.find((b) => b.id === selectedBuildingId)` on the *raw* id is right (that list is keyed in the same raw space `useAdminBuildings()` returns) — the part of this pattern most often gotten wrong. Two inherited properties worth knowing before it spreads further: `canonicalizeBuildingKeyedRecord` folds keys through `canonicalBuildingId` with **last-write-wins**, so a record holding both `high` and `orono-high-school` silently loses one rather than merging; and `handleUpdateBuilding` spreads the canonicalized record, so editing one building re-keys every other building's entry in that doc. Confirmed the helper's `Object.create(null)` result is spread into a plain literal before `onChange`, so nothing null-prototype reaches Firestore.
  - **#3087 — checked the numerator and denominator for the *next* version of the same asymmetry.** The dedup fix is correct and the `Map` lookup incidentally removes a `questions.find()` from inside a per-answer loop on a path that re-runs per response snapshot. But `getEarnedPoints` ends with `Math.round(totalPoints)` while the new `getMaxAnsweredPoints` returns the raw sum. Identical for every integer `points` value in use today — and if fractional or bonus-weighted points ever land, the numerator rounds and the denominator doesn't, reintroducing a smaller copy of exactly the asymmetry this PR closes.
  - **#3089 — bounded the direction of the behavior change rather than calling a one-liner obviously safe.** Confirmed `stableStringify` was already imported (L54) and used at ~16 sites in the same file including the `updateWidgets` bulk sibling, and re-ran the grep: no `JSON.stringify`-based config comparison remains. Since this touches `DashboardContext.tsx`, established the delta is one-directional — cases that differed *only* by key order now compare equal, so a version bump can be newly suppressed but never newly caused, and a real content change can't be missed. The one way it could surface as a regression rather than a fix: any consumer accidentally relying on the spurious bumps (e.g. a `version`-keyed listener forcing a re-render after a no-op resubmit) stops firing. Nothing in the diff suggests one exists.
  - **#3084 — separated "new write amplification" from "the helper has always worked this way."** The retrofit is mechanical and precedent-matching, and dropping the `theme.value as 'light' | 'dark' | 'glass'` assertion is a net reduction in unsafe casts. Noted that `handleRadioGroupKeyDown` implements selection-follows-focus, so arrowing Light → Glass fires an `updateWidget` for the intervening `dark` — correct WAI-ARIA behavior and identical across all ~15 consumers, flagged only so it isn't misread as introduced here. Also confirmed the retained `text-sm` is *correct*, not a scaling miss: this is back-face settings JSX, which CLAUDE.md explicitly exempts. **Test gap worth noting:** every prior run in this D3 series had `legacySnapshots.test.tsx` as an incidental backstop; this file isn't in that suite's coverage, so nothing in CI would catch a regression of this retrofit.
  - **#3085 — read the journal against the PR it describes.** The run-89 D3 SHIPPED row's account of #3084 (one `role="group"` converted, the "Interactive Pins" repeater correctly left alone as a non-exclusive-choice false positive) matches that PR's actual diff, reviewed in this same pass. Observed rather than found: the archive split at run 77 bought headroom that is being spent again, and the per-row prose has grown long enough that a fresh subagent's odds of finding an existing precedent keep dropping — the exact failure mode runs 88 and 89 each recorded in their own Notes. Run 89's takeaway (promote a repeatedly-named precedent to a numbered `D#-E#` row) is the right instinct and is worth acting on structurally rather than as another paragraph.
- Cross-PR checks run this review:
  - **No merge conflicts between the open PRs.** File sets are disjoint: #3084 `components/widgets/HotspotImage/`; #3085 `docs/routines/unifier.md`; #3086 `components/common/library/AssignTargetingSection.tsx` + `components/widgets/MiniApp/` + `docs/scheduled-tasks/*`; #3087 `components/widgets/QuizWidget/`; #3088 `components/layout/`; #3089 `context/DashboardContext.tsx`; #3090 `components/admin/NoteConfigurationPanel*`; #3091 `functions/src/`. Merge order is unconstrained.
  - Base freshness: all eight head branches sit on `dev-paul`'s head `54b1bd7`. `dev-paul` did not move during the run.
  - **Every diff this run was small** (largest non-docs: #3086 at +258/−67 across 7 files; five of eight under 90 lines changed), and every code-touching PR shipped tests with it except #3086 — noted above as this run's one real coverage gap.

## 2026-09-04

- PRs reviewed: **7** (every open PR). All seven target `dev-paul`, all seven are drafts, and every head branch was a `nightly/*` or `scheduled-tasks` branch — none was `main`- or `dev-*`-headed, so all were writable.
  - **#2836** docs(debugger): log run 60 (2026-09-04) — 5 bugs shipped (`nightly/debugger-log-2026-09-04`)
  - **#2835** fix(css-scaling): cap ConceptWeb node text at `min(24px, 15cqmin)` (`scheduled-tasks`)
  - **#2834** fix(rules): stop pending Activity Wall posts leaking via likes/comments (`nightly/build-tooling-2026-09-04`)
  - **#2833** fix(admin): canonicalize building id in `DiceConfigurationPanel` (`nightly/admin-config-2026-09-04`)
  - **#2832** fix(sync): advance the save baseline after a hidden-tab/teardown flush (`nightly/state-data-2026-09-04`)
  - **#2831** fix(a11y): roving tabindex + arrow-key nav for `SurfaceColorSettings` radiogroup (`nightly/dashboard-layout-2026-09-04`)
  - **#2830** fix(smartNotebook): make object-link saves atomic against the server doc (`nightly/widgets-2026-09-04`)
- Comments processed: **8 — 0 fixed, 0 replied to.** Five of seven PRs returned `totalCount: 0` inline review threads. #2831 had one thread; it was already **resolved**, with the author's fix (`22d7ee7`) and a self-correction posted beneath it. #2830's follow-up finding (`handleRemoveObjectLink` carrying the identical race) had likewise already been fixed on-branch and re-reviewed clean by a second `claude[bot]` pass. The remaining six comments were summary-level bot reviews concluding "no issues found." Nothing met the fix bar; nothing met the reply bar either, since every open item had already been answered on the PR by the time this run read it. **Fourth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect, and nothing found during independent review rose to a confident, in-scope, mechanical fix. All findings were raised in the reviews instead.
- Reviews posted: **7** (one structured review per open PR). Findings that came from verifying claims against the codebase rather than restating the diff:
  - **#2835 — worked the numbers behind the cap instead of accepting "matches the file's convention."** The one-line change caps the ConceptWeb node `<textarea>` at `min(24px, 15cqmin)` to match the sibling `min(16px, 4cqmin)` icons. But those siblings are chrome and this is the widget's **primary content**. Node boxes are 15%×15% of the widget with `containerType: size`, so `cqmin` resolves against the node's own short side: at the default 800×600 the cap is inert (≈13.5px wanted), at 1080p full-screen it binds by a hair (≈24.3px wanted), and on a 4K display or any org that raises `defaultNodeHeight` it **binds hard — ≈48.6px wanted, 24px delivered, half size on the element a concept map exists to be read from**. CLAUDE.md's own table puts primary content at 20–30cqmin with 20–32px as a *floor* and recommends `clamp()` for hero text precisely so it keeps scaling; `min(Xpx, Ycqmin)` is documented as a blur guard, not a legibility ceiling. Suggested `clamp(12px, 15cqmin, 48px)`. Ships no visible regression today, which is exactly why it would go unnoticed.
  - **#2834 — checked the client predicate against the rule it claims to mirror, and they disagree on one shape.** The rules fix is correct and correctly placed (create-time, because a `resource.data`-dependent read rule kills the whole unfiltered listen). But the rule defaults a *missing* `status` to approved (`get('status', 'approved')`) while `EngagementFooter`'s new `writable` requires `submission.status === 'approved'` strictly. `ActivityWallSubmission.status` is required in `types.ts`, so current write paths always set it — a legacy doc predating the field deserializes to `undefined` and the UI disables engagement the server would allow. Fails safe (over-restrictive, never permissive), so non-blocking, but `(submission.status ?? 'approved')` would make the two sides agree exactly. Also flagged the `exists(ref)` + `get(ref)` same-path pair against the per-request document-access budget on top of `awParentSession()`'s own `get` — cached, and the emulator enforces the limit so 1519/1519 green is real evidence, but that limit fails as a blanket permission-denied in production rather than as a test failure, which is worth naming.
  - **#2832 — proved the two baseline-capture shapes are equivalent rather than assuming the mirror was faithful.** The flush captures via `getDashboardSaveState(active)`, whose `widgets` field runs through `stripDerivedPixels`; the debounced handler it mirrors builds its own capture with raw `JSON.stringify(active.widgets)`. Traced every consumer of the parsed baseline (`utils/widgetMergeFields.ts`): the merge reads `config`/`version`, `LAYOUT_FIELDS` (`xProp`/`yProp`/`wProp`/`hProp`/…), `STYLE_FIELDS` and `annotation` — **never** the raw `x`/`y`/`w`/`h` that `stripDerivedPixels` drops. So the forms are equivalent, and the flush now matches the init (~L1990) and dashboard-switch (~L2728) sites: three of four agree and the debounced one is the outlier. Also verified the capture running *after* `saveDashboard(...)` is invoked is safe — `scrubDashboardPII` returns a new object and every mutation is behind an `await` — while noting that hoisting it would spare the next reader that proof.
  - **#2831 — checked the one thing that would silently break the extraction.** The shared `handleRadioGroupKeyDown` maps DOM node index → `options[nextIdx]`, so it is only correct if the `role="radiogroup"` container holds exactly the preset buttons. Confirmed in `SurfaceColorSettings` that the custom `<input type="color">` and the opacity range are siblings *outside* the radiogroup, inside the enclosing `role="group"` — alignment holds. Also confirmed the extracted function is byte-identical to the removed one, and that the `hasSelectedColorPreset` fallback is load-bearing rather than defensive: `cardColor` can legitimately hold a non-preset value from the picker below, which without the fallback leaves the entire group at `tabIndex={-1}`.
  - **#2830 — followed the sibling write path the fix deliberately left alone.** `handleSaveObjectLinksBatch` still uses a bare `arrayUnion`. That reads like a gap and isn't: a concurrent `arrayUnion` bumps the document version, so an in-flight transaction aborts and retries against the updated array rather than clobbering it. Recorded as reasoned-not-asserted, since no test interleaves the two. Also noted both handlers now rewrite the full `objectLinks` array rather than issuing a targeted array op — one extra read plus a larger write per operation, fine at hotspot frequency.
  - **#2833 — named the blast radius the diff doesn't show.** `handleUpdateBuilding` spreads the *canonicalized* `buildingDefaults` back into the saved config, so editing any one building rewrites every other legacy key in that org's record. Desired migration behavior and identical to the sibling panels, but it means a single edit's write surface is the whole map. Confirmed `BuildingSelector` still correctly receives the raw `selectedBuildingId` — the part of the 3-part pattern most often gotten wrong.
  - **#2836 — read the log against the branches it describes and found it disagreeing with itself.** The systemic-gap row's body was updated to "**29 files still unfixed**" and its candidate list trimmed to 29 entries, but the Status column still reads `30 files remain`. Separately, the radiogroup backlog row still says the helper is "duplicated verbatim … worth extracting to a shared `utils/` module" — that extraction already landed in #2831's `22d7ee7`, as `components/common/radioGroupKeyNav.ts`, *not* under `utils/`. This PR's head predates that commit so the drift is understandable, but this file is the input to future dispatch prompts: left as-is the next run gets dispatched to redo finished work, which is the exact stale-row failure mode this file's own Notes section keeps recording.
- Cross-PR checks run this review:
  - **CI is green on all six code-touching PRs** — seven checks each (Build, Code Quality, Unit, E2E, Firestore Rules, Claude Code Review, summary), all `success`, confirmed at each PR's exact head SHA. **#2836 (docs-only) has zero checks, and that is correct, not a gap:** `pr-validation.yml` carries `paths-ignore` for Markdown and `docs/**` so journal-only PRs don't burn the six-job suite. Worth restating each run because "no checks" and "checks failed" look alike on the PR list. Note the corollary: nothing mechanically validated #2836's Markdown table syntax, which is where both of its inconsistencies live.
  - **No merge conflicts between the open PRs.** File sets are disjoint: #2830 `components/widgets/SmartNotebook/`; #2831 `components/common/{SurfaceColorSettings,TypographySettings,radioGroupKeyNav}`; #2832 `context/DashboardContext.tsx`; #2833 `components/admin/DiceConfigurationPanel*`; #2834 `firestore.rules` + `components/activityWall/engagement/`; #2835 `components/widgets/ConceptWeb/` + `docs/scheduled-tasks/*`; #2836 `docs/routines/debugger.md`. Merge order is unconstrained.
  - Base freshness: all six `nightly/*` branches and `scheduled-tasks` sit on `dev-paul`'s head `425fb94`. `dev-paul` did not move during the run.
  - **Regression surface is narrow.** Only #2832 touches `context/DashboardContext.tsx`, and it is confined to the `flushPendingSave` closure inside the teardown-listener effect — nowhere near `getAdminBuildingConfig` or the config-merge pipeline. Nothing touches `WidgetRegistry.ts`, the `WidgetType`/`WidgetConfig` union, or `ConfigForWidget`, so those checks are vacuous this run. `functions/src/index.ts` is untouched.
  - Diff hygiene: grepped all seven diffs for `it.only`/`describe.only`/`.skip`, `console.log`, `@ts-ignore`/`@ts-expect-error`, ` as any`, and `eslint-disable` — clean across the board.
- Verification standard: no code was pushed, so no pre-push gate was needed. Verification went into independent re-derivation instead — 1 full trace of the baseline consumers through `utils/widgetMergeFields.ts` plus a non-mutation check on `saveDashboard` (#2832), 1 DOM-containment check of the radiogroup children (#2831), 1 arithmetic pass over node/container sizing against `config/widgetDefaults.ts` (#2835), 1 `types.ts` field-optionality check against the rule's default branch (#2834), 1 sibling-pattern comparison (#2833), and 1 read of the untouched batch write path (#2830).
- Notes:
  ...
  - **Log placement:** pushed to this session's designated branch `claude/pensive-bell-gnafpu`, branched fresh from `dev-paul` (which already carries the 09-03 entry — `origin/dev-paul` and `origin/scheduled-tasks` hold identical copies of this file, so no chaining was needed). Kept **off** `scheduled-tasks`, which heads open PR #2835 and carries a code change (`ConceptWeb/Widget.tsx`), not just journals — same reasoning as the 08-26 through 09-01 runs.
  - **Phase 1 was empty for the fourth run running, but for a new reason worth distinguishing.** On 09-01 there was genuinely nothing to act on. Tonight there *were* two real change requests — #2831's helper duplication and #2830's `handleRemoveObjectLink` sibling race — and both had already been fixed, pushed, replied to, and re-reviewed clean by the time this run read them, within roughly 30 minutes of being raised. Phase 1's premise is a backlog of unaddressed reviewer feedback; a fleet that closes its own review loop that fast doesn't produce one. The routine's value is now entirely in Phase 2, and specifically in the part that checks a claim against the codebase rather than reading the diff.
  - **A pattern across #2835 and #2834: both apply a correct rule at the wrong strictness, and both fail invisibly.** #2835 caps content at a chrome ceiling, which looks right at every size a developer tests and only costs on the projector the widget exists for. #2834's client predicate is stricter than the server rule it mirrors, which is safe today only because `types.ts` makes the field required. Neither is a bug; both are "matched the neighboring pattern without asking whether the neighbor is the same kind of thing." Worth a line in the sweep routines' checklists next to the existing half-conversion check.
  - **The append-contention recommendation still stands, unactioned for a seventh run.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed on 08-26, 08-28, 08-29, 08-30, 08-31 and 09-01; the 08-29 entry was itself lost to that mechanism and restored by #2647, and the 09-03 entry was lost to a squash-merge race and restored by #2797 — **two documented losses now, not one.** No contention tonight (single log-writing run, clean base), but the file is past 3,500 lines and the structural fix remains a small standalone PR a maintainer could take at any time.
  - Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/review operations used `mcp__github__*` equivalents, and diffs were read locally via `git diff origin/dev-paul...origin/<head>` after fetching, which is cheaper and more reliable than paging large API payloads. Local env runs Node 22 against a repo pinned to Node 24, so it was used only for reading, grepping and diffing — CI on Node 24 remains the authoritative gate.

## 2026-09-03

- PRs reviewed: **9** (every open PR). All nine target `dev-paul`, all nine are drafts, and every head branch was a `nightly/*` or `scheduled-tasks` branch — none was `main`- or `dev-*`-headed, so all were writable.
  - **#2793** fix(typography-settings): add roving-tabindex arrow-key nav to both radiogroups (`nightly/dashboard-layout-2026-09-03`)
  - **#2792** fix(dashboard-pii): abort save instead of silently scrubbing PII when Drive is disconnected (`nightly/state-data-2026-09-03`)
  - **#2791** fix(rules): type-check `recordingNoticeAckedAt` on quiz response updates (`nightly/build-tooling-2026-09-03`)
  - **#2790** fix(admin): canonicalize building id in `ChecklistConfigurationPanel` (`nightly/admin-config-2026-09-03`)
  - **#2789** fix(css-scaling): cap ActivityWall `WordCloudLayout` font-size cqmin (`scheduled-tasks`)
  - **#2788** docs(unifier): log run 75 — D1/D2/D3 shipped, D4/D5 aligned (`nightly/unifier-log-2026-09-03`)
  - **#2787** fix(admin): pair Activity Wall config labels with their selects (`nightly/unify-settings-labels-2026-09-03`)
  - **#2786** fix(activity-wall): route brand-red map pins through `--spart-accent` (`nightly/unify-brand-colors-2026-09-03`)
  - **#2785** fix(activity-wall): unify wall library empty state with `ScaledEmptyState` (`nightly/unify-empty-states-2026-09-03`)
- Comments processed: **1 unresolved — 1 fixed.** Eight of nine PRs returned `totalCount: 0` inline review threads. #2786's single thread was already resolved and answered by the author. The one live thread was #2792's, and it was a change request with a mechanical fix available — **breaking a four-run streak of empty Phase 1s.**
- Fixes pushed: **1.**
  - **#2792** / `nightly/state-data-2026-09-03` / `4dc9e754` — route the no-Drive PII abort through `authError()` so it fires the existing latched "Reconnect" toast instead of a bare `Error` that only ever surfaced as a generic "Failed to sync changes".
- Reviews posted: **9** (one structured review per open PR).

### The finding that mattered: #2793 removes 28 controls from the Tab order

The highest-value result this run came from checking a default value against an option list — not from reading the diff, which looks uniformly correct.

#2793 adds the WAI-ARIA roving-tabindex contract to both radiogroups in `TypographySettings.tsx`. The font-family group is fine. The **Text Color** group is not: `TypographySettings.tsx:59` defaults `fontColor` to `'#334155'`, and that hex **is not in `FONT_COLORS`** (`config/fonts.ts:56`, 28 entries, zero matches). So `tabIndex={fontColor === color ? 0 : -1}` assigns `-1` to *every* swatch whenever `fontColor` is unset — the default state of every widget — and the whole section drops out of the Tab order. Before the PR all 28 were plain `<button>`s with implicit `tabIndex=0`.

I confirmed this by writing a throwaway probe against the branch rather than reasoning about it:

```
config={{}}                       → presets: 28, tabbable: 0
config={{ fontColor: '#123456' }} → tabbable: 0
```

The second case is the one that makes it more than a fresh-widget edge: the `<input type="color">` directly below the swatches lets a teacher pick any custom color, which puts `fontColor` permanently outside the preset list. **An accessibility PR that makes the control less keyboard-accessible than before, for the users it was written for.** The font group escapes only because its default (`'global'`) happens to be a real `FONTS` entry — the pattern was applied to both groups uniformly without checking that the second one's default is representable in its own option list. Reported with the standard fallback fix (make index 0 tabbable when nothing matches); not pushed, since Phase 2 is review-only.

### Other findings from verification rather than diff-reading

- **#2789 — the cap doesn't just prevent blur, it deletes the visualization's only encoding channel.** `buildWordCloud` (`utils/activityWallWordCloud.ts:95-99`) sets `weight = count / maxCount`, so weight ∈ (0,1] with the top word always exactly 1.0, and font size spans a fixed 5× range (3–15cqmin). A word caps once `w > (4800/minDim − 3)/12`: at a 600px widget that flattens ~58% of the weight range, at 900px ~81%, at 1080px ~88%. On a projected board the most-frequent word becomes visually indistinguishable from a middling one. The `vmin` branch is left uncapped, so the *same wall* now renders with full dynamic range full-screen and a flattened one in the widget. Recommended `clamp(11px, Xcqmin, 96px)` — CLAUDE.md's own hero-text tier is `clamp(24px, 25cqmin, 120px)`; 48px is its *decorative icon* tier, and word-cloud terms are primary content.
- **#2785 — the empty/filtered split is a real bug fix hiding inside a cosmetic PR.** Traced `LibraryGrid.tsx:102`: it renders `emptyState` when `items.length === 0`, and `items={sorted}` — the *filtered* list. So the slot previously told a teacher whose search matched nothing that the library was empty. Gating on `entries.length === 0` (unfiltered) is the correct discriminator. Separately: `ScaledEmptyState` is documented as container-query-scaled "for widgets", but here it renders inside a `Modal` that portals to `document.body`, so with no `container-type: size` ancestor its three **uncapped** `cqmin` values (`gap: 2cqmin`, `padding: 4cqmin`, `gap: 0.5cqmin`) resolve against the viewport — ~43px padding at 1080p. Text and icon clamp to their px caps and look fine; the spacing is silently viewport-coupled.
- **#2792 — the caller fragility flagged on #2710 two runs ago is now easier to reach, and still unaddressed.** `saveDashboards` could previously only reject from Firestore; it can now reject from an absent Google token, without any Drive call failing. `reorderDashboards` catches into a toast (handled, and pinned by the PR's test). `updateWidgetConfigsAcrossBoards` still doesn't catch, and neither does its consumer in `MaterialsWidget/Settings.tsx`. **This is the second PR in three runs to widen the reachability of that same uncaught path.**
- **#2791 — ruled out the failure mode that would have made this a bad tightening.** A rules change that rejects a legitimate client write would break the notice gate for every student. `acknowledgeRecordingNotice` has exactly one call site (`QuizStudentApp.tsx:672`) and it passes `Date.now()`, always integral, so `is int` can never falsely reject. Also confirmed the field appears in `firestore.rules` exactly once — no second unconstrained CREATE path. The residual is that the rule constrains *type* but not *value*: a student can still write any integer, and for a field described as durable Tennessen-notice proof, a client-controlled timestamp is weak evidence. The siblings (`handRaisedAt == request.time`, server-stamped `lastWriteAt`) show the stronger pattern; closing it is a `number` → `Timestamp` migration, correctly out of scope.
- **#2787 — checked for the half-conversion pattern and it largely isn't one.** All three 1:1 label→control pairs in the file are now associated, and the four checkbox `<label>`s wrap their inputs (valid implicit association). The convention choice is right for a non-obvious reason: the file's *own* sibling (`aw-default-max-posts`, `:138`) already used static-string ids, so this was drift within one file, and matching the neighbour beats importing `useId()` from elsewhere. The one remaining orphan (`:71`, heading `BuildingSelector`) is already filed as D3-E28.
- **#2790 — the fix is a no-op for orgs already on canonical ids**, since `canonicalBuildingId` returns its input unchanged when no alias exists. That's the right failure mode. Same silent whole-document migration on first edit as #2711's Countdown fix — desirable, but the write is broader than the edit.
- **#2786 — re-derived the reachability matrix independently** and got the same answer as the author's thread reply: real theming on the two teacher-side surfaces (`Widget.tsx:460` inside `#dashboard-root`; `SectionsEditor`'s `MapPinPicker` via the `document.documentElement` copy of the vars), exact no-op on the two provider-free public routes. Noted that `DashboardView.tsx:1330-1334`'s decision to set the vars on `documentElement` *as well as* `#dashboard-root` is load-bearing for the portaled editor case — a future "simplification" to `#dashboard-root` only would silently break it.
- **#2788 — a journal that records a reviewer being right about the conclusion and wrong about one mechanism.** Its D2 row states the #2786 review's `ActivityWallStudentApp → LayoutRouter → MapLayout` claim was checked and found incorrect while the underlying concern held; I re-derived the same matrix and agree. D5-E31's verdict also required reading the component rather than pattern-matching — `ShareModal.tsx` calls `addToast()` for the same failure it renders inline, so it isn't a toast-avoidance case at all despite looking like one to a grep for local `error` state.

### Cross-PR checks

- **CI green on all eight code-touching PRs** — 7/7 checks each (Build, Code Quality, Unit, E2E, Firestore Rules, Claude Code Review, summary), all `success`, confirmed at each PR's head SHA. **#2788 is docs-only and has zero checks, which is correct, not a gap:** `pr-validation.yml` carries `paths-ignore: ['**/*.md', 'docs/**']`. #2792 will re-run against `4dc9e754`.
- **No merge conflicts between the open PRs.** File sets are disjoint: #2793 `components/common/TypographySettings.tsx`; #2792 `context/DashboardContext*`; #2791 `firestore.rules` + its rules test; #2790 `components/admin/ChecklistConfigurationPanel*`; #2789 `WordCloudLayout.tsx` + `docs/scheduled-tasks/*`; #2787 `ActivityWallConfigurationPanel.tsx`; #2786 `MapLayout.tsx` + `MapPinPicker.tsx`; #2785 `WallLibraryModal.tsx`; #2788 `docs/routines/unifier.md`. Merge order is unconstrained. Worth noting four PRs (#2789, #2787, #2786, #2785) all touch the new ActivityWall tree but no two touch the same file.
- Base freshness: all nine head branches sit on `dev-paul`'s head `be23746`.
- **Regression surface is narrow.** Only #2792 touches `context/DashboardContext.tsx`, and it goes nowhere near `getAdminBuildingConfig` or the config-merge pipeline. Nothing touches `WidgetRegistry.ts`, the `WidgetType`/`WidgetConfig` union, `ConfigForWidget`, or `functions/src/index.ts` — those checks are vacuous this run.

### Verification standard

- For the one pushed fix: `pnpm run type-check` ✓, `pnpm run lint` (`--max-warnings 0`, root + functions) ✓, `prettier --check` on both changed files ✓, and `pnpm exec vitest run context/` → **25 files / 186 tests pass**. I also temporarily reverted the fix and re-ran to confirm the new assertion **fails** against the plain-`Error` version (`reconnectToast?.action?.label` → `undefined`) before restoring it — a test that passes both with and without the change pins nothing.
- The #2792 test needed editing because it incidentally depended on toast **ordering** (`.find(t => t.type === 'error')` now returns the reconnect toast first). Rewritten to assert both toasts by content, which is strictly stronger than what it asserted before.
- For #2793, wrote and ran a throwaway probe (deleted, not committed) rather than asserting the tabindex claim from reading.
- Independent re-derivation elsewhere: 1 weight-range + cap-threshold derivation from `buildWordCloud` source (#2789), 1 `LibraryGrid`/`items`/`sorted` trace (#2785), 1 single-call-site trace from `acknowledgeRecordingNotice` to `Date.now()` plus a repo-wide `recordingNoticeAckedAt` grep (#2791), 1 helper-existence + five-sibling comparison (#2790), 1 four-route `--spart-accent` reachability matrix (#2786), 1 full label/control audit of `ActivityWallConfigurationPanel` (#2787).

### Outcomes (recorded post-run, same session)

**All nine PRs reviewed in this run merged into `dev-paul` within ~4.5 hours**, and the two findings this run rated most serious were both acted on before their PR merged. Recording this because a review routine's only real measure is whether its findings change the code, and prior entries in this log have never been able to close that loop.

- **#2793 — the headline regression was fixed before merge.** The merged `TypographySettings.tsx` (`dev-paul` @ `3a05298c`) carries a `hasSelectedColorPreset` guard: `tabIndex={fontColor === color || (!hasSelectedColorPreset && color === TEXT_COLOR_PRESETS[0]) ? 0 : -1}` — exactly the WAI-ARIA "first option tabbable when nothing is selected" fallback the review proposed. **Both** cases are now pinned by tests, including the non-obvious one: `tests/components/common/TypographySettings.test.tsx:263` ("keeps exactly one text color swatch tabbable when fontColor is unset") and `:277` (the same for a custom non-preset color — the state the `input[type=color]` control puts every customized widget into permanently). Each asserts `toHaveLength(1)` **and** that the tabbable one is `swatches[0]`, which is stronger than "at least one is reachable."
- **#2791 — the residual value-forgeability note was adopted** (merged as `9e8f4e4f`). Commit `28f32c80` took the review's cheap-hardening suggestion verbatim: `is int && > 0`, with a test covering both `0` and `-1`. The rule comment was also rewritten to state *why* the field can't be pinned to `request.time`, rather than claiming it "has no value constraint to enforce" — the framing the review pushed back on. The full `serverTimestamp()` migration correctly remains a follow-up.
- **Three of the four "missing tests" gaps were closed in the merged versions** — `components/widgets/ActivityWall/WallLibraryModal.test.tsx`, `components/activityWall/render/layouts.test.tsx`, and `tests/components/admin/ActivityWallConfigurationPanel.labels.test.tsx` are all new files on `dev-paul` that did not exist when the reviews were written, covering #2785's empty-vs-filtered split, #2789's word-cloud sizing, and #2787's label associations respectively.
- **#2792 merged** (`5842be90`) carrying this run's Phase 1 fix, `4dc9e754`.

**What this says about where the routine's value sits.** The 09-01 entry concluded Phase 2 was carrying the routine, and that the useful part of Phase 2 was checking claims against the codebase rather than reading diffs. Tonight supports that and sharpens it: the finding that changed the most code came from a *two-line grep* — checking whether a default value (`'#334155'`) appears in the list it is compared against (`FONT_COLORS`) — and then from spending five minutes on a throwaway probe to confirm it before asserting it. Neither step required understanding the feature. **The reusable technique is "verify the precondition the diff assumes," and it is cheap enough to apply to every diff.**

**Process note — this section was nearly lost, and the mechanism is worth knowing.** It was pushed to PR #2795 as a second commit minutes before that PR was squash-merged. GitHub squashed the head as of merge initiation, so the merge captured only the first commit and this section never reached `dev-paul`; it was restored afterwards as a separate PR. **A late push to a PR that is about to be squash-merged can be silently dropped** — the branch keeps the commit, `dev-paul` does not, and nothing reports a conflict. Worth verifying content actually landed after any merge that races a push, rather than trusting the merged/closed signal.

### Notes

- **Environment gap worth recording for future runs:** the session-start `install:all` ran before the PR branch was checked out, leaving `leaflet` / `react-leaflet` / `@types/leaflet` uninstalled even though they are in `package.json` and `pnpm-lock.yaml` (added by #2774). The first `type-check` produced five errors in `MapLayout.tsx` / `MapPinPicker.tsx` that looked like code defects and were purely environmental. `pnpm install --frozen-lockfile` after checkout resolved them. **A type-check failure in files a diff doesn't touch is an install-state question before it is a code question.**
- Branch safety: **no push to `main`, and no push to any `dev-*` branch.** The single code push went to `nightly/state-data-2026-09-03`, a PR head branch, as Phase 1 directs.
- **Log placement:** pushed to this session's designated branch `claude/pensive-bell-kjrbpo`, branched fresh from `dev-paul` (which already carries the 09-02 entry, so no chaining off a prior log branch was needed). Kept **off** `scheduled-tasks`, which heads open PR #2789 and carries a code change (`WordCloudLayout.tsx`), not just journals — same reasoning as the 08-26, 08-28, 08-29, 08-30, 08-31 and 09-01 runs.
- **Phase 1 was non-empty for the first time in five runs, and the thread it found was worth acting on.** The four preceding runs concluded that this routine's value had migrated almost entirely into Phase 2. Tonight's thread was a substantive change request with a mechanical fix available — the codebase already had the exact machinery (`authError()`, documented as "use at every Drive/Sheets throw site") the diff had bypassed. Worth noting the shape: the fix wasn't inventing a recovery path, it was **connecting a new throw site to a recovery path that already existed**, which is the most common form a genuinely automatable fix takes.
- **A pattern across #2793 and #2789: both apply a correct general rule to a case whose specifics defeat it.** #2793 applies the roving-tabindex contract to a group whose default value isn't in its own option list. #2789 applies CLAUDE.md's px-cap rule to the one element class where size *is* the data. Neither is a careless diff; both are what happens when a rule is applied without asking what the rule was protecting against. That's a different failure mode from the "half-conversion" pattern logged on 09-01, and probably deserves its own line in the sweep routines' checklists: **before applying a sizing or state rule, ask what this specific element encodes.**
- **The append-contention recommendation still stands, unactioned for a seventh run.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed on 08-26, 08-28, 08-29, 08-30, 08-31 and 09-01; the 08-29 entry was itself lost to that mechanism and restored by #2647. No contention tonight (single log-writing run, clean base), but the file is now ~3,440 lines and the structural fix remains a small standalone PR a maintainer could take at any time.
- Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/diff/review/reply operations used `mcp__github__*` equivalents, and diffs were read locally via `git diff origin/dev-paul...origin/<head>` after fetching.
- Verification env runs Node 22 (repo pins 24, "Unsupported engine" warning throughout); CI on Node 24 remains the authoritative gate on the one pushed commit.
...
- Cross-PR checks run this review:
  - **CI is green on all six code-touching PRs** — seven checks each (Build, Code Quality, Unit, E2E, Firestore Rules, Claude Code Review, summary), all `success`, confirmed at each PR's exact head SHA (#2710 and #2707 via check-runs; #2712/#2711/#2709/#2708 via the `pr-validation.yml` run list). **The two docs-only PRs (#2713, #2706) have zero checks, and that is correct, not a gap:** `pr-validation.yml` carries `paths-ignore: ['**/*.md', 'docs/**']`, added deliberately so the nightly fleet's journal-only PRs don't burn the full six-job suite on Markdown. Worth stating plainly because "no checks" and "checks failed" look similar at a glance on the PR list, and the nightly log PRs will always be in the former state. Since no code was pushed this run, CI on Node 24 remains the authoritative gate; the local env here runs Node 22 (repo pins 24) and was used only for reading and grepping.
...
- PRs reviewed: **6** (all open PRs; every one targets `dev-paul`, none heads `main` or a `dev-*` branch, so all were eligible for push under the branch-safety rule).
  - **#2872** fix(css-scaling): cap ExpectationsWidget category-card icon containers (`scheduled-tasks`)
  - **#2871** fix(a11y): add roving tabindex + arrow-key nav to TimeTool radiogroups (`nightly/unify-settings-labels-2026-09-05`)
  - **#2870** fix(tests): use `@/` alias for cross-directory `functions/` import (`nightly/unify-import-paths-2026-09-05`)
  - **#2849** docs(unifier): log runs 76-77, archive old Run Log to `unifier-archive-2026.md` (`nightly/unifier-log-2026-09-04`)
  - **#2848** fix(tests): unify `tests/testHelpers` imports to the `@/` alias (`nightly/unify-import-paths-2026-09-04`)
  - **#2847** fix(a11y): associate the "Related widgets" checkbox group with its label (`nightly/unify-settings-labels-2026-09-04`)
- Comments processed: **5 total — 1 inline review thread + 4 review-level summary comments; 0 fixed, 0 replied to.** (The two counts are different denominators: GitHub inline threads anchored to a line, vs. PR-level review bodies with no anchor. Both are counted here because both can carry a finding.) The one inline review thread across all six PRs was on #2871 (`claude[bot]` on `TimeTool/Settings.tsx:47`, the `radioGroupKeyNav` falsy-option bug) and it was **already fixed in `e85c5a4` and marked resolved** before this run started. The other four pieces of feedback were summary-level `claude[bot]` comments on #2872/#2870/#2848/#2847, each concluding "LGTM / no issues found" with no change requested. Nothing met the fix bar; nothing met the reply bar either, since replying "no fix needed" under a comment that already says nothing needs fixing adds a notification without adding information. **Fourth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect, and the one finding this run produced independently (#2872, below) is a sizing judgement with two defensible fixes — it fails the "obvious correct fix" bar and was raised in review rather than pushed.
- Reviews posted: **6** (one structured review per open PR). Findings that came from checking a claim against the code rather than restating the diff:
  - **#2872 — the three containers the diff treats identically are not equivalent, and one of them now under-fits its own contents.** Read all three call sites rather than trusting the journal's "all three icons are capped at `min(48px, 11cqmin)`" claim: **that holds only for the Volume card.** Group Size (`:560-561`) and Interaction (`:629-630`) size their icons at `width/height: '80%'` — percentage, so they always fill the box and the 72px cap is straightforwardly safe there. Volume (`:472-473`) is the only genuinely-capped icon *and* the only container stacking extra content in a `flexDirection: 'column'`: icon + `gap` + a "Level"/number block. Once every cap engages that column needs `48 + 2 + 8 + 14 = 72px` of content plus `2 × 8px` padding = **88px**, inside a box now ceilinged at **72px**, under `overflow-hidden`. Stated the regression boundary precisely rather than overclaiming: the column is over-subscribed at *every* size (inner `14cqmin` vs. children `17.5cqmin`), so the squeeze is pre-existing — what the cap removes is the one regime where it self-resolved, above a widget smaller-dimension of ~490px where the uncapped `18cqmin` container had grown past 88px. At the 320×350 default (`widgetDefaults.ts:167-171`) nothing changes; the affected case is an enlarged/projected widget. Suggested `min(88px, 18cqmin)` for that one container as the smaller of the two fixes.
  - **#2871 — checked the two failure modes the loosened bounds check could have introduced, and the one thing that can silently desync.** The helper fix (`if (!nextOption)` → `if (nextIdx < 0 || nextIdx >= options.length)`) is correct in both divergence directions: `nextIdx` is always computed modulo `nodes.length` so `nodes[nextIdx]` can never be `undefined`, and an `options` array shorter than `nodes` now returns early instead of leaking `undefined` into `onSelect`. Separately verified the two **hand-built** option arrays against DOM order, since the other four groups `.map()` over the array they pass and are aligned by construction: `VOICE_LEVEL_OPTIONS` (6) vs. the "None" button + `[0,1,2,3,4].map(...)` = 6 radios, and `TRAFFIC_COLOR_OPTIONS` (4) vs. None/Red/Yellow/Green = 4 radios, both in matching order, with no stray `role="radio"` in either container to shift the `querySelectorAll` walk.
  - **#2849 — verified the archive split by row count, not by reading the commit message.** This file's own run-58 entry records that a careful patch of `unifier.md` can silently misplace content, so the move was checked mechanically: base (`bd8e9ce7`) **382** Run Log rows; head **150** live + **242** archived = **392**; the run-76 and run-77 log commits add **10** (5 per run); `382 + 10 = 392` ✓. Then set-compared actual row content — **0 rows in both files** (no duplication), **0 base rows absent from live ∪ archive** (no loss). Noted a two-day discrepancy: the archive title says "through 2026-07-31" but its rows run through **2026-08-02**.
  - **#2870 — named the thing that looks like a problem and isn't.** Root `tsconfig.json` carries `"exclude": [..., "functions", ...]`, which reads like it should break `@/functions/src/shared`. It doesn't: `exclude` filters only the program's root file set, not files pulled in as dependencies of included files — and the relative import already crossed that boundary, so the alias changes nothing about what's in the program. Also confirmed the target is `functions/src/shared.ts`, a **file**, so there's no directory/index-resolution ambiguity under `moduleResolution: bundler`.
  - **#2847 — checked the surrounding surface an a11y PR would be expected to have swept.** `useId()` is called unconditionally (no conditional-hook hazard), `role="group"` is right for a multi-select set where `radiogroup` would be wrong, and the raw `<span>` rather than `SettingsLabel as="span"` is correct *here* — `SettingsLabel`'s widget-settings typography wouldn't match this admin form's `text-sm font-medium text-slate-700`. The adjacent search input already carries `aria-label="Search widgets"`; the decorative `<Search />` icon at `:296` still lacks `aria-hidden`, which the project's own accessibility baseline calls for.
  - **#2848 — trivial and confirmed green**, no finding beyond the test run below.
- Cross-PR checks run this review:
  - **Tests run locally rather than trusted from PR descriptions**, since three of the six make behavioral or resolution-affecting changes. #2871: `vitest run components/common` → **64 files / 415 tests passing**, which is the check that matters because it edits a *shared* helper — `TypographySettings.tsx` and `ColorPresetPicker.tsx` are the other consumers and neither passes falsy options, so neither depended on the old truthiness short-circuit; plus the new helper test and TimeTool suites → **4 files / 80 passing**. #2870: **21/21**. #2848: **159/159**. Local env is Node 22 against a repo pinning 24, so CI remains the authoritative gate.
  - **#2871's new test file does not trip the count guard.** Checked `scripts/checkTestCounts.mjs` directly rather than assuming: the aggregate check is a *floor* and growth above baseline is always allowed, so `scripts/test-count-baseline.json` needs no edit. `@testing-library/jest-dom` is imported in `tests/setup.ts`, so the new file's `toHaveFocus()` resolves.
  - **Regression surface is narrow.** Nothing touches `context/DashboardContext.tsx`, `WidgetRegistry.ts`, the `WidgetType`/`WidgetConfig` union, `ConfigForWidget`, or `functions/src/index.ts` — those checks are vacuous this run. #2871 is the only shared-code change (`components/common/radioGroupKeyNav.ts`), and its blast radius was enumerated and tested above.
  - **No merge conflicts between the open PRs.** File sets are disjoint: #2872 `ExpectationsWidget/Widget.tsx` + `docs/scheduled-tasks/*`; #2871 `radioGroupKeyNav.*` + `TimeTool/Settings.tsx`; #2870 and #2848 different test files; #2849 `docs/routines/*`; #2847 `HelpCenter/HelpItemForm.tsx`. Merge order is unconstrained. Note #2849/#2848/#2847 sit on the older base `bd8e9ce7` while #2872/#2871/#2870 sit on `e610884f` — no conflict, since nothing between them touches a shared file.
  - **Docs-only PRs (#2849) correctly show zero checks, which is not a gap:** `pr-validation.yml` carries `paths-ignore: ['**/*.md', 'docs/**']`. Restating this because "no checks" and "checks failed" look alike at a glance on the PR list. It is also exactly why #2849's split was verified by row arithmetic here — nothing else was ever going to check it.
- Verification standard: no code was pushed, so no pre-push gate was needed. Verification went into independent re-derivation instead — 3 local test runs (595 tests total), 1 full read of all three ExpectationsWidget container call sites plus the already-capped sibling pair at `:289-300` for the arithmetic (#2872), 1 option-array/DOM-order audit across both hand-built arrays (#2871), 1 row-level set comparison across three git revisions of `unifier.md` (#2849), and 1 module-resolution trace through `tsconfig.json`/`vite.config.ts` to the file on disk (#2870).
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** No push to any PR head branch either, since no fix was warranted.
  - **Log placement:** pushed to this session's designated branch `claude/pensive-bell-ahjk4f`, branched fresh from `dev-paul` (whose newest entry is 2026-09-01, so no chaining off a prior log branch was needed). Kept **off** `scheduled-tasks` — it heads open PR #2872 and carries a code change (`ExpectationsWidget/Widget.tsx`), not just journals. Same reasoning as the 08-26 through 09-01 runs, and it also keeps this run's log out of a PR this same run reviewed.
  - **A pattern worth naming across #2872 and #2871: both are "apply the same fix to N call sites" changes where the sites turned out not to be equivalent.** #2872 capped three containers that differ in whether their icon is px-capped or percentage-sized, and in whether anything else shares the column — two are fine, one now under-fits. #2871 wired six radiogroups, four of which are aligned by construction and two of which depend on a hand-maintained array matching render order. In both, the risk sat entirely in the site that differed from its siblings, and in both it was invisible from the diff alone. This is the same shape as the 09-01 run's "half-conversions a grep would call done" note, one step earlier: not *what the sweep missed*, but *which site the sweep shouldn't have treated uniformly*.
  - **Phase 1 has now been effectively empty for four consecutive runs**, and this run more completely than 09-01: the single inline thread that existed had already been fixed and resolved before the run began. The routine's value continues to sit entirely in Phase 2, and specifically in the part that checks a claim against the codebase — every finding above came from reading whole call sites, doing the arithmetic, or comparing rows across revisions, none from reading the diff.
  - **The append-contention recommendation still stands, unactioned for a seventh run.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed on 08-26, 08-28, 08-29, 08-30, 08-31 and 09-01; the 08-29 entry was itself lost to that mechanism and restored by #2647. No contention tonight (single log-writing run, clean base), but the file is now **~600KB / 3,555 lines** — past this tooling's 256KB single-read limit, so every run that wants prior context must read it in chunks. Still a small standalone PR a maintainer could take at any time.
  - Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/diff/review/comment operations used `mcp__github__*` equivalents. Diffs and file contents were read locally via `git show origin/<head>:<path>` after fetching, which is cheaper and more reliable than paging large API payloads.

---

## 2026-09-06

- PRs reviewed: **20 of 22 open** (every open PR targets `dev-paul`; none heads `main` or a `dev-*` branch, so all were eligible for push under the branch-safety rule). #2870 and #2848 were skipped as unchanged since their 2026-09-05 reviews — re-posting an identical review on an untouched head is noise, not coverage.
  - **#2888** feat(settings): widget settings drawer — waves 0 + 1a (`feat/settings-drawer-w01a`)
  - **#2887** docs(debugger): log run 62 (`nightly/debugger-log-2026-09-06`)
  - **#2886** fix(blooms-taxonomy): self-remove orphaned detail companion (`nightly/widgets-2026-09-06`)
  - **#2885** fix(quiz): stop concurrent recording-take commits from dropping takes (`nightly/state-data-2026-09-06`)
  - **#2884** fix(auth): require a verified email before `resolveOrgForUser` resolves an org (`nightly/build-tooling-2026-09-06`)
  - **#2883** fix(admin): canonicalize building id indexing in `UrlConfigurationPanel` (`nightly/admin-config-2026-09-06`)
  - **#2882** fix(a11y): roving-tabindex nav into `PublishScoresModal` radiogroup (`nightly/dashboard-layout-2026-09-06`)
  - **#2881** fix(a11y): roving tabindex + arrow-key nav in `TimeToolConfigurationPanel` (`nightly/unify-settings-labels-2026-09-06`)
  - **#2880** fix(need-do-put-then): convert hand-rolled empty state to `ScaledEmptyState` (`nightly/unify-empty-states-2026-09-06`)
  - **#2879** docs(debugger): log run 61 (`nightly/debugger-log-2026-09-05`)
  - **#2878** fix(ci): remove `--concurrency` from type-aware lint (`nightly/build-tooling-2026-09-05`)
  - **#2877** fix(admin): canonicalize building id indexing in `ClockConfigurationPanel` (`nightly/admin-config-2026-09-05`)
  - **#2876** fix(dashboard): `globalStyle`/`sharedGroups` edits were never autosaved (`nightly/state-data-2026-09-05`)
  - **#2875** pr-review: 6 PRs reviewed — 2026-09-05 (`claude/pensive-bell-ahjk4f`)
  - **#2874** fix(a11y): roving tabindex in `TextSizePresetSettings`/`WidgetBuildingToggle` (`nightly/dashboard-layout-2026-09-05`)
  - **#2873** fix(instructional-routines): sort grade-level label by canonical order (`nightly/widgets-2026-09-05`)
  - **#2872** fix(css-scaling): cap `ExpectationsWidget` icon containers + `DiceWidget` die sizes (`scheduled-tasks`) — **re-review**, head advanced `b609e52` → `cc0d36e5`
  - **#2871** fix(a11y): roving tabindex + arrow-key nav in TimeTool radiogroups (`nightly/unify-settings-labels-2026-09-05`)
  - **#2849** docs(unifier): log runs 76-78, archive old Run Log (`nightly/unifier-log-2026-09-04`) — **re-review**, head advanced `3419385` → `4632fc2`
  - **#2847** fix(a11y): associate the "Related widgets" checkbox group with its label (`nightly/unify-settings-labels-2026-09-04`) — **re-review**, head advanced `0e2faba` → `161c03b`
- Comments processed: **0 unresolved.** Enumerated inline review threads on all 22 open PRs: five threads exist in total (#2878 ×1, #2876 ×2, #2871 ×2) and **every one is already resolved**, each with a reply recording the fixing commit. No PR carried an outstanding change request. Separately, the two prior-run review findings that were still open at last check have both been acted on in later commits — #2849's archive date range (`f1fddc6`) and #2847's decorative-icon `aria-hidden` (`161c03b`) — which is why both were re-reviewed rather than replied to.
- Fixes pushed: **none.** No comment identified an unaddressed defect. Every finding produced this run is either a design decision with more than one defensible resolution (#2886's cascade location, #2885's two failure paths), a cross-PR merge-ordering hazard that no single-branch push can fix (#2871/#2874, #2879/#2887), or an out-of-scope discovery raised for the backlog (#2881's raw-id indexing). All fail the "obvious correct fix" bar.
- Reviews posted: **20.** Findings that came from checking a claim against the code rather than restating the diff:
  - **#2871 and #2874 will conflict on `components/common/radioGroupKeyNav.ts`, and resolving it wrong silently reverts a real fix.** Both edit the same three lines: #2871 replaces `if (!nextOption) return` with a bounds check (the falsy-option bug — `VOICE_LEVEL_OPTIONS` contains `null` and `0`, so keyboard users could never reach "None" or Level 0); #2874 keeps that line and adds Down→Right/Up→Left normalization above the key guard. **The correct merged result keeps both.** Taking either side wholesale loses the other, and only #2871's own new test file would catch it. Raised on both PRs plus #2882, which consumes the helper and depends on the outcome.
  - **#2882's radiogroup is vertical and the helper on `dev-paul` has no vertical arrow keys.** The score-visibility options render in a `space-y-2` stack, but `handleRadioGroupKeyDown` returns early on ArrowDown/ArrowUp — the fix for that lives in #2874, a *different* open PR. Its own test is titled `'ArrowRight/ArrowDown moves focus…'` and only fires ArrowRight, so the file asserts exactly the coverage it lacks. Also: the buttons carry `disabled={submitting}`, which guards the click path but not the group-level keydown, so arrows can change the selection under an in-flight submit.
  - **#2886 turns every dangling `parentWidgetId` into a silent delete.** Grepped the whole codebase: `parentWidgetId` appears in exactly one place outside `components/widgets/BloomsTaxonomy/` — its own declaration at `types.ts:2166`. Nothing remaps it, while `DashboardContext.tsx:5434`'s batch add mints fresh UUIDs and carries `config` through verbatim. So any template/paste/import path yields a `blooms-detail` pointing at an id that no longer exists; before this PR that rendered a wrong-but-recoverable panel, after it the panel deletes itself on first render. Compounded by the mechanism: CLAUDE.md explicitly steers event-triggered logic out of `useEffect`, and `removeWidget` — which already cascades group dissolution — is the natural home, keying off an actual deletion rather than "I can't find my parent right now."
  - **#2881 is wiring accessible keyboard access onto a panel whose values may not round-trip.** While reading the file for the a11y change, found `TimeToolConfigurationPanel.tsx` indexing `buildingDefaults[selectedBuildingId]` by the raw id on both read (`:93-97`) and write (`:105`), with `canonicalBuildingId` not imported at all — the 9th instance of the class #2877 and #2883 are fixing as the 7th and 8th, and one the debugger backlog's systemic-gap row doesn't name. Out of scope for that PR; raised there and on #2887 so it lands in the backlog rather than being rediscovered.
  - **#2885's transaction introduced two silent "didn't write" paths, and the caller ignores both.** `QuizStudentApp.tsx:757` discards the return value and proceeds to `runRecordingUpload` regardless. Before this PR a missing response doc made `updateDoc` reject and the upload never ran; now `if (!snap.exists()) return` resolves cleanly and an audio blob is written to Storage that no `answers` row will ever reference — directly contradicting the comment two lines above it ("the metadata lands first"). Second, `runTransaction` requires connectivity where `updateDoc` queued offline, which is a real change for students on classroom wifi.
  - **#2884's fix is consistent, and I checked rather than assumed.** The concern with a verified-email gate is that the same trust decision exists elsewhere and only one copy gets fixed. It doesn't here — `aiGeneration.ts`'s `resolveOrgIdForToken` already carries the identical rail with the reasoning spelled out at `:209-213`. `resolveOrgForUser` was the outlier, and the file's own doc comment already *claimed* to check a verified token. Noted that the reject path is silent by design: `{orgId: null}` is a success, so `AuthContext`'s operator-org fallback doesn't engage and an affected user just loses their org with nothing in the logs.
  - **#2872's prior finding is resolved with the right value and the right scope.** Re-did the arithmetic on the current head: the Volume card's capped children total 48+2+8+14 = 72px plus 16px padding = **88px**, exactly the new `min(88px, 18cqmin)` ceiling, while the two percentage-sized siblings correctly keep 72px. Also verified the new `DiceWidget` caps: the 3-dice cap (130px) being *smaller* than the 4-dice cap (140px) reads backwards until you check `getGridCols()` — three dice render `grid-cols-3` (one row) and four render `grid-cols-2` (2×2), so four genuinely have more room each. Dividing each cap by its coefficient puts every engagement threshold in a 293-367px band, so no die count freezes noticeably earlier than another.
  - **#2879 and #2887 conflict with each other, and `debugger.md` predicted it verbatim.** `dev-paul` is at run count 60; both log PRs branch from it independently and both rewrite the trailing run counter and the same backlog rows — `git merge-tree` reports 6 both-changed regions. `debugger.md:706` records this exact incident from 2026-08-15 ("guaranteeing a merge conflict and a contradictory PR-attribution if merged out of order") along with its remedy: rebase the later log branch onto the earlier unmerged one. Run 62's own Phase 0 saw 13 unmerged nightly PRs and chose to proceed — defensible for the code worktrees, whose file sets are disjoint, but the log branch is the one place every run touches the same lines.
  - **#2888's board-isolation surface is clean, which was the thing most worth checking.** `utils/widgetConfigPersistence.ts` refactors `APPEARANCE_CONFIG_KEYS` from a literal `Set` into a `const` list plus a derived `Set` — membership byte-for-byte unchanged, no key added. That matters twice, because the `widgetDefaults` backfill materializes `fontFamily`/`fontColor`/`cardColor`/`cardOpacity` into four widgets' defaults; `mergeWidgetConfig`'s layer order (`defaults < adminConfig < saved < overrides`) means a teacher's saved appearance still wins, and no content key gained account-wide reach. Traced one backfilled value end to end: `clock.themeColor: '#1e293b'` is exactly `STANDARD_COLORS.slate`, the existing fallback at `ClockWidget/Widget.tsx:24`. Real defect found: the ~15-line JSDoc documenting `WIDGET_SETTINGS_COMPONENTS`'s exceptions is now bound to the newly inserted `WIDGET_SETTINGS_SCHEMAS` above it, so the "always handle `undefined`" warning no longer surfaces on the export it was written for.
  - **#2871's Mode radiogroup makes arrow-key nav destructive.** `selectMode` doesn't only set `mode` — it also writes `duration: 600, elapsedTime: 600, isRunning: false, startTime: null`. That reset is right for a deliberate click, but roving tabindex means selection follows focus, so a teacher arrowing across the group wipes a running timer. Standard ARIA behavior meeting a non-standard side effect; a `m === config.mode` no-op guard covers the likely accident.
  - **#2880's `text-slate-400` overrides look like they violate the appearance standard and don't.** Checked the surface rather than the rule: `TileGrid` renders inside the light drawer panel (`hover:bg-slate-100`, sibling controls at `text-slate-500`), and the standard explicitly carves out light surfaces where bumping toward white *reduces* contrast. Flagged instead that the swap is a genuine restyle — the label goes from italic to `font-black uppercase tracking-widest` — and that the shared component adds an `aria-hidden` the hand-rolled version lacked.
- Cross-PR checks run this review:
  - **Two file-level collisions between open PRs, both verified mechanically.** `components/common/radioGroupKeyNav.ts` (#2871 vs #2874) confirmed by reading both branches' versions of the same hunk; `docs/routines/debugger.md` (#2879 vs #2887) confirmed with `git merge-tree` (6 both-changed regions). Last run reported the six PRs it covered as file-disjoint, which was true then and is no longer true of this batch — worth stating because "no conflicts" is a claim that expires.
  - **The shared helper's blast radius was checked before treating its conflict as serious.** `handleRadioGroupKeyDown` has six consumers (`SegmentedControl`, `TypographySettings`, `SurfaceColorSettings`, `ColorPresetPicker`, TimeTool, plus the two #2874 adds). All pass all-truthy option arrays except #2871's, so the falsy bug is latent everywhere but live in exactly one place — which is precisely why an accidental revert during conflict resolution would go unnoticed.
  - **Test-count guard needs no edit, re-confirmed for a much larger diff.** `checkTestCounts.mjs` is a floor, so #2888's ~20 new suites pass it. Noted on that PR that the baseline drifts further from reality as a result.
  - **Regression-surface checks were not vacuous this run**, unlike 09-05. #2876 sits directly in `DashboardContext.tsx`'s onSnapshot merge; #2888 touches `WidgetRegistry.ts`, `config/widgetDefaults.ts` and `utils/widgetConfigPersistence.ts`; #2884 changes a Cloud Function's behavior. Each was read against the specific invariant it could break — the merge's baseline bookkeeping, the three registry maps' mutual consistency, and `functions/src/index.ts`'s export surface respectively. None broken.
  - **Docs-only PRs (#2879, #2887, #2849, #2875) correctly show zero checks**, because `pr-validation.yml` carries `paths-ignore` for `**/*.md` and `docs/**`. Restating because "no checks" and "checks failed" look alike on the PR list — and it is exactly why the log PRs' conflicts had to be found with `git merge-tree` rather than trusted to CI.
- Verification standard: no code was pushed, so no pre-push gate was needed. Verification went into independent re-derivation instead — three whole-file reads to establish findings the diffs don't show (`TimeToolConfigurationPanel`'s raw-id indexing, `QuizStudentApp`'s discarded return value, `radioGroupKeyNav`'s consumer set), one codebase-wide grep proving `parentWidgetId` is unmapped anywhere, one `git merge-tree` conflict check, and two independent arithmetic re-derivations of #2872's container budgets.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** No push to any PR head branch either, since no fix was warranted.
  - **Log placement:** pushed to this session's designated branch `claude/pensive-bell-wyfz4r`, branched from **`claude/pensive-bell-ahjk4f`** (#2875's head) rather than `dev-paul`. `dev-paul`'s copy of this file still ends at 2026-09-01 because #2875 is unmerged, so branching from `dev-paul` would have produced two PRs appending to the same file tail — the exact contention this log has recommended fixing for seven runs, and the same shape as the #2879/#2887 collision found tonight. Kept **off** `scheduled-tasks` for the same reason as prior runs: it heads open PR #2872 and carries code, not just journals.
  - **The batch's dominant failure mode this run was not defective diffs — it was diffs that are individually correct and jointly incoherent.** Four of tonight's five most consequential findings are about interactions *between* open PRs: two file-level conflicts, one PR whose feature doesn't work until another lands (#2882 needing #2874), and one wiring accessible access onto a panel another PR class hasn't fixed yet (#2881/#2877/#2883). Every one is invisible from any single PR's diff, and none of the individual PRs is wrong. With 22 PRs open against `dev-paul` and a nightly cadence producing five more per night, cross-PR coherence is now the scarce property, not per-PR correctness.
  - **Phase 1 has been effectively empty for five consecutive runs** — this run most completely, with zero unresolved threads across 22 PRs. The routine's value continues to sit entirely in Phase 2, and specifically in the part that reads whole call sites rather than diffs. Every finding above required leaving the diff.
  - **The append-contention recommendation is no longer hypothetical, and this is the eighth run it has been raised.** One file per run under `docs/scheduled-tasks/pr-review-log/` was proposed on 08-26, 08-28, 08-29, 08-30, 08-31, 09-01 and 09-05; the 08-29 entry was itself lost to that mechanism and restored by #2647. Tonight it forced a real decision — chaining this branch off #2875's head instead of `dev-paul` — and the identical shape appeared independently in `debugger.md` between #2879 and #2887, where the routine's own doc records a past incident of exactly this producing a contradictory merge. Two routines hitting the same structural problem in one night is the argument the seven prior mentions lacked.
  - Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/diff/review operations used `mcp__github__*` equivalents. Diffs and file contents were read locally via `git show origin/<head>:<path>` and `git diff origin/dev-paul...origin/<head>` after fetching, which is cheaper and more reliable than paging large API payloads — the `get_diff` call for #2888 alone exceeded the tool's response limit.
- Post-run outcomes (recorded 2026-09-06T14:20Z, after `dev-paul` advanced to `a924e938`): eight of the reviewed PRs merged the same day, and the three findings this run rated most consequential were all acted on before merge.
  - **#2886 was reworked from a render-effect self-delete into the `removeWidget`/`removeWidgets` cascade the review proposed** (`d519ee4a`), keyed off the actual deletion event rather than "I can't find my parent right now." That removes the dangling-`parentWidgetId` hazard entirely — an imported or pasted detail panel no longer deletes itself, because nothing evaluates orphanhood at render time.
  - **The `radioGroupKeyNav.ts` conflict between #2871 and #2874 was reconciled correctly — both changes survive.** Verified against `origin/dev-paul`: the file now carries #2874's Down→Right/Up→Left normalization *and* #2871's `if (nextIdx < 0 || nextIdx >= options.length) return` bounds check. This was the failure mode the review flagged as silent, so it is worth recording that it did not occur. #2890 additionally pinned the ArrowDown/ArrowUp aliasing in the helper's own suite, which is the helper-level coverage gap raised on #2874.
  - **#2875's 5-vs-1 comment tally was clarified before its squash-merge**, splitting the count into "1 inline review thread + 4 review-level summary comments." That amended line is the one kept when this run's branch merged `dev-paul` back in.
  - **Merge mechanics worth carrying forward: chaining off an unmerged log branch trades a guaranteed conflict for a conditional one.** #2875 was squash-merged, so its commit is not an ancestor of `dev-paul` and this branch's copy of the 09-05 entry conflicted with the squashed equivalent — resolved by merging `dev-paul` in and taking `dev-paul`'s side for the 09-05 entry, this branch's for 09-06. Branching from `dev-paul` instead would have conflicted too, and worse (two divergent 09-05 entries rather than one duplicated). The per-run-file split remains the only resolution that avoids both.

## 2026-09-07

- PRs reviewed: 11 — all open, all targeting `dev-paul`, none with a `main`/`dev-*` head branch.
  - #2903 `docs(debugger): log run 63 — 5 bugs shipped, worktree-staleness lesson generalized`
  - #2902 `fix(admin): canonicalize building id in TimeToolConfigurationPanel`
  - #2901 `fix(quiz): stop submitAnswer from dropping a sibling write under concurrency`
  - #2900 `fix(common): give SegmentedControl Down/Up arrow-key nav via the shared radiogroup helper`
  - #2899 `fix(settings): forward scaleMultiplier fallback into schema TextSizePreset field`
  - #2898 `fix(rules): require a real, approved submission for wall likes/comments`
  - #2897 `fix(css-scaling): cap CatalystVisualWidget icon and title cqmin`
  - #2896 `feat(settings): widget settings drawer — wave 2 (pickers + Timer/Note/Embed/Clock/Lunch migrations)`
  - #2895 `docs(unifier): log run 79 — 2 shipped (D3, D4), 3 aligned (D1, D2, D5)`
  - #2894 `fix(settings): use @/ alias for settings-drawer schema imports (D4)`
  - #2893 `fix(a11y): roving-tabindex keyboard nav for MusicWidget radiogroups (D3)`
- Comments processed: 10 total — 0 fixed, 10 required no code change.
  - 1 inline review thread (#2897, `CatalystVisualWidget.tsx`) — already resolved *and* already fixed on the branch before this run: the hard `min()` ceiling was replaced with `clamp(32px, 40cqmin, 220px)` / `clamp(14px, 6cqmin, 36px)` in `ff8ac6cf`. Verified against the current head rather than trusting the resolved flag.
  - 9 review-level summary comments from `claude[bot]` across #2902, #2901, #2900, #2899, #2898, #2897 (×2), #2894, #2893. Every one is an LGTM/no-changes-requested summary containing no change request, so none met the "fix is needed" bar.
  - The one item that looked actionable — #2897's nit that the PR description still described `min(...)` while the diff used `clamp(...)` — was checked against the live PR body and found **already corrected**; the description now documents the review-driven revision. No reply posted.
  - **No "no fix needed" replies were posted, deliberately.** The task's step 4 prescribes replying to each unresolved comment, but all nine remaining items are the bot's own approving summaries with nothing to answer, and the single inline thread is both resolved and already answered by the author. Nine "no action required" replies on nine LGTMs is noise on the threads a human actually reads.
- Fixes pushed: **none.** Phase 1 produced no code change on any branch, because no unresolved comment requested one.
- Reviews posted: 11 (one per open PR).
- Findings that mattered this run:
  - **#2896 is red on its own acceptance tests, and that is the run's headline.** All 8 failing E2E specs are files this PR adds — every `tests/e2e/settings-drawer-*.spec.ts` (clock ×2, embed ×2, lunchCount, text, time-tool ×2); 25 passed, 8 failed. The `tab stop 21 left the dialog` assertion at `settings-drawer-time-tool.spec.ts:174` failed on the initial run **and both retries**, so it is a deterministic focus-trap defect, not flake. Recorded because "8 failed" on a 6,584-line PR invites a flake assumption that the retry record disproves.
  - **#2896 reintroduces, in new code, the exact bug it fixes in old code.** `SettingsDrawerHost.updateConfig` is correctly changed to a sparse patch (`updateWidget(widgetId, { config: patch })`) with a comment explaining that `updateWidget` merges over the live config so an async write can't revert newer keys. But both fallback components added *in the same PR* — `legacy/SchemaSettingsFallback.tsx:59` and `legacy/SchemaAppearanceFallback.tsx:59` — still spread a captured `widgetConfig`. Confirmed the merge premise directly at `DashboardContext.tsx:5686` (`newConfig = { ...w.config, ...updates.config }`, inside a functional `setDashboards`), so the sparse patch is sound and the spread is both redundant and stale-prone. Second-order consequence worth more than the first: `updateWidget` also calls `saveWidgetConfig(widgetType, updates.config)` at `:5725`, which filters to `APPEARANCE_CONFIG_KEYS` and writes them **account-wide** — so under the full spread, every settings change in the flag-off path rewrites all of that widget's account-wide appearance defaults, including on a pure content edit. The host got a dedicated `SettingsDrawerHost.sparsePatch.test.tsx`; the two fallbacks got none, which is plausibly why the old shape survived into them.
  - **#2901's transaction conversion trades offline durability on the app's hottest write path.** `submitAnswer` runs on every debounced keystroke autosave for written responses, for every student. `runTransaction` requires a server round trip and does not use the offline cache, where `updateDoc` queued locally and flushed on reconnect — and the function's own comments name the flush paths (`visibility-hidden`, `beforeunload`) that are exactly when classroom wifi is least reliable. Also ~2× billed ops on that path. Noted that the file already carries four other `runTransaction` sites, so the pattern is established and this is consistent with it; the difference is call frequency, which is why it warrants an explicit decision rather than silent inheritance. Same shape as the #2885 finding recorded on 09-06 — this is that finding's second instance, now on a hotter path.
  - **#2901 adds a silent-drop path.** `if (!snap.exists()) return null` resolves cleanly where `updateDoc` previously rejected, so a student's answer would be discarded with the caller seeing success and nothing logged. Verified both call sites (`QuizStudentApp.tsx:639`, `VideoActivityStudentApp.tsx:410`) `await` without a `.catch()`. Directly parallel to the #2885 finding on 09-06 — the same guard shape recurring in a sibling function.
  - **#2900 aliases Down/Up on a `tablist`, which is correct for radiogroups and non-conformant for horizontal tabs.** `SegmentedControl`'s default role is `tablist`; per the WAI-ARIA APG, Down/Up navigate a tablist only when `aria-orientation="vertical"`. The new test exercises the default role and asserts ArrowDown moves between tabs, so the deviation is now pinned by a test — which is the part worth flagging, since it converts an open question into a locked-in contract.
  - **#2902's canonicalization performs a silent whole-record migration on an unrelated edit.** `handleUpdateBuilding` spreads the *canonicalized* record, so the first edit to any building rewrites every key in stored `buildingDefaults` to canonical form. Almost certainly intended, but `canonicalizeBuildingKeyedRecord` (`config/buildings.ts:140`) is last-write-wins, so a config holding both `schumann` and `schumann-elementary` silently loses one set of defaults on read *and* write. Identical to the prior 8 fixes, so consistent rather than novel — recorded so it is a known property, not a rediscovery.
  - **#2898 was checked for scope rather than assumed safe.** The `!exists(ref) || approved` → `exists(ref) && approved` flip removes a documented allowance for untracked/legacy submission ids, which reads as a breaking change until you confirm the blast radius: both call sites (`firestore.rules:4606`, `:4626`) are `allow create` only, so existing likes/comments still read and delete normally. Only new writes are gated. Flagged the one real client consequence — a like against a post deleted between render and click now returns `permission-denied` where it previously succeeded.
  - **#2896's i18n gap widens rather than closes.** `locales/en.json` gains ~127 keys; `de`/`es`/`fr` gain none. Counted programmatically: `dev-paul` baseline is en 1954 / others 1914 (40-key gap), this PR takes it to en 2081 / others 1914 (167). i18next falls back to English so nothing breaks, but every new settings label is untranslated in three of four supported locales, and no `tests/i18n/*Locales.test.ts` suite appears to gate the new `widgetSettings.*` namespace.
- Cross-PR checks run this review:
  - **No file-level conflicts among the 11 open PRs** — verified from the actual diffstats, not assumed. The two settings-drawer PRs are disjoint (#2894 rewrites import specifiers in `components/settings/renderer/**`; #2896 adds new files under the same tree and edits `SettingsDrawerHost.tsx`), though **merge order matters**: #2894 adds a `no-restricted-imports` rule that #2896's new files must satisfy, so #2894 landing first is the cheaper sequence.
  - **#2893 and #2900 both concern `handleRadioGroupKeyDown` and do not collide.** #2900 edits the helper (widening its selector to `[role="radio"], [role="tab"]`); #2893 only consumes it, and both `MusicWidget` groups are radio-only, so the widened selector is a no-op there regardless of order. Checked because this exact helper produced the #2871/#2874 silent-revert hazard on 09-06.
  - **#2896's registry change was verified against all three maps.** Each of the five migrated types (`text`, `embed`, `lunchCount`, `clock`, `time-tool`) is added to `WIDGET_SETTINGS_SCHEMAS` and removed from **both** `WIDGET_SETTINGS_COMPONENTS` and `WIDGET_APPEARANCE_COMPONENTS` — no type double-registered, no orphan. Also confirmed no dead exports survive the five `Settings.tsx` deletions: each widget's `index.ts` re-export was removed in step, and `UnsupportedField` has zero remaining references after its file was deleted.
  - **Docs-only #2903 and #2895 correctly show zero check runs**, matching `pr-validation.yml`'s `paths-ignore` for `**/*.md` and `docs/**`. Restated for the third run running, because "no checks" and "checks failed" are visually identical on the PR list.
- Verification standard: no code was pushed, so no pre-push gate was needed. Verification went into re-derivation the diffs don't show — reading `updateWidget`'s body at `DashboardContext.tsx:5674-5735` to confirm the merge semantics that make #2896's sparse patch correct *and* its fallbacks wrong; grepping `awEngagementSubmissionOk`'s call sites to establish #2898 is create-only; reading `canonicalizeBuildingKeyedRecord` to characterize #2902's collision behavior; confirming `WIDGET_PALETTE` contains no white, so #2893's `[...WIDGET_PALETTE, '#ffffff']` has no duplicate-hex ambiguity today; counting locale keys programmatically across four files on two branches; and pulling #2896's E2E job log to establish that its 8 failures are its own new specs and that the focus-trap assertion failed on all three attempts.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** No push to any PR head branch either, since no fix was warranted.
  - **Log placement: pushed to this session's designated branch `claude/pensive-bell-u2b1rq`, branched from `dev-paul`.** Kept **off** `scheduled-tasks` for the same reason as prior runs, which applies unusually directly tonight: that branch heads open PR #2897, which carries a real code change (`CatalystVisualWidget.tsx`). Appending a 600KB-file bookkeeping commit there would have broadened a clean, single-purpose, review-complete PR and invalidated the review posted to it minutes earlier.
  - **Append contention was absent tonight, and that is worth recording as data rather than relief.** `dev-paul` and `scheduled-tasks` hold byte-identical copies of this file through the 09-06 entry, and none of the 11 open PRs touches it — so branching from `dev-paul` was clean, with none of the chaining gymnastics the 09-06 run needed. The reason is simply that the prior log PR merged; the structural hazard is unchanged and will recur the first night one doesn't. This is the ninth run recommending one file per run under `docs/scheduled-tasks/pr-review-log/`.
  - **Phase 1 has now been effectively empty for six consecutive runs** — this run entirely, with zero actionable comments across 11 PRs and zero fixes pushed. All value again sat in Phase 2, and specifically in the parts that left the diff: the #2896 fallback finding required reading `DashboardContext.tsx`, and the #2896 E2E finding required reading a CI job log. Neither is visible from the PR diff at all.
  - **Two of tonight's findings are recurrences of findings recorded on 09-06**, both from the same `runTransaction` conversion pattern: the silent `!snap.exists()` drop and the loss of offline write queuing. On 09-06 they were raised against #2885 (`commitRecordingTake`); tonight the identical pair appears in #2901 (`submitAnswer`), on a path that runs orders of magnitude more often. A per-PR review catches each instance; what makes it a pattern is the log. Worth stating plainly on the next conversion rather than a third time after the fact.
  - Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/diff/review operations used `mcp__github__*` equivalents. Skill files were read from the in-repo `.claude/skills/` (`new-widget`, `admin-widget-config`) — the task's `/mnt/skills/user/` paths do not exist in this environment. Diffs were read locally via `git diff origin/dev-paul...origin/<head>` after fetching each head, which again proved necessary: `get_diff` on #2893 and #2899 returned multi-hundred-KB payloads dominated by a single `legacySnapshots.test.tsx.snap` line, and #2903/#2895's docs diffs exceeded the inline output limit.

## 2026-09-08

- PRs reviewed: 5 — all open PRs, all draft, all based on `dev-paul`, all authored by OPS-PIvers.
  - #2905 `nightly/unify-d2-brand-colors-2026-09-08` — use `--spart-primary` CSS var for settings-drawer focus outline (D2)
  - #2906 `nightly/unify-d3-settings-labels-2026-09-08` — dedupe radiogroup keyboard nav in the settings-drawer `Segmented` field (D3)
  - #2907 `nightly/unify-d4-import-paths-2026-09-08` — use `@/` alias for board-fixture imports in `boardLoadPaths.test.ts` (D4)
  - #2908 `nightly/unifier-log-2026-09-08` — unifier run-80 log entry (docs)
  - #2909 `skill-fix/new-widget-checklist-ref` — swap stale ClockWidget/LunchCount Settings+Appearance skill reference (docs)
- Comments processed: **0 actionable.** Zero inline review threads across all five PRs (`get_review_comments` returned an empty thread list for each), zero submitted reviews, and three issue-level comments — all of them `claude[bot]`'s own approving summaries on #2905/#2906/#2907, none requesting a change.
  - **No "no fix needed" replies were posted, deliberately.** The task's step 4 prescribes a reply per unresolved comment, but all three are LGTM summaries on issue-level threads with nothing to answer. Three "no action required" replies on three approvals is noise on threads a human reads. Same call as the prior six runs.
- Fixes pushed: **none.** No comment requested a code change, so Phase 1 produced no diff on any branch.
- Reviews posted: 5 (one per open PR).
- Findings that mattered this run:
  - **The run's most valuable result was a regression that turned out not to exist — and the way it was ruled out is the point.** #2906 removes an inline `handleKeyDown` whose `NAV_KEYS` array explicitly listed `ArrowDown`/`ArrowUp`, and replaces it with `handleRadioGroupKeyDown`, whose guard names only `ArrowRight`/`ArrowLeft`/`Home`/`End`. Read as a diff, that is a dropped key binding on a `flex-wrap` radiogroup where options genuinely wrap to multiple rows — a real WAI-ARIA defect. It is not one: `radioGroupKeyNav.ts` normalizes `ArrowDown → ArrowRight` and `ArrowUp → ArrowLeft` *before* the guard. Confirmed by checking out `132865d` and running `fieldAccessibility.test.tsx` + `Segmented.test.tsx` — 32/32 pass, and `fieldAccessibility.test.tsx:234` asserts `ArrowUp` moves selection, so vertical nav is under live assertion rather than inference.
  - **That near-miss came from reading a helper on the wrong branch, and it is worth recording as a method failure, not a lucky catch.** The first read of `components/common/radioGroupKeyNav.ts` was against the checked-out working tree, which sat on a `main`-based branch. `main` predates the Down/Up normalization; `dev-paul` has it. Every fact derived from that read was wrong, and the same stale-tree read also produced a bogus "the PR description names consumers that don't use the helper" finding (on `dev-paul`, `SegmentedControl`, `TypographySettings`, `TextSizePresetSettings` and `RosterPicker` all do — 12 call sites total). **Takeaway: when reviewing a PR based on `dev-paul`, read supporting files with `git show origin/<head>:<path>` or from a checkout of the head — never from whatever the working tree happens to be on. `git grep <pattern> origin/dev-paul -- <path>` for the same reason.** An earlier grep for `data-settings-target` returned nothing for exactly this reason and would have supported a false "this CSS rule is dead code" finding on #2905.
  - **#2905's selector and variable were both verified live, at each end.** `[data-settings-target]` is produced by `SETTINGS_TARGET_ATTRIBUTE` (`components/settings/useSettingsDrawerFocus.ts:7`) and asserted in two drawer tests. `--spart-primary` is written by `DashboardView.tsx` twice — inline on `#dashboard-root` (~:1544) and onto `document.documentElement` via effect (~:1513) — and the `documentElement` mirror is what keeps portaled/maximized widgets inside the variable's scope. The `, #2d3f89` fallback covers the pre-effect interval. Both halves needed checking because a theme-var conversion is only correct if the variable actually resolves where the selector matches.
  - **#2909's claims were verified individually rather than accepted as a set.** All five widgets named as schema-only (`clock`, `time-tool`, `text`, `embed`, `lunchCount`) have no `Settings.tsx` on `dev-paul`, appear in neither `WIDGET_SETTINGS_COMPONENTS` nor `WIDGET_APPEARANCE_COMPONENTS`, and all five are in `WIDGET_SETTINGS_SCHEMAS` (`WidgetRegistry.ts:271-276`). The replacement reference is sound: `Checklist/Settings.tsx` backs both `ChecklistSettings` (`:287`) and `ChecklistAppearanceSettings` (`:434`), and there is no separate `Checklist/Appearance.tsx`, so pointing the "Settings + Appearance split" row at `Settings.tsx` is accurate rather than a half-truth.
  - **#2909 can only half-close the skill-file divergence it touches.** `.claude/skills/new-widget/SKILL.md` (617 lines) gets the note in two places — the Step-3.5 coexistence paragraph and the reference table; `.agents/skills/new-widget/SKILL.md` (546 lines) gets the table only, because it has no Step-3.5 section at all. Pre-existing drift the PR inherits, not one it introduces, but the two copies are now 71 lines apart and diverging per-PR. Flagged so it is not read as an oversight in this diff.
  - **#2908's log entry was fact-checked against the tree, not taken at face value.** Its D3 row spells the shared helper's key set as "Right-Down-Left-Up/Home-End" — the claim most likely to be wrong given the guard's literal contents, and the same one that produced this run's near-miss. It is correct as written. Also confirmed: `Embed/Settings.tsx` is gone (so the D5-E26 → D5-E39 supersession is real), and all five `Settings.tsx` deletions the entry describes actually happened.
- Cross-PR checks run this review:
  - **No file-level conflicts among the five open PRs** — verified from diffstats, not assumed. Each touches a disjoint path: `index.css`, `components/settings/renderer/fields/Segmented.tsx`, `tests/utils/boardLoadPaths.test.ts`, `docs/routines/unifier.md`, and the two `SKILL.md` copies. Merge order is free.
  - **#2908 documents #2905/#2906/#2907, so it should land last** — not a conflict, but if any of the three is rejected or reworked, the run-80 log entry describing it as "SHIPPED" becomes wrong on merge.
  - **CI verified per PR, not assumed from the list view.** #2905/#2906/#2907 each carry 14 check runs, all `success`, on their current heads. #2908/#2909 show **zero** check runs, which is correct — both are docs-only and match `pr-validation.yml`'s `paths-ignore`. Restated for the fourth run running, because "no checks" and "checks failed" look identical on the PR list.
- Verification standard: no code was pushed, so no pre-push gate applied. Verification went into re-derivation the diffs don't show — running two settings-drawer suites on #2906's actual head to settle the arrow-key question empirically, running `boardLoadPaths.test.ts` on #2907's head (7/7) and confirming all five fixture files exist there, tracing `--spart-primary` from its two writers in `DashboardView.tsx` to the `[data-settings-target]` selector's producer, and checking each of #2909's five schema-only claims against all three registry maps plus the filesystem.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** No push to any PR head branch either, since no fix was warranted.
  - **Log placement: pushed to `scheduled-tasks`, per the task's Post-Task step.** Unlike the 09-06 and 09-07 runs, that branch heads **no** open PR tonight (all five open PRs are `nightly/*` or `skill-fix/*`), so appending here broadens nothing under review. It does carry two unrelated commits from the tuesday-audit routine (`3ec40be`, `293ea6a`) that any PR from this branch necessarily includes.
  - **The `jest-axe` cold-start gap hit again, for the third consecutive observation, and #2908's own log predicted it.** The first `vitest` invocation failed with `Cannot find module 'jest-axe'` from `tests/setup-axe.ts` across the target suites, despite the SessionStart hook having just run `pnpm run install:all`. A plain `pnpm install` fixed it in ~2s. Recognized instantly from the run-79/80 notes rather than investigated — which is the doc paying for itself, and is also why #2908's proposed "plain `pnpm install` as Phase-0 step 0" is worth adopting beyond the unifier routine. **Any scheduled task in this container that runs tests should do it unconditionally, before the first test invocation.**
  - **Phase 1 has now been effectively empty for seven consecutive runs** — zero actionable comments across five PRs, zero fixes pushed. All value again sat in Phase 2, and specifically in the part that left the diff: #2906's central question was unanswerable from the diff alone and needed a checkout plus a test run.
  - **Tonight is the first run where the *review method* was the finding.** Six prior runs recorded findings about the code; this one recorded that reading a supporting file from the wrong branch nearly produced two confident, wrong, high-severity findings on two separate PRs. The reviews posted are correct because the claims were re-derived from the head branches before posting — that re-derivation step should be treated as mandatory, not diligent.
  - Tooling: GitHub via the MCP server (no `gh` CLI in this environment); all PR list/read/diff/review operations used `mcp__github__*` equivalents. The task's `/mnt/skills/user/` skill paths do not exist here; the in-repo `.claude/skills/new-widget/SKILL.md` and `.agents/skills/new-widget/SKILL.md` were read only in the sections #2909 touches (that PR's subject matter), and `admin-widget-config` was not read at all — no PR this run touched `components/admin/`, so the admin-config criteria had no diff to apply to. Project widget/appearance standards otherwise came from the repo `CLAUDE.md`. Diffs were read locally via `git diff origin/dev-paul...origin/<head>` after `git fetch origin 'refs/heads/*:refs/remotes/origin/*'`; #2908's docs diff again exceeded the inline output limit and was read via a filtered `grep '^+'`.

## 2026-09-09

- PRs reviewed: **9** (every open PR). All nine target `dev-paul`, all nine are drafts, and every head branch is a `nightly/*` or `scheduled-tasks` branch — none is `main`- or `dev-*`-headed, so all were writable.
  - **#2942** docs(debugger): log run 65 — 5 shipped (1 security), zero clean (`nightly/debugger-log-2026-09-09`)
  - **#2941** fix(functions): require verified email for org-admin-by-email gates (`nightly/build-tooling-2026-09-09`)
  - **#2940** fix(schedule-config): canonicalize building ids in ScheduleConfigurationPanel (`nightly/admin-config-2026-09-09`)
  - **#2939** fix(sync): key-stabilize mergeWidget's config/annotation comparisons (`nightly/state-data-2026-09-09`)
  - **#2938** fix(library): keep orphaned folder subtrees pickable in FolderPickerPopover (`nightly/dashboard-layout-2026-09-09`)
  - **#2937** fix(settings): stop List field from stealing focus across rows on remove (`nightly/widgets-2026-09-09`)
  - **#2936** audit(wednesday): daily audits clean, 0 new issues (`scheduled-tasks`)
  - **#2935** docs(unifier): log run 81 — 1 shipped (D3), 4 aligned (`nightly/unifier-log-2026-09-09`)
  - **#2934** fix(settings): retrofit NextUp theme-color swatches to radiogroup pattern (D3) (`nightly/unify-d3-settings-labels-2026-09-09`)
- Comments processed: **6 total — 4 fixed, 2 explained.** All six were `claude[bot]` inline review threads on #2937 (five in the initial review, one more posted against the fix itself); the other eight PRs had zero review threads and zero unresolved comments. **This ends a seven-run streak of empty Phase 1s.**
- Fixes pushed: **2, both to #2937 / `nightly/widgets-2026-09-09`**
  - `5810ca0` — guard `List.tsx` row identity against primitive rows, and cover the sortable-reorder path that no test reached. All 14 CI checks green on this head.
  - `00583e6` — name the by-value id tradeoff for primitive rows in the comment, per the follow-up review round.
- Reviews posted: **9** (one per open PR).
- Findings that mattered this run:
  - **The reported crash was real but the suggested patch would not have fixed the worst case, and the review's own follow-up said so.** `getRowId` read `row.id` *before* any object check, so a `null` row threw on the property access, not on the `WeakMap`. Both the original `suggestion` block and its correction kept that ordering, so applying either verbatim would have closed the primitive case and left the `null` case crashing. Fixed with the guard ordered first.
  - **The reviewer's fix shape would have introduced a render-loop defect of its own.** Minting `row-${rowIdCounter++}` fresh on every call for a primitive means a new React key every render, so the row remounts continuously rather than crashing once. Primitives now key off a value-keyed `Map` alongside the object `WeakMap`. The lesson generalizes: for an identity scheme, "don't throw" and "be stable" are separate requirements, and a guard that satisfies only the first trades a crash for a livelock.
  - **`handleRowChange` had the identical unguarded `WeakMap.set` and no thread mentioned it.** Same input, same throw, one call site further down. Reviewing only the lines a comment anchors to would have shipped a half-fix.
  - **The sortable branch was untested in a way a reorder test would not have revealed.** The `SortableList` mock ignored `getId` entirely and rendered `renderItem` results directly, so React never saw the identity keys at all — any reorder test written against that mock passes regardless of what `getRowId` returns. The mock now keys by `props.getId(item)`, matching the real component (`SortableList.tsx:165`), and only then does the new reorder-focus test mean anything. **A test added against a mock that doesn't exercise the mechanism is worse than no test: it reports coverage that does not exist.**
  - **`act()` was required and its absence failed loudly, which was luck.** The new reorder test drives `capturedOnReorder` directly; without `act()` the state update never flushed and the assertion caught it. The pre-existing reorder test in the same file makes the same unwrapped call and passes only because it asserts on a mock rather than on rendered output.
  - **Two threads were correctly left unfixed, and both reduce to the same design question.** A Firestore snapshot replaces `config` with a freshly `JSON.parse`d object, so equal-content-new-reference rows defeat the `WeakMap`; and two entries pointing at one object collide to a single id. A persisted `row.id` assigned in `createRow()` resolves both, and `getRowId` already prefers `row.id` when present — but it changes the `RowSchema.createRow` contract and writes a synthetic field into persisted config, which has to be reconciled with `widgetConfigPersistence.ts` board isolation. Not a change to push unattended. The per-render dedupe alternative for the collision case cannot be done cleanly inside `getRowId`, because `SortableList` calls `getId` from inside a `useMemo` and a de-dup closure would have to assume call order.
  - **#2941's `evaluateClaim` double-gate is correct, not redundant.** The handler throws before the verdict function is ever reached, so the parameter looks like dead defense — but the pure function is independently unit-tested and independently reusable, so the rail survives a future caller that forgets the handler check. Flagged separately that `organizationUserActivity.ts` now has two adjacent throws where the *older* one carries the message `'Caller must have a verified email.'` for the missing-email case, which now reads as a duplicate of the real check below it.
  - **#2941's two new handler-level tests are negative-only.** `createOrganizationInvites` and `claimOrganizationInvite` had no `onCall` tests before this PR, so nothing asserts a *verified* admin still gets through those two wrappers — a gate that over-blocks would pass the suite. The other two files' pre-existing tests were correctly updated with `email_verified: true`, which keeps their positive paths meaningful.
  - **#2938 fixes the orphan subtree but a pure cycle stays invisible, and that is fine.** Folders in a cycle whose parent ids are all present in the set are unreachable from `walk(null)` and skipped by the orphan loop, since their parents *are* in `knownIds`. The new `visited` set prevents that shape from hanging the browser, which is the half that matters. The same set also turned the old `out.some(...)` dedupe from O(n²) into O(n).
  - **#2934's `aria-label` announces a raw hex.** `Theme color #2d3f89` reads aloud as "number two D three F eight nine" — a name that exists but conveys nothing, on a PR whose entire purpose is the a11y of that control. Also carries a two-line wrapped comment against the standing one-line rule; the file's existing multi-line runs are sequential *distinct* comments, so they don't establish an exempting convention.
  - **#2940 inherits last-write-wins from the shared helper.** `canonicalizeBuildingKeyedRecord` does `out[canonicalBuildingId(rawKey)] = value`, so a config holding both `high` and `orono-high-school` silently drops one on the next save. Established behavior across all 12 instances of this fix — a one-time migration concern, not something to change piecemeal in this PR.
- Cross-PR checks run this review:
  - **No file-level conflicts among the nine PRs** — verified from the diffs, not assumed. The five code PRs touch fully disjoint paths (`functions/src/organization*`, `components/admin/ScheduleConfigurationPanel.tsx`, `utils/dashboardSaveMerge.ts`, `components/common/library/FolderPickerPopover.tsx`, `components/settings/renderer/fields/List.tsx`); the four docs PRs touch four different files. Merge order is free.
  - **#2942 and #2935 document the code PRs**, so both should land after them — not a conflict, but if any code PR is reworked, the log entries describing it as shipped become wrong on merge.
  - **#2934 and #2937 both touch settings-renderer surface but not the same file** (`NextUp/Settings.tsx` vs `renderer/fields/List.tsx`) — no overlap.
- Verification standard: the pushed fix ran `pnpm run type-check` (clean), `eslint --max-warnings 0` on both changed files (clean after fixing one `@typescript-eslint/no-base-to-string` on a `String(row)` in the new test), `prettier --check` (clean), and the affected suites — `List.test.tsx` 9/9 and `fieldAccessibility.test.tsx`, 36/36 together. The primitive-row test was reasoned through against the pre-fix code to confirm it actually fails there (`WeakMap.set` with a string key throws) rather than merely passing after.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** One push to a PR head branch (`nightly/widgets-2026-09-09`), which is a `nightly/*` branch and writable.
  - **Threads resolved, not just replied to:** the three fixed threads on #2937 were marked resolved; the two design-question threads were replied to and deliberately left open, since closing them would bury the decision the author still has to make.
  - **`gh` CLI does not exist in this environment** — all PR list/read/diff/review/reply/resolve operations used `mcp__github__*` equivalents. Worth noting the task prompt is still written entirely in `gh` commands; the mapping is mechanical but has to be re-derived each run.
  - **The `/mnt/skills/user/` skill paths named in the task prompt do not exist here.** Widget and admin-config standards came from the repo `CLAUDE.md` instead. Only #2937 and #2934 touched settings/widget surface, and both are back-face settings panels where container-query scaling rules explicitly don't apply — so the widget-standards criteria had little diff to bite on either way.
  - **No `jest-axe` cold-start failure this run**, breaking a three-observation streak — the SessionStart hook's `install:all` did materialize the dependency this time (it installed `jest-axe`/`@types/jest-axe` visibly during startup). The gap is environment-dependent rather than deterministic, which is an argument for the unconditional plain `pnpm install` rather than against it.
  - **Node version mismatch is present but benign:** the container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation. Type-check, lint, and the test suites all ran clean regardless. Recorded because a future run seeing a genuine failure here should not spend time re-diagnosing the warning.
  - **The review re-ran against the fix and found something the first pass couldn't.** The new thread noted the primitive fallback keys by *value*, so `['a', 'a']` collides — a real observation about code that didn't exist an hour earlier. Took the cheap half of its suggestion (name the tradeoff in the comment) and declined the other half (mint per-occurrence), because two equal primitives are indistinguishable to every consumer in this component: `renderRow` gets the value, the handlers take an index, nothing reads a per-occurrence id back. **Worth noting the review loop closed properly here — fix, re-review, narrower follow-up, resolve — rather than the more common pattern of a reviewer repeating an unaddressed point.**
  - **Phase 1 produced real work for the first time in eight runs, and the value was in disagreeing with the review.** Three of five threads were actionable, but two of those three needed a different fix than the one suggested, and one needed a fix at a call site no thread mentioned. Applying the `suggestion` blocks as offered would have left a `null`-row crash open and introduced a per-render remount. **Treating an automated review comment as a diagnosis to verify rather than a patch to apply is what produced the correct fix here.**

## 2026-09-10

- PRs reviewed: **9** (every open PR). All nine target `dev-paul`, all nine are drafts, and every head branch is a `nightly/*` or `scheduled-tasks` branch — none is `main`- or `dev-*`-headed, so all were writable.
  - **#2954** docs(debugger): log run 66 (2026-09-10) (`nightly/debugger-log-2026-09-10`)
  - **#2953** fix(rules): require verified email across the org-role hierarchy [SECURITY] (`nightly/build-tooling-2026-09-10`)
  - **#2952** fix(admin): canonicalize building id in ScoreboardConfigurationPanel (`nightly/admin-config-2026-09-10`)
  - **#2951** fix(layout): preserve aspect ratio when min-size floor clamps a tiny widget (`nightly/state-data-2026-09-10`)
  - **#2950** fix(settings): fall back to legacy content on a failed schema chunk load (`nightly/dashboard-layout-2026-09-10`)
  - **#2949** fix(concept-web): clamp node drag/resize to container bounds (`nightly/widgets-2026-09-10`)
  - **#2948** fix(custom-widget): drop opaque bg-slate-800 from loading/no-content states (`scheduled-tasks`)
  - **#2947** docs(unifier): log run 82 — 1 shipped (D3), 4 aligned (`nightly/unifier-log-2026-09-10`)
  - **#2946** fix(settings): retrofit MathToolInstance Settings radiogroups to WAI-ARIA radio pattern (D3) (`nightly/unify-d3-settings-labels-2026-09-10`)
- Comments processed: **2 total — 0 fixed, 0 explained (0 actionable).** Both were `claude[bot]` inline threads that the author had already fixed and replied to before this run: the `applyMinSizePreserveAspect` zero-dimension `NaN` note on #2951 (fixed in `32923a0b`) and the `text-slate-400` contrast note on #2948 (fixed in `77e084dd`). Both threads are resolved. Verified the fixes actually landed rather than trusting the replies — #2951 carries the `rect.w <= 0 || rect.h <= 0` early return plus two regression tests, and #2948's file has zero remaining `text-slate-400`/`bg-slate-800` occurrences on its head.
- Fixes pushed: **none.** No open comment required a code change, so Phase 1 was correctly a no-op rather than a manufactured one.
- Reviews posted: **9** (one per open PR).
- Findings that mattered this run:
  - **#2953's real risk is the ruleset size cap, not the rule logic.** The `email_verified` gate itself is correct at all 7 sites — operator precedence on the two reworked compound `allow read` clauses is sound (the new `&& (resource == null || …)` sits inside the existing parenthesized self-probe group, leaving the `|| isOrgMember() || isSuperAdmin()` alternatives intact), and `get('email_verified', false) == true` handles an omitted claim correctly. But the PR takes `firestore.rules` from 251,971 → 254,116 bytes: **+2,145 bytes, now 96.93% of the 262,144-byte cap with 8,028 bytes left, and ~1,600 of those bytes (22 lines) are comment prose rather than rule logic.** This is not a style nit: `scripts/checkRulesSize.mjs`'s own header records that **#2915 added 569 bytes to a file 92 bytes under the cap, passed PR validation green, and then broke the `dev-paul` deploy** — the emulator does not enforce the cap, so `test:rules` structurally cannot catch it, and the failure lands post-merge in a deploy carrying indexes, Storage rules and Functions too. This PR adds ~4× the bytes that incident did and spends ~21% of remaining headroom. `check:rules-size` is warn-only above 90%, so CI is green and stays green — which is exactly how #2915 shipped. Measured rather than inferred, and flagged as the one thing to act on before merge.
  - **#2953's `resource == null` carve-outs leave a real existence oracle, and it is the right trade.** On both `admins/{email}` and `members/{emailLower}`, an unverified caller self-asserting an arbitrary address gets `allow` when the doc is absent and `deny` when it exists — distinguishable outcomes, so admin/org-membership status of any guessable address stays enumerable without proving ownership. The bootstrap UX genuinely needs the absent-doc read (`checkAdminStatus()` runs that `get()` for every signed-in user), so the carve-out is correct; recorded so the residual is a decision rather than an oversight.
  - **#2953 is a hard cutover for unverified-but-real accounts, with no grace path.** Any admin or org member whose IdP reports `email_verified: false` loses access at deploy. Traced the realistic exposure to nil — Google SSO sets the claim true, and anonymous/custom-token student sessions carry no `email` at all so they never reached these branches — but it belongs in release notes rather than being discovered by a locked-out admin.
  - **#2949 fixes the bug going forward and leaves already-escaped nodes stranded.** The clamp runs only during interaction; I checked `displayNodes` (line ~385) and the render path applies no clamp. So a node persisted outside 0–100 on a pre-fix board still renders off-container and — because the container is `overflow-hidden` — still cannot be grabbed to drag back. That is precisely the "no in-UI recovery short of editing exported JSON" symptom the PR sets out to fix, now closed only for new occurrences. A clamp when deriving `displayNodes` would heal existing boards in a couple of lines. **The PR description's own framing is what made this gap visible: it argues the bug matters because it is unrecoverable, which invites the question of whether the fix recovers anything.**
  - **#2949's resize clamp can still overflow by a hair, but only on the same legacy data.** In `dim.w = Math.max(5, Math.min(maxW, …))`, when `maxW < 5` (node anchored at `x > 95`) the `Math.max(5, …)` wins and the right edge lands past 100%. Unreachable through normal interaction now that the drag clamp holds `x ≤ 100 − width`. Cosmetic, and it collapses into the same residual as the finding above.
  - **#2946's "Tool Type" group has a keyboard tab stop only by grace of its default config.** Roving tabindex gives a `radiogroup` a tab stop only while some option is selected. The two smaller groups guard that with a default (`?? 'integers'`, `?? 'both'`); "Tool Type" uses a bare `config.toolType === type`. In practice one option always matches — `WIDGET_DEFAULTS.mathTool` sets `toolType: 'ruler-in'` and `TOOL_TYPES = MATH_TOOL_META` enumerates every `MathToolType` — but a widget carrying a persisted `toolType` outside that set would leave every button at `tabIndex={-1}` and make the group keyboard-unreachable, where before the retrofit plain buttons were all tab-reachable. **A retrofit whose entire purpose is keyboard access is the wrong place to depend on defaults being present**; a `TOOL_TYPES.some(...)` fallback to index 0 makes it structurally safe. Verified both the default and the enumeration rather than asserting the risk abstractly.
  - **#2952 is a complete fix, not a half one — checked the part this bug class usually gets wrong.** The remaining raw `selectedBuildingId` uses are `BuildingSelector`'s `selectedId` prop and the `BUILDINGS.find(...)?.name` display lookup, both of which *must* stay raw. That is the documented exception, so nothing was left un-canonicalized. Also noted the inherited last-write-wins in `canonicalizeBuildingKeyedRecord` and that a save silently rewrites other buildings' keys — established behavior across all 13 instances, not this PR's to change.
  - **#2951 trades a narrowing for an assertion.** Hoisting the `aspectFitted` predicate into a boolean const means TypeScript no longer narrows `widget.aspectRatio`, so the diff introduces `widget.aspectRatio as number`. Safe — the predicate guarantees it — but hoisting the *value* first would narrow naturally and need no assertion, which better matches the repo's no-unsafe-assertions standard.
  - **#2950's error is now swallowed silently.** Correct against a permanently stuck skeleton, and there is no logger convention here to hook into, but a `ChunkLoadError` after a deploy invalidates cached chunk hashes now leaves no trace at all. Also noted that its test mutates the shared exported `WIDGET_SETTINGS_SCHEMAS.clock` and relies on Vitest's default `isolate: true` holding.
  - **The two docs-only PRs were checked against the diffs they describe, not read as prose.** #2954's substantive claims all held: the `GraphicOrganizer` "ruled out — fixed grid templates, nothing to clamp" matches what #2949 actually touches, the 27-file count matches its own enumerated list, and the "96.9% of the size cap" figure is accurate (measured 254,116 bytes / 96.93%). #2947's new D3-E29 exception is the most valuable line in either log: it records that `MaterialsWidget/Settings.tsx:442` "Available Materials" is a genuine multi-select and must stay `role="group"`, which is a misconversion a grep-driven sweep would otherwise walk straight into.
- Cross-PR checks run this review:
  - **No file-level conflicts among the nine PRs** — verified from the diffs. The seven code/docs paths are disjoint: `firestore.rules` + `tests/rules/*`, `components/admin/ScoreboardConfigurationPanel.tsx`, `utils/proportionalLayout.ts`, `components/settings/SettingsDrawerHost.tsx`, `components/widgets/ConceptWeb/Widget.tsx`, `components/widgets/CustomWidget/Widget.tsx`, `components/widgets/MathToolInstance/Settings.tsx`. Merge order is free.
  - **#2954 and #2947 document the code PRs**, so both should land after them; if any code PR is reworked, the log entries describing it as shipped go stale on merge. Same standing caveat as prior runs.
  - **#2948 (`scheduled-tasks`) carries this run's own log commit**, since that is the branch the post-task step writes to — so its diff mixes a widget className fix with review-log bookkeeping. Established pattern for this branch, noted because it makes #2948's diff look broader than its code change is.
- CI status: **all six code PRs green, 14/14 checks each** (including `Firestore Rules Tests` on #2953). The two docs-only PRs (#2954, #2947) have **zero check runs** — expected for that path, but it means neither has any automated signal beyond review. #2946's earlier `Unit Tests (shard 1/3)` failure was correctly diagnosed as unrelated (`ActivityWallStudentApp.test.tsx`, tracked in the debugger journal) and passed on re-run.
- Verification standard: **no code was pushed this run, so nothing needed local validation.** Findings were verified by reading the head-of-branch source rather than the diff alone — measured `firestore.rules` byte sizes on both `dev-paul` and the PR head and read `checkRulesSize.mjs` for the actual cap and threshold; grepped `ConceptWeb/Widget.tsx` for a render-path clamp; grepped `ScoreboardConfigurationPanel.tsx` for residual raw-id uses; read `WIDGET_DEFAULTS.mathTool`, `MATH_TOOL_META` and `radioGroupKeyNav.ts` to test the tab-stop concern instead of asserting it; and grepped `CustomWidget/Widget.tsx` to confirm no `text-slate-400` remained. Two of those checks (the `mathTool` default, the `MATH_TOOL_META` enumeration) turned what looked like a bug into a documented-risk note, which is the point of running them.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** The only push is this log commit to `scheduled-tasks`.
  - **Phase 1 was genuinely empty, and that is the correct outcome.** Both existing threads were already fixed, replied to, and resolved by the author before this run started. The remaining review bodies were clean LGTMs, one explicitly-labelled "out of scope, not blocking" follow-up on #2953 (the `plc_invitations` / beta-allowlist `email_verified` siblings — already captured as a #2954 backlog row), and one CI-flake explanation on #2946. **Acting on the #2953 out-of-scope note would have widened a shipped, reviewed-clean security PR on an automated agent's own judgment; it belongs in the backlog it is already in.**
  - **`gh` CLI does not exist in this environment** — all PR list/read/diff/review operations used `mcp__github__*` equivalents. The task prompt is still written entirely in `gh` commands; the mapping is mechanical but has to be re-derived every run. Worth updating the prompt.
  - **The `/mnt/skills/user/` skill paths named in the task prompt do not exist here.** The equivalents are at `/root/.claude/skills/synced/*/spart-{new-widget,widget-admin-config}/SKILL.md`, with repo-local copies under `.claude/skills/`. Widget and admin-config standards came from the repo `CLAUDE.md`. Little was lost: the only widget-surface PRs this run were #2946 (a back-face settings panel, where container-query scaling rules explicitly don't apply), #2948 (a front-face className fix that keeps its `cqmin` inline size), and #2949 (pointer math, no CSS) — and no PR added a new widget, so the 8-file registration checklist had nothing to bite on.
  - **Node version mismatch present and benign again:** container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation. No commands beyond `git`/`grep` were needed this run. Recorded so a future run seeing a real failure here does not re-diagnose the warning.
  - **Two of this run's nine findings came from reading the tooling rather than the diff.** The #2953 size-cap finding only exists because `checkRulesSize.mjs`'s header documents the #2915 deploy failure — the diff itself looks like harmless comments, and CI agrees. **When a repo has written down how it broke before, that file is part of the diff's context.**

---

## 2026-09-11

- PRs reviewed: **6** (every open PR). Four target `dev-paul`, one targets `codex/quiz-target-results` (stacked), and every head branch was `codex/*`, `nightly/*` or `scheduled-tasks` — none was `main`- or `dev-*`-headed, so all were writable.
  - **#2961** Quiz target mastery results and student grouping (`codex/quiz-target-results`)
  - **#2962** feat(plc): aggregate quiz mastery by target (`codex/plc-target-aggregates`, stacked on #2961)
  - **#2964** fix(a11y): retrofit DiceWidget count picker to WAI-ARIA radio pattern (D3) (`nightly/unify-d3-settings-labels-2026-09-11`)
  - **#2965** fix(lint): widen plc no-restricted-imports regex to cover notes/ (D4) (`nightly/unify-d4-import-paths-2026-09-11`)
  - **#2966** docs(unifier): log run 83 — 2 shipped (D3, D4), 3 aligned (`nightly/unifier-log-2026-09-11`)
  - **#2967** fix(css-scaling): convert MiniAppManager front-face to cqmin (`scheduled-tasks`)
- Comments processed: **2 — 1 fixed, 1 explained.** Five of six PRs returned `totalCount: 0` inline review threads. Both open threads were on #2961, from `claude[bot]`. **First run in five with something actionable to act on.**
- Fixes pushed: **1.**
  - **#2961 / `codex/quiz-target-results` / `ad20aee`** — converted every front-face size in `QuizTargetResults.tsx` from hardcoded Tailwind classes to `min(Xpx, Ycqmin)` inline styles, matching the `QuestionsScreen`/`StudentsScreen` convention in the same file. `TargetChips` deliberately left alone: it is shared with PLC and library (non-widget) surfaces. Thread resolved.
- Reviews posted: **6** (one structured review per open PR).
- Findings that came from verifying claims rather than restating the diff:
  - **#2967's conversion is correct, but only because a Tailwind class elsewhere is silently dead.** The converted thumbnails render inside `LibraryItemCard`, whose root carries `@container` (`:450`). Had that been a real query container, `cqmin` would resolve against the *card* with the block axis falling back to the small viewport, and the whole conversion would have been an inert no-op that still measured as "cqmin present." It isn't: this repo is on Tailwind **3.4.19** with only `tailwindcssAnimate` and `reducedMotionPlugin` — no `@tailwindcss/container-queries`. **Confirmed empirically rather than from release-note memory** by running the project's own Tailwind version over a probe file: a control `underline` emitted CSS, `@container` and `@[400px]:flex` emitted nothing. So the nearest real container is `DraggableWindow.tsx:2230` (`containerType: 'size'`) and the PR works as intended. The same probe establishes a pre-existing latent bug — `LibraryItemCard`'s `@[Npx]:` action-label collapsing, described in its own comment at `:444-446`, has never applied. Flagged as out of scope; it is a silent no-op, not a visible failure, which is why it has survived.
  - **#2962 reports "no data" as failure.** `correctPercent: attempted > 0 ? pct(...) : 0` emits `0` for an unattempted target; downstream `masteryBandFor(...) ?? 'beginning'` coerces the null band to `beginning`; and the detail view filters only on `kind !== 'standard'`, never on `attempted > 0`. A target whose questions were never served or graded therefore renders **0% / beginning / red**. This is the inverse of its own sibling in #2961, which filters `servedCount > 0` and uses `null` → `—`. `AggregateTargetRow` carries no served field, so `attempted > 0` is the only available discriminator. The single finding this run rated Needs changes.
  - **#2961's `lowSample` uses two different bases behind one label.** Group rows are `attempted < 5`; per-question detail rows are `servedCount < 5`; both render the same badge, whose tooltip reads "Fewer than 5 scored student-question pairs." For the question rows that label is simply wrong — those are served, not scored.
  - **#2961's mastery-cutoffs thread was a real question with a findable answer, not a judgment call.** Rather than replying "needs product context," traced it: design fact #26 reads "personal views use the defaults," this screen is a personal view, and the PLC counterpart in the stacked #2962 does wire `learningTargetList?.masteryCutoffs ?? DEFAULT_MASTERY_CUTOFFS`. Also surfaced the reason a naive fix is blocked — `QuestionTargetTag.ownerId` is per-tag, so one quiz can mix standards, personal targets and targets from two PLCs, while `computeTargetStats` takes a single cutoffs object. Left open for human confirmation with the evidence attached.
  - **#2965 is correct and the third recurrence of its own bug class.** Verified `components/plc/notes/` exists, that `notes` landed in correct alphabetical position, and — by grep — that zero existing imports violate the widened rule, so it cannot fail lint. Also noted the list is drifting *both* ways: `members`, `sharedBoards` and `todos` are still listed but no longer exist in the tree (`todos` presumably stale since #2958). #2966's own log flags this as the third recurrence (runs 28, 34, 83) and calls it "worth a standing watch-item." Recommended promoting it from watchable to fixed: derive the alternation from `fs.readdirSync('components/plc', { withFileTypes: true })` at config load, which closes both the missing-entry and stale-entry modes permanently.
  - **#2964's helper contract actually checks out.** `handleRadioGroupKeyDown` queries `[role="radio"], [role="tab"]` within `e.currentTarget` and guards `nextIdx >= options.length`; `DICE_COUNTS` is hoisted to module scope and shared between render and handler so node count and option count cannot drift, meaning that guard never trips. Exactly one `tabIndex={0}`. The snapshot diff is limited to the new ARIA attributes — no class changes — confirming the retrofit is non-visual as claimed.
  - **#2961's security-relevant claim verified on both write paths.** Target tags are stripped from the student payload unless the assignment opts in — gated in `projectPublicQuestionForMode` (create path) *and* the re-sync path at `useQuizAssignments.ts:1974`. Also confirmed no `firestore.rules` change is needed: the `quiz_sessions` update rule is an ownership + immutable-identity allowlist with no per-field `hasOnly` (`~:3856`), so the additive `showLearningTargets` field passes as-is.
  - **#2962 breaks no Cloud Function signature.** `resolveGroupQuestions` gained a third parameter, but it defaults to `[]` and has exactly one production caller (`recomputePlcAssessments.ts:163`). `AGGREGATE_SCHEMA_VERSION` correctly bumped 2 → 3, with `perTarget`/`perStandard` parsed as optional so v2 docs still deserialize.
  - **#2962's privacy boundary is the best thing in either quiz PR.** `buildTargetRows` skips `kind === 'personal'` so a teacher's private target labels — and their linked standards — never cross into the shared PLC aggregate, with a comment explaining why. Recorded as a deliberate decision rather than letting it pass unremarked.
- Cross-PR checks run this review:
  - **One real stacking dependency.** #2962 is based on `codex/quiz-target-results`, not `dev-paul`, so #2961 must land first; rebasing #2962 afterward pulls in the `ad20aee` container-query fix pushed to its base this run. The other four PRs are file-disjoint (`DiceWidget/Settings.tsx`, `eslint.config.js`, `docs/routines/unifier.md`, `MiniApp/components/MiniAppManager.tsx`) — merge order among them is free.
  - **#2966 documents #2964 and #2965**, so it should land after both; if either is reworked, its log entry goes stale on merge. Same standing caveat as prior runs.
  - **#2967 (`scheduled-tasks`) carries this run's own log commit**, so its diff again mixes a widget scaling fix with review-log bookkeeping. Established pattern for this branch.
- Verification standard: the one pushed fix was validated locally before push — `pnpm type-check` ✓, `pnpm lint` ✓ (`--max-warnings 0`, app + functions), `prettier --check` ✓, and the **full unit suite ✓ (957 files, 10,502 tests passed, 5 skipped)**. A filter arg failed to narrow the run, so the whole suite ran; kept as the stronger signal. Findings on the other PRs were verified against head-of-branch source, not the diff alone: probed Tailwind's actual output for `@container`, read `WidgetRegistry.ts` and `DraggableWindow.tsx` to confirm which element is the query container, grepped `useQuizAssignments.ts` for both payload-gating paths, read the `quiz_sessions` rule, counted `resolveGroupQuestions` callers, and grepped `components/plc/` for both the new directory and any violating import.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** Two pushes total — `ad20aee` to `codex/quiz-target-results`, and this log commit to `scheduled-tasks`.
  - **A near-miss worth recording: the scaling fix briefly replaced `﻿` with a literal BOM character** when rewriting `QuizTargetResults.tsx`'s CSV export. Prettier and ESLint both pass on a literal BOM, and the behavior is identical, so **nothing in the verification chain would have caught it** — an invisible character would have been committed into a string literal. Caught by re-reading the rewritten file against the original rather than trusting green checks. Whole-file rewrites carry this risk in a way targeted edits do not.
  - **`gh` CLI still does not exist in this environment** — all PR list/read/diff/review operations used `mcp__github__*` equivalents. The task prompt remains written entirely in `gh` commands; the mapping is mechanical but is re-derived every run. Fifth consecutive run noting this. Worth updating the prompt.
...
- PRs reviewed: **6** (every open PR). All six target `dev-paul`, all six are drafts, and every head branch is a `claude/*`, `nightly/*` or `scheduled-tasks` branch — none is `main`- or `dev-*`-headed, so all were writable under the branch-safety rule.
  - **#3016** feat(quiz-translation): teacher review + student serving (PR3) (`claude/quiz-translation-plan-p2rzdu-pr3-review-and-serving`)
  - **#3015** feat(quiz-translation): PR4 — free-response back-translation (`claude/quiz-translation-plan-p2rzdu-pr4-back-translation`)
  - **#3014** fix(css-scaling): uncap BloomsTaxonomy pyramid tier height/label (`scheduled-tasks`)
  - **#3013** feat(quiz-translation): generation plumbing behind the feature flag (PR2) (`claude/quiz-translation-plan-p2rzdu-pr2-generation-plumbing`)
  - **#3011** docs(unifier): log run 85 — 1 shipped (D3), 4 aligned (`nightly/unifier-log-2026-09-13`)
  - **#3010** fix(a11y): retrofit Countdown event-color picker to radiogroup semantics (`nightly/unify-radiogroup-2026-09-13`)
- Comments processed: **3 total — 2 fixed, 1 explained.** First non-empty Phase 1 in six runs; the streak of zero inline threads is broken by the quiz-translation stack, which draws real `claude[bot]` review comments because it ships Cloud Functions that spend money.
  - **#3015 `utils/backTranslationHash.ts` (r3998852011) — fixed.** `sha256(text + locale)` with no separator. The reviewer proposed adding a delimiter, but that does not actually close it: `answer.locale` is documented as client-asserted and a student controls *both* halves for their own response, so any separator character could itself appear inside the locale. Shipped the length-prefixed form `` `${locale.length}:${locale}${text}` `` instead, which is injective regardless of content. The existing test computes its expected key *through* `backTranslationCacheKey`, so it passes under either encoding — noted on the PR that a `('fooe','s') !== ('foo','es')` assertion is what would actually pin the fix.
  - **#3015 `functions/src/quizTranslation.ts` (r3998851800) — fixed, but not by editing this PR.** Back-translation checked only the per-teacher daily `backCount` yet billed into the *same* `global_translation_{YYYY-MM}` `units`/`outputTokens` fields that gate `translateQuizV1` — so it could never be stopped by the org cap while silently exhausting it for forward quiz translation. **The fix already existed upstream:** PR2's `7cf97c6` introduces `billBackTranslation` writing `backUnits`/`backOutputTokens`, and this branch had simply forked from `48ef895` before it. Merged the PR2 branch in rather than re-implementing — zero file overlap between the two commits, so the merge was clean.
  - **#3013 `functions/src/quizTranslation.ts` (r3998757774) — explained, no fix.** Quota charged only on success, so a caller could burn unlimited paid Vertex calls on input that reliably fails `validateQuizTranslation`. Already addressed by `7cf97c6` (thread was marked outdated): `assertQuotaAvailable` became `reserveTranslationUnit`, which reads and increments `count` + `units` in one `runTransaction` *before* the model call, with tokens billed in a `finally`. Verified the replacement actually closes the hole rather than assuming the commit message — it also fixes the two-callers-at-cap-1 race the original comment did not raise.
  - All three threads replied to and resolved.
- Fixes pushed: **1 branch, 2 commits.**
  - **#3015 `claude/quiz-translation-plan-p2rzdu-pr4-back-translation` → `df32969`** — length-prefixed back-translation cache key, plus a merge of the PR2 branch (`d628ecd`) so back-translation stops spending the org quiz-translation cap.
- Reviews posted: **6** (one structured review per open PR). Findings that came from checking a claim against the codebase rather than restating the diff:
  - **#3010 — checked the retrofit against its own siblings, and found the one line they all have that it doesn't.** `tabIndex={eventColor === color ? 0 : -1}` has no "nothing selected" fallback, and `CountdownAppearanceSettings` renders an `<input type="color">` directly beneath the swatches — so any custom hex outside `TEXT_COLOR_PRESETS` leaves **every** swatch at `tabIndex={-1}` and the group becomes keyboard-unreachable. Strictly worse than the pre-PR state, where the swatches were plain buttons with default tabbability: an a11y PR that introduces a keyboard trap. Confirmed the fallback is universal elsewhere — `IconPicker:51`, `Segmented:26`, `EmojiPicker:26`, `SoundPicker:32`, `TimeTool/settingsFields:51` all use `selected || (selectedIndex < 0 && index === 0)`, and `ColorPresetPicker` uses the equivalent `hasChecked` form. Also checked the *default* case before claiming breadth: `#2d3f89` **is** in `FONT_COLORS`, so a fresh widget is fine and the custom picker is the sole trigger. Separately flagged that `components/common/ColorPresetPicker.tsx` already implements this entire control — radiogroup, roving tabindex with the fallback, custom color input, hex field — so reuse would fix it for free.
  - **#3016 — compared file *contents* across branches, not just the file list, and found the stack isn't a stack.** #3016 is not a descendant of #3013 (`git merge-base --is-ancestor` says no) yet adds its own copies of 26 shared files. Hashed the three biggest: `functions/src/quizTranslation.ts`, `hooks/useQuizTranslations.ts` and `utils/quizDriveService.ts` are **byte-identical to `48ef895`** — PR2 *before* its hardening commit. So whichever merges second conflicts on those files, and resolving toward #3016 would silently revert the reserve-then-spend metering, the server-side feature gate and the input bounds that were already reviewed and fixed on #3013. This is the same fork-point problem as #3015's, but #3015 has now been brought current and #3016 has not.
  - **#3016 — one concrete code defect in the file with no test.** `hooks/useEphemeralAppLanguage.ts` fires `i18n.changeLanguage(locale)` in the effect and `changeLanguage(languageBefore)` in the cleanup, both un-awaited with no generation token: if `locale` changes mid-flight the cleanup's revert can resolve last and strand the shell in the previous language. It is also the only new file in the PR with zero test coverage. The double `restoreKey(storedBefore)` (synchronous *and* in `.finally()`) is dead weight — only the `.finally()` call can observe the detector's write.
  - **#3016 — checked the size-budget coupling before reporting it as a bug, and it held.** `enforceSessionSizeBudget` can only drop locales present in `targetedCountByLocale`, which looked like a hole (an attached-but-untargeted locale would be undroppable). Read the caller: `useQuizAssignments.ts:981` derives the attached set from `Object.keys(targetedLocaleCountByCode)`, so the two match by construction. Downgraded to a coupling note rather than reporting a defect that cannot fire.
  - **#3014 — checked whether the fix reaches the state the original reviewer was worried about.** The author answered an earlier thread by adding `blooms-taxonomy: { w: 280, h: 300 }` to `WIDGET_MIN_SIZE_OVERRIDES` and claimed "no clipping risk at every legal size." Verified the arithmetic (`13cqmin ≥ 24px` needs `cqmin ≥ 184.6px`; a 280×300 window clears it after chrome) — but `minW`/`minH` are read at `DraggableWindow.tsx:1501-1502`, which is **inside `handleResizeStart`** (`:1421`), while render uses `widget.w`/`widget.h` raw at `:2227`/`:2232`. The override gates future resizes, not stored state: a blooms widget already saved below 280×300 — legal before this PR at the 150×100 default — loads at its stored size and clips. Narrow (default is 450×550) but real, and it is precisely the residual of the thread that was marked resolved.
  - **#3013 — credited the hardening, then looked for what it left behind.** `billTranslationTokens` writes `outputTokens` onto the daily doc as well as the monthly one, but nothing reads the daily field — `reserveTranslationUnit` checks only `monthly.outputTokens`. Also noted that one reserved unit funds two Gemini calls (generate + repair retry), which is defensible as "a unit is one request" but is worth stating next to the reservation. Checked App Check before flagging its absence and did **not** flag it: `enforceAppCheck` appears nowhere in `functions/src/`, so this is house convention, not a regression.
  - **#3011 — read the log against the PR it describes.** The `SHIPPED (run 85)` row asserts the Countdown conversion matches "the `WorkSymbols`/`DiceWidget`/`NextUp`/`MathToolInstance`/`TimeTool` reference shape." Those references all carry the roving-tabindex fallback; #3010 as pushed does not — so the claim is currently false, and the right fix is to correct #3010, not to soften the log. Also noted the ordering coupling: the BACKLOG rewrite removes Countdown from the candidate list that drives future runs, so if #3010 is dropped the memory file is silently wrong. Spot-checked D3-E30/E31/E32 and all three reclassifications are correct — destructive-action pairs and list/text repeaters, none with exclusive-choice semantics.
- Cross-PR checks run this review:
  - **The quiz-translation stack is the whole story tonight.** #3013/#3015/#3016 share 26 files and forked from each other at different points. Correct merge order is #3013 → #3015 → #3016, with #3016 rebased first. #3015 was brought current during Phase 1; #3016 was not, and that is its review's merge blocker.
  - **The two nightly PRs are coupled the other way.** #3011's log records the outcome #3010 delivers. Merge together or #3010 first.
  - **#3014 and #3011 do not overlap** despite both touching `docs/` — `docs/scheduled-tasks/*` vs `docs/routines/unifier.md`.
  - **Regression surface outside the stack is narrow.** Nothing touches `context/DashboardContext.tsx` or `components/widgets/WidgetRegistry.ts`. `types.ts` changes are additive across all three stack PRs with no new `WidgetType`, so the `WidgetConfig` union and `ConfigForWidget` checks are vacuous. `functions/src/index.ts` gains only the additive `translateQuizV1`/`translateResponseV1` export. `firestore.rules` is byte-identical across the three stack branches — one new `match /admin_settings/quiz_translation` block, correctly present for the new collection.
  - **Test-count baseline needed no bump** — `scripts/test-count-baseline.json` is a floor and its own comment says growth is always allowed. Checked rather than assumed, because the stack adds ~1,900 lines of tests.
- Verification standard: the one pushed branch was gated on the full pre-push set — `pnpm run type-check:all` ✓, `pnpm run lint` (app + functions, `--max-warnings 0`) ✓, `pnpm run format:check` ✓, the six affected root suites (30 tests) ✓, and the entire `functions/` suite (61 files, 1,360 tests) ✓. The merge that carried the quota fix was verified as a merge, not just as a diff: confirmed zero file overlap between `7cf97c6` and `d9972ec` beforehand, then re-read the merged `translateResponse` to confirm it actually calls `billBackTranslation` rather than trusting the commit message.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** The one code push went to a `claude/*` head branch.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt. Same tradeoff as the last two runs and worth re-stating: `scheduled-tasks` heads open PR **#3014**, which carries a code change (`DraggableWindow.tsx`, `Pyramid.tsx`), so this journal commit rides along on a code PR — and this run also *reviewed* #3014, so the review and its subject share a branch. Appending at end-of-file keeps it conflict-free.
  - **Phase 1 came back to life, and the reason is worth recording.** Five consecutive empty runs, then three threads in one night — all on Cloud Functions that spend metered Vertex tokens. Review bots find quota and billing bugs because those are the ones where reading the code end-to-end pays off. **Both #3015 findings were fixable, but only one was fixable by editing #3015**: the quota bug's fix already existed one branch upstream. Checking whether a finding is already solved elsewhere in the stack before writing code is the cheaper first move, and it is not the default instinct.
  - **A pattern across #3010 and #3016, at two different altitudes.** #3010 hand-rolls a control that `ColorPresetPicker` already implements correctly, and loses the one line that makes it safe. #3016 hand-carries a foundation that #3013 already owns, and loses the hardening commit. Same failure: a copy that forks from the canonical thing and silently drifts behind it. The unifier routine catches the first class; nothing catches the second, because stacked-PR drift is invisible to a file-level sweep — it needs a content hash across branches.
  - **`/mnt/skills/user/` still does not exist in this environment** (`/mnt/skills/` is absent entirely) — fourth run recording this. Repo-local equivalents are at `.claude/skills/{new-widget,admin-widget-config}/`; widget and admin-config standards came from those plus the repo `CLAUDE.md`. Low cost again: no PR added a widget, and the only widget-surface changes were a back-face settings panel (#3010, where scaling rules explicitly do not apply) and one front-face `clamp()` pass (#3014).
  - **Node version mismatch present and benign again:** container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation. All checks and 1,390 tests passed under it regardless. Recorded so a future run seeing a real failure here does not re-diagnose the warning.
  - Tooling: GitHub via the MCP server (**no `gh` CLI in this environment**, contrary to the task prompt's Pre-Task step — `gh auth status` is not runnable here); all PR list/read/reply/resolve/review operations used `mcp__github__*` equivalents. Diffs were read locally via `git diff origin/dev-paul...origin/<head>` after fetching, and cross-branch comparison used `git show <ref>:<path> | md5sum`, which is what surfaced the #3016 stale-foundation finding — an API-level diff read would not have shown it.
  - **The append-contention recommendation still stands, unactioned.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed since 08-26; two entries have been lost to the current single-file mechanism (08-29, restored by #2647; 09-03, restored by #2797). No contention tonight, but the file is now past 4,000 lines.

## 2026-09-14

- PRs reviewed: **12** (every open PR) — the largest batch this log has recorded. All twelve target `dev-paul`; eleven are drafts and **#3030 is the only non-draft**. Head branches are `codex/*`, `nightly/*` and `scheduled-tasks` — none is `main`- or `dev-*`-headed, so all were writable under the branch-safety rule.
  - **#3040** docs(debugger): log run 70 — 5 shipped (1 security) (`nightly/debugger-log-2026-09-14`)
  - **#3039** fix(security): require verified email in `migratePlcs`/`adminAnalytics` (`nightly/build-tooling-2026-09-14`)
  - **#3038** fix(admin): canonicalize building id in `ClassesConfigurationPanel` (`nightly/admin-config-2026-09-14`)
  - **#3037** fix(dashboard): `stableStringify` for undo-history invalidation (`nightly/state-data-2026-09-14`)
  - **#3036** fix(overflow-menu): thread widget host through context, not DOM `closest()` (`nightly/dashboard-layout-2026-09-14`)
  - **#3035** fix(settings-drawer): dedupe `field.section` headings per group (`nightly/widgets-2026-09-14`)
  - **#3034** fix(css-scaling): convert `VideoActivityManager` front face to cqmin (`scheduled-tasks`)
  - **#3033** docs(unifier): log run 86 — 2 shipped (D3, D4) (`nightly/unifier-log-2026-09-14`)
  - **#3032** fix(quiz-translation): `@/tests` alias for fixture import (D4) (`nightly/unify-d4-import-paths-2026-09-14`)
  - **#3031** fix(a11y): Checklist "List Source" → radiogroup semantics (`nightly/unify-d3-settings-labels-2026-09-14`)
  - **#3030** feat(settings): migrate five more widgets to sidebar drawer (`codex/migrate-next-five-widgets-to-sidebar`)
  - **#3028** feat(settings): migrate next five widgets to sidebar drawer (`codex/identify-and-migrate-widgets-to-sidebar`)
- Comments processed: **3 — 1 fixed, 2 explained.** Ten of twelve PRs returned zero inline review threads. All open threads were on **#3028**; **#3030** carried its finding as a top-level bot comment.
  - **#3028 `Checklist/settings.schema.ts` (r4001599356) and `Weather/settings.schema.ts` (r4001599540) — explained, no fix.** Both say the same true thing: the schema registers the widget as drawer-owned while one `type: 'custom'` field wraps the entire legacy panel, so `settingsFilter.ts` indexes one label for what is 11 (checklist) and 15 (weather) settings. Decomposing those into real fields *is* the migration work, and `WIDGET_SETTINGS_DRAWER.md`'s D26 gate names "count of `schema-gap` Custom fields in the five schemas" as an explicit product-owner checkpoint — auto-rewriting it during review would pre-empt the gate. Replied on both with the two sub-points that *are* mechanically fixable without a scope call: the `// schema-gap: compositeControl` comments don't name a replacement field type (the plan requires it), and the PR description reads as a field-level migration when this wave is a registration-only move.
  - **#3030 top-level bot comment (5657965251) — misattributed, fixed on the correct PR instead.** The comment lists four stale `docs/nexus.md` pointers (`random/RandomSettings.tsx:39-57`, three `Checklist/Settings.tsx` refs) as a gap in #3030. Checked the branch rather than the claim: **both files still exist on #3030's head** — those renames belong to #3028. #3030 correctly updates the two `SoundWidget/Settings.tsx` pointers it actually invalidates, which is exactly right scope. The four genuinely-dead pointers were fixed on #3028's branch, where the renames live. Recorded on both reviews so the correction is visible from either side.
- Fixes pushed: **1 branch, 1 commit.**
  - **#3028 `codex/identify-and-migrate-widgets-to-sidebar` → `c6c175f`** — `docs/nexus.md`: `random/RandomSettings.tsx` → `random/SchemaControls.tsx` and three `Checklist/Settings.tsx` → `Checklist/SchemaControls.tsx`. Confirmed both renames are 100%-similarity (`git diff -M --summary`) before keeping the cited line ranges, then spot-read `SchemaControls.tsx:110` and `:146` to see `importFromRoutine`/`importFromTextWidget` still land inside the quoted spans. `docs/nexus.md` exists to give working `file:line` pointers, so dangling entries there are a real defect, not cosmetics.
- Reviews posted: **12** (one structured review per open PR). Findings that came from checking a claim against the codebase rather than restating the diff:
  - **#3028/#3031 — found a merge conflict neither PR knows about, by actually merging them.** Replayed both onto `dev-paul` in sequence: #3028 then #3031 conflicts on `tests/components/settings/__snapshots__/legacySnapshots.test.tsx.snap`. #3028 *deletes* the `'checklist'` snapshot entry (the widget stops being legacy-rendered); #3031 *updates* that same entry with its new ARIA attributes. The useful half is what did **not** conflict: git follows the `Checklist/Settings.tsx` → `SchemaControls.tsx` rename, so #3031's radiogroup change lands in the renamed file automatically. Conflict is snapshot-only and the resolution is to take #3028's deletion. Also verified #3030 and #3035 merge clean on top of #3028 despite all three touching `WidgetRegistry.ts`, `locales/en.json` and `docs/nexus.md` — disjoint regions, so merge order there is unconstrained.
  - **#3031 — the merge check has a second-order consequence worth naming.** The a11y behavior is covered *only* through the legacy snapshot #3028 is scheduled to delete. Land both and the radiogroup conversion ends up with zero test coverage, silently. Flagged on #3031 rather than #3028, since it's #3031's behavior that goes unguarded.
  - **#3039 — took the fix's own reasoning and applied it to the rest of the codebase.** The gate is correctly placed (before `assertCallerIsAdmin`, so an unverified caller never reaches the `/admins/{email}` read) and `!== true` correctly rejects a *missing* claim rather than treating `undefined` as a pass. Then checked coverage: 15 files in `functions/src/` read a caller email, 10 have the gate after this PR. **`quizTranslation.ts:1027` and `:1066` pass `request.auth.token.email` straight into `assertQuizTranslationFeature(...)` — a feature-authorization decision on an unverified claim, the same class of bug this PR fixes.** Also noted the check is now copy-pasted in five files; a shared `assertVerifiedCallerEmail(auth)` is what would have stopped these two being missed in the first place.
  - **#3034 — verified the container context exists before accepting the cqmin conversion.** `cqmin` silently resolves against the viewport when no `container-type` ancestor exists, which is how these conversions usually break. Confirmed `video-activity` is `skipScaling: true` in `WidgetRegistry.ts`; confirmed `VideoActivityManager` has exactly one consumer (`VideoActivityWidget/Widget.tsx:536`) rather than being shared with a page-level view; confirmed `VideoActivityPreviewPaneContent` renders inline and `LibraryPreviewPane` has no `createPortal`, so the preview pane stays inside the widget's query container. All three hold, so the conversion is sound — but none of that is visible in the diff.
  - **#3037 — checked whether the new call is consistent with the file rather than novel.** `stableStringify` was already imported at `:53` and already defends the same Firestore key-order echo at `:360-361`, `:2065` and `:2142-2143`, so this brings the undo check onto an existing rail. Flagged the cost honestly: recursive key sorting now runs over the active board's whole `widgets` array per snapshot, and this file already has a documented fast path for exactly that tradeoff at `:2153`. Non-blocking — the change can only *preserve* history where the old code cleared it, never the reverse, which is the right direction for a data-loss bug.
  - **#3032 — ran it instead of reasoning about it.** The one-line `@/tests` alias switch hinges entirely on whether the root alias resolves for JSON under Vitest. Ran the suite: 10/10 pass. Cheaper than reading three config files.
  - **#3036 — checked the two things that make a context-threading refactor go wrong, and both are right.** The provider passes the ref *object* (`value={windowRef}`), not `.current` — passing the latter would change identity every render and re-run every consumer effect. Effect deps `[open, hostRefFromContext, anchorRef]` are all stable. Also noted `SettingsPanel` needs its own provider because settings render outside `DraggableWindow`'s `{children}` subtree, which the PR gets right, and that the `.closest()` fallback is retained so un-migrated call sites don't regress.
  - **#3035 — named what the dedupe trades away.** Per-group deduping means a *non-contiguous repeat* of a section renders its later fields with no heading, orphaned under the preceding section's heading. Old code printed a duplicate; new code produces an orphan. Both are wrong for interleaved sections — grouping fields by section would fix ordering rather than just the heading. Probably academic, but it's the case the added test doesn't cover. Also flagged the IIFE-with-mutable-`Set` in JSX as a readability nit (it is correct — `.map` is synchronous and the `Set` is per-render).
  - **#3038 — the diff is three lines; the blast radius isn't.** `handleUpdate` spreads the *canonicalized* record into what it persists, so the first edit to any one building rewrites every key in the org's `buildingDefaults` — including buildings the admin didn't touch. Almost certainly the intended one-shot migration, but it is not visible from the diff and deserves to be deliberate. Same shape as the #2833 finding from 09-04, on a sibling panel.
- Cross-PR checks run this review:
  - **One merge-order constraint across twelve PRs:** #3028 ↔ #3031 (snapshot conflict, above). Everything else is independent — verified by replay, not by inspecting file lists.
  - **#3028 and #3030 are the same wave asking the same unanswered question.** Both use whole-panel `custom` shims; the D26 gate should be answered once for both rather than separately per PR.
  - **#3033 and #3040 are the logs for work in this same batch** (#3031/#3032 and #3039 respectively). Docs-only, no overlap with each other or with `docs/scheduled-tasks/*`.
  - Regression surface is narrow. `context/DashboardContext.tsx` is touched only by #3037, in the snapshot handler's history-invalidation branch — not `getAdminBuildingConfig` or the config merge pipeline. `WidgetRegistry.ts` is touched by #3028/#3030; all three maps verified consistent, each widget registered in exactly one place. `types.ts` untouched (no new `WidgetType`, so the `WidgetConfig`/`ConfigForWidget` checks are vacuous). `functions/src/index.ts` gains no signature changes — #3039 only adds guards inside two existing handlers.
  - Base freshness: all twelve branches sit on `dev-paul`'s head `e663e56`; `dev-paul` did not move during the run.
- Verification standard: the one pushed commit was doc-only, so the meaningful gates were `pnpm run type-check` ✓ (clean on #3028's branch, confirming the branch is sound independent of the doc edit) and `npx prettier --check docs/nexus.md` ✓; `lint-staged` re-ran Prettier at commit time. Separately ran `utils/quizTranslationHash.test.ts` (10/10 ✓) to verify #3032's claim directly rather than reviewing it on inspection.
- Notes:
  - Branch safety: **no push to `main`, and no push to any `dev-*` branch this run.** The one code push went to a `codex/*` head branch.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt — which again heads an open PR (**#3034**) carrying a code change, and which this run also reviewed. Fourth run recording the same tradeoff. Appended at end-of-file to stay conflict-free.
  - **The most valuable check tonight was the cheapest: actually merging the PRs.** The #3028/#3031 conflict is invisible to per-PR review — both are individually clean against `dev-paul`, and a file-list comparison misses it because the conflict is in a snapshot neither PR's description mentions. Two `git merge` replays found it in under a minute. Worth making a standing step: with twelve PRs off one base, pairwise-clean is not the same as jointly-clean.
  - **Second time this run that a finding was true but pointed at the wrong PR** (#3030's nexus.md comment). Both times the fix was to check the branch rather than the claim. A review bot reading a diff cannot see which branch owns a rename; a reviewer with a checkout can, in one `ls`.
  - **`/mnt/skills/user/` still does not exist in this environment** — fifth consecutive run recording this. Repo-local equivalents are at `.claude/skills/{new-widget,admin-widget-config}/`; widget and admin-config standards came from those plus the repo `CLAUDE.md`. Cost was low again: the only front-face widget change was #3034's cqmin pass, and #3031 is a back-face settings panel where scaling rules explicitly don't apply.
  - **Node version mismatch present and benign again:** container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation. `type-check` and the Vitest run passed under it regardless.
  - Tooling: GitHub via the MCP server (**no `gh` CLI in this environment**, contrary to the task prompt's Pre-Task step). Diffs read locally via `git diff e663e56..origin/<head>` after fetching; conflict detection via `git merge --no-commit` replays onto a scratch branch.
  - **The append-contention recommendation still stands, unactioned.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed since 08-26; two entries have been lost to the current single-file mechanism. No contention tonight, but the file is now past 4,000 lines and this was the largest entry yet.

## 2026-09-15

- PRs reviewed: 10 — all open PRs in the repo.
  - **#3060** docs(debugger): log run 71 — 5 shipped (1 security), zero clean [`nightly/debugger-log-2026-09-15`]
  - **#3059** fix(settings): scope the settings-close signal per widget, not globally [`nightly/dashboard-layout-2026-09-15`]
  - **#3058** fix(admin): canonicalize building id in TrafficLightConfigurationPanel [`nightly/admin-config-2026-09-15`]
  - **#3057** fix(deps): patch browserslist OOM/crash advisories via pnpm override [`scheduled-tasks`]
  - **#3056** fix(quiz): flag stimulus read-aloud edits as dirty in editor [`nightly/widgets-2026-09-15`]
  - **#3055** fix(assign): stable-stringify override comparison in target payload diff [`nightly/state-data-2026-09-15`]
  - **#3054** fix(functions): stop unverified email from bypassing OCR admin quota [`nightly/build-tooling-2026-09-15`]
  - **#3053** docs(unifier): log run 87 — 1 shipped (D3), 4 aligned [`nightly/unifier-log-2026-09-15`]
  - **#3052** fix(a11y): retrofit MusicWidget Source/Layout pickers to radiogroup semantics [`nightly/unify-d3-settings-labels-2026-09-15`]
  - **#3051** Consolidate settings updates and fix quiz translation issues [`dev-paul` → `main`]
- Comments processed: **1 total — 1 fixed, 0 explained.** Exactly one unresolved review thread existed across all ten PRs: a CodeQL prototype-pollution alert (alert 93) on #3051. The nine nightly PRs each carry a `claude[bot]` summary comment with no change requests, and no inline review threads at all.
- Fixes pushed: **1**
  - **#3051 / `dev-paul` / 6e79dba** — guarded the test Firestore double's `deepMerge` in `functions/src/quizReadAloudLocale.test.ts` against prototype pollution (skip `__proto__`/`constructor`/`prototype` before writing). Verification before push: functions `tsc --noEmit` ✓, ESLint under the functions config ✓ (the root config ignores `functions/`, so linting the file from the root reports only an "ignored" warning — run it via `functions/`'s own config), Prettier ✓, `quizReadAloudLocale.test.ts` 22/22 ✓. Post-push CI on the head went fully green: 3 unit shards, functions + count guard, lint app/functions/format, type-check, Firestore rules, build, E2E, CodeQL.
- Reviews posted: **10** (one per PR).
- What the reviews actually checked, beyond restating the diffs:
  - **#3051 — verified the two invariants that would matter if they were wrong, rather than asserting them.** (1) `QuestionTranslation` now carries `answer` (the translated FIB answer key) and the quiz session doc is world-readable to students, so the leak path is real if anything spreads it; `toPublicQuestion` (`hooks/useQuizSession.ts:387`) builds each locale entry as an explicit allowlist — `{ text: tr.text }`, never a spread — so it fails closed when a future field is added. (2) All three registry maps are consistent across the ten-widget settings migration, and `settings.schema.next-batch.test.ts` now asserts that exclusivity per widget, so the invariant is test-enforced rather than review-enforced.
  - **#3051 — chased the one migration removal that looked like a lost control.** `sound` drops its `WIDGET_APPEARANCE_COMPONENTS` entry *and* declares no `styleKeys`, which reads as a dropped appearance panel. It isn't: the old `SoundAppearanceSettings` was only the visual-mode picker, now a `segmented` field in the schema's `display` group, and `settingsFilter.ts` renders a widget's `display` group into the Style tab — same place for the teacher. Checked rather than assumed, because "the schema covers it" was the claim most likely to be false.
  - **#3051 — scripted the rules check instead of eyeballing it.** Extracted every collection path introduced in the diff and tested each against `firestore.rules`: zero unmatched. `firestore.rules` is unchanged because the feature adds *fields* to existing docs, not new collections — which is the answer, not an omission.
  - **#3054 — verified the degrade path, which is where this class of fix usually breaks.** Nulling an unverified email only helps if downstream treats `null` as "not admin" rather than throwing. `chargeOcrQuota` (`quizStimulusText.ts:344`) computes `isAdmin = email ? … : false`, and `isGlobalFeatureGranted` (`quizMediaArchive.ts:584`) skips the lookup on an empty email — so unverified teachers keep working OCR at normal quota. Also noted the `buildDefaultExtractDeps` half reads `emailVerified` off the Admin SDK record, not the client token, so it is authoritative.
  - **#3054 / #3060 — the bug class is not closed, and said so on both PRs.** #3060's log names three confirmed-real, unfixed siblings: `studentAssignmentTargets.ts` (org / test-class authority — the only one with a cross-account authorization consequence rather than a quota bypass), `quizReadAloud.ts::buildDefaultDeps().isFeatureGranted`, and `aiGeneration.ts::generateWithAI` (~412). Flagged so they are tracked work rather than a log row.
  - **#3059 — named the semantic change the title doesn't mention.** `consumeSettingsJustClosed` now clears the signal *only on a fresh match*, where it previously cleared unconditionally. That is correct under scoping (widget B must not eat widget A's pending signal), but it is a real behavior change, not a mechanical id-threading pass. Confirmed by grep that no zero-arg call site survives anywhere on the branch.
  - **#3052 — checked the roving tabindex can't strand the group.** `const { layout = 'default', source = 'curated' } = config` guarantees exactly one option is active, so exactly one button holds `tabIndex={0}` even on a freshly added widget. Also flagged that `handleRadioGroupKeyDown` selects on arrow (ARIA selection-follows-focus), so arrowing across `Source` *commits* a switch to `personal` in passing and fires the `syncWithTimeTool` reset — specified behavior, but a genuine change from focus-then-click.
  - **#3052 — the shared-handler refactor is the part with no test.** Coverage is a snapshot update only; nothing asserts the keyboard path fires the `syncWithTimeTool` side-effect, which is exactly what hoisting `selectSource` was for.
  - **#3057 — the risk in a browserslist bump isn't the API.** `baseline-browser-mapping` 2.9.15 → 2.11.23 can shift what `defaults`-style queries expand to, changing the transpilation target. A green `build` proves it compiles, not that the output still meets the browser floor claimed in `CLAUDE.md`. Cheap to confirm once; not something CI answers.
  - **#3051 — one release-sequencing call left to a human.** A user-visible settings-UI rewrite for ten widgets ships in the same deploy as a large new feature. Both are individually well-tested, but "Weather's settings look wrong" and "raise-hand didn't gate" would arrive together, making rollback expensive. Not a defect — a decision.
  - **#3051 — named the coverage trade rather than letting it pass as cleanup.** 13 legacy settings snapshots were deleted (79 → 66 entries) for the migrated widgets. Expected, since those panels are gone, but their *rendered markup* is no longer pinned; what replaces it validates the schema, not the output. Defensible given the field renderers have their own tests — worth being deliberate about.
- Cross-PR checks run this review:
  - **Nine of the ten PRs share one base** (`dev-paul` at `b048f6b`) and are file-disjoint: `docs/routines/debugger.md`, `docs/routines/unifier.md`, `settingsCloseSignal.ts` + `SettingsDrawerHost.tsx` + `DraggableWindow.tsx`, `TrafficLightConfigurationPanel.tsx`, `package.json`/lockfiles + `docs/scheduled-tasks/*`, `QuizEditorModal.tsx`, `studentTargetRef.ts`, `quizStimulusText.ts`, `MusicWidget/Settings.tsx`. No merge-order constraint found.
  - **#3051 is the odd one out and everything else lands inside it eventually** — it is `dev-paul` → `main`, so the nine nightly PRs are its future contents, not its peers. Reviewing it as a snapshot of a moving base is the right frame; it moved once during this run (my own fix).
  - **#3055 and #3051 contain the same `stableStringify` fix in two places** — #3055 in `studentTargetRef.ts`, #3051 already carrying the `DashboardContext.tsx` one from #3037. Same bug class, no conflict.
- Verification standard: the one pushed commit was a test-file change, gated on functions `type-check` ✓, functions-config ESLint ✓, Prettier ✓, and the affected suite 22/22 ✓ before pushing; post-push CI confirmed green on the head independently.
- Notes:
  - Branch safety: **no push to `main`.** One code push went to `dev-paul`, which the task prompt permits specifically when a PR merging `dev-paul` into `main` carries review comments — #3051 did (the CodeQL thread). No other `dev-*` branch was touched.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt — which again heads an open PR (**#3057**) carrying a code change, and which this run also reviewed. Fifth run recording the same tradeoff. Appended at end-of-file to stay conflict-free.
  - **A near-miss worth recording: `git checkout <branch> -- .` to inspect a branch overwrites the working tree of the branch you are standing on.** Used it to grep call sites on #3059's branch while checked out on `dev-paul`, which reverted the just-committed fix in the tree (the commit itself was safe and already pushed). Recovered with `git reset --hard HEAD`. Use `git show <ref>:<path>` or `git diff <base>...<ref>` to read another branch — never a path-checkout into a tree you care about.
  - **`/mnt/skills/user/` still does not exist in this environment** — sixth consecutive run recording this. Repo-local equivalents are at `.claude/skills/{new-widget,admin-widget-config}/`; widget and admin-config standards came from those plus the repo `CLAUDE.md`. Cost was low again: the only widget-settings work was back-face panels, where container-query scaling rules explicitly don't apply.
  - **Node version mismatch present and benign again:** container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation. The functions type-check, ESLint, Prettier and Vitest runs all passed under it regardless.
  - **`pnpm run lint` from the repo root does not lint `functions/`** — the root ESLint config ignores that directory, so linting a functions file from the root returns an "ignored by pattern" warning that `--max-warnings 0` then reports as a failure. Looks like a lint error on a clean file. Use `functions/`'s own script (`node ../node_modules/eslint/bin/eslint.js …` from `functions/`). Worth knowing before trusting a root-lint result on a functions change.
  - Tooling: GitHub via the MCP server (**no `gh` CLI in this environment**, contrary to the task prompt's Pre-Task step — every `gh` command in the prompt has an MCP equivalent). `pull_request_read` with `get_files` on #3051 exceeded the tool's output cap; local `git diff --stat origin/main...origin/dev-paul` after fetching is both cheaper and complete, and is the better default for any large PR.
  - **The append-contention recommendation still stands, unactioned.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed since 08-26; two entries have been lost to the current single-file mechanism. No contention tonight; the file is now past 4,000 lines.

## 2026-09-16

- PRs reviewed: 3 — all open PRs in the repo.
  - **#3069** fix(css-scaling): convert QuizManager.tsx front face to cqmin (4th of 4) [`scheduled-tasks`]
  - **#3068** docs(unifier): log run 88 — 1 shipped (D3), 3 aligned, 1 caught false positive (D4) [`nightly/unifier-log-2026-09-16`]
  - **#3067** fix(a11y): exclusive-choice radiogroup for SyntaxFramer Mode/Alignment (D3) [`nightly/unify-d3-settings-labels-2026-09-16`]
- Comments processed: 2 total — 0 fixed, 0 explained, 2 correctly skipped.
  - Both were `claude[bot]` LGTM summaries (#3069, #3067) reporting no findings. Neither is a change request, so neither meets the prompt's "fix is needed" bar, and neither warrants a reply — replying "no fix needed" to a comment that already says "no issues found" is noise on the PR. Zero inline review threads existed on any of the three PRs (`get_review_comments` returned `totalCount: 0` for all three). Phase 1 was genuinely a no-op tonight, not a skipped step.
- Fixes pushed: none. No PR carried an actionable finding.
- Reviews posted: 3 (#3069, #3068, #3067).
- Findings worth carrying forward:
  - **#3069 — verified the container-query preconditions rather than trusting the PR description.** The conversion is only valid if this subtree really sits inside a `container-type: size` container. Confirmed `quiz` is registered `skipScaling: true` (`WidgetRegistry.ts:767`); confirmed `LibraryPreviewPane` renders as a right-side *sibling of the grid*, not a portal; confirmed `AssignmentArchiveCard`'s only `createPortal` (`:175`) is the "more actions" dropdown, which `QuizArchiveRow`'s converted chips do not feed. All three conversions are genuinely in-container.
  - **#3069 — completeness checked by grep, not by assertion.** The only hardcoded size classes surviving in the file are at `:2268`, `:2276`, `:2302`, `:2318` and `:3020`–`:3114`, and every one sits inside `AssignModal`'s `extraSlot`/`plcSlot` — the documented intentional exclusion. Nothing inside the container was missed, so "4th of 4" is accurate.
  - **#3069 — a 1px cap reduction that is precedent, not drift.** `text-xs` (12px) maps to `min(11px, 4cqmin)`, not `min(12px, 4.5cqmin)`. Checked before flagging it: `min(11px, 4cqmin)` already has 35 occurrences under `components/widgets/`, including in the three sibling managers this PR matches. Consistent — worth recording so a future audit doesn't re-open it.
  - **#3069 — the journal's stated `cqmin` rationale is factually wrong, though the decision it justifies is right.** `docs/scheduled-tasks/css-scaling.md` says "cqmin computes to zero outside a CQ container." Per CSS Containment Level 3, container query units fall back to the **small viewport size** when no eligible container exists, not zero. Leaving the portaled `AssignModal` content unconverted is still correct (viewport-relative sizing in a modal is as wrong as zero), so no code changes. But that sentence is now repeated across all four journal entries in this series and will mislead whoever reasons from it next. Flagged as docs-only, to ride a later commit.
  - **#3067 — checked the one thing that silently breaks this pattern.** `handleRadioGroupKeyDown` (`radioGroupKeyNav.ts:24-40`) picks the next option by **DOM index** from `querySelectorAll('[role="radio"], [role="tab"]')` and then indexes the caller's `options` array by that same position — so a mismatch between array order and render order selects the wrong value with no type error to catch it. Both groups line up (`['text','math']` vs Text→Math; `['left','center']` vs AlignLeft→AlignCenter). Correct, but this is the failure mode to check first on every future file in this retrofit series.
  - **#3067 — settings panels are exempt from cqmin, so the retained `w-4 h-4`/`text-sm` are correct.** Worth stating explicitly because a scaling-focused audit reading this file in isolation would flag them; CLAUDE.md exempts back-face panels by design.
  - **#3067 — constant placement diverges cosmetically from its own reference.** `MODE_OPTIONS`/`ALIGNMENT_OPTIONS` sit in the component body; `MusicWidget/Settings.tsx:23,92` hoists the equivalents to module scope. No functional difference (never a memo/effect dependency) — noted as a nit only.
  - **#3068 — the D4-E3 addition is the substantive content, and it is the right fix.** It promotes a precedent affirmed across runs 69/77/78/81/85 from buried backlog prose to a numbered exception, after a subagent misread the ambiguous Canon text, converted three blessed files, and widened a `no-restricted-imports` regex that would have locked the wrong rule in repo-wide. Discarded pre-push; nothing to revert. The generalizable rule — promote a pattern to a numbered `D#-E#` row once it has been called acceptable across 3+ runs, because a grep-and-convert agent trusts what it can pattern-match over prose it must go find — applies beyond D4.
- Cross-PR checks run this review:
  - **All three PRs share one base (`dev-paul`) and are fully file-disjoint:** `QuizManager.tsx` + `docs/scheduled-tasks/*`; `docs/routines/unifier.md`; `SyntaxFramer/Settings.tsx` + `legacySnapshots.test.tsx.snap`. No merge-order constraint, no conflict risk.
  - **#3068 narrates #3067 as shipped**, so merging #3068 first leaves its log row briefly describing a change not yet on the base. Cosmetic only — flagged on the PR so a stale-looking row isn't mistaken for an error.
  - **No PR targets `main`** tonight — all three are `→ dev-paul`. Unlike the 09-15 run there was no `dev-paul` → `main` PR open, so no push to `dev-paul` was needed or made.
- Verification standard: no code was pushed, so no pre-push gate applied. Each PR's CI was read directly instead of assumed — #3069 (`2a15d83`) and #3067 (`1264601`) are green across all 14 checks (type-check, lint-app, lint-functions, format, 3 unit shards, functions + count guard, rules, build, E2E, Claude Code Review). Every review claim above was verified against the local tree or the branch diff, not inferred from the PR description.
- Notes:
  - Branch safety: **no push to `main`**, and no `dev-*` branch touched. Only `scheduled-tasks` was written to, for this log.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt — which again heads an open PR (**#3069**) carrying a code change that this run also reviewed. Sixth consecutive run recording the same tradeoff. Appended at end-of-file to stay conflict-free. Note this commit re-triggers CI on #3069.
  - **`gh` CLI is absent from this environment**, contrary to the prompt's Pre-Task step; GitHub access was entirely via the MCP server. Every `gh` command in the prompt has an MCP equivalent.
  - **`pull_request_read` with `get_files` blew the output cap on #3069** (65,818 chars, one line) — the same failure the 09-15 run hit on #3051. Local `git fetch` + `git diff origin/dev-paul...origin/<head>` is cheaper and complete. Recommend making the local-diff path the default for any PR touching a large file, not the fallback.
  - **`/mnt/skills/user/` still does not exist** — seventh consecutive run recording this. Repo-local equivalents are at `.claude/skills/{new-widget,admin-widget-config}/`; widget and admin-config standards came from those plus the repo `CLAUDE.md`. Cost was low again: no PR added a new widget or admin config panel.
  ...
  - **The append-contention recommendation still stands, unactioned.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed since 08-26; two entries have been lost to the current single-file mechanism. No contention tonight, but the file is now past 4,100 lines and ~750 KB — large enough that reading it for context is itself becoming a cost.

## 2026-09-19

- PRs reviewed: 10 — all open PRs in the repo. Every one targets `dev-paul`; none targets `main`, and no head branch is `main` or `dev-*`.
  - **#3167** docs(debugger): log nightly run 75 — 5 shipped (1 security), 0 collisions [`nightly/debugger-log-2026-09-19`]
  - **#3166** fix(projects): clamp individual score overrides to the rubric max [`nightly/widgets-2026-09-19`]
  - **#3165** fix(admin): canonicalize building ids in VideoActivityConfigurationModal [`nightly/admin-config-2026-09-19`]
  - **#3164** fix(ai-overlay): catch Escape at the document level, not via div onKeyDown [`nightly/dashboard-layout-2026-09-19`]
  - **#3163** fix(projects): clear rubric/rubricMaxPoints/dueAt on ensureRun when removed [`nightly/state-data-2026-09-19`]
  - **#3162** Saturday scheduled tasks — journal audits + widget-registry fix [`scheduled-tasks`]
  - **#3161** fix(security): stop building_admin from bypassing cross-org authz via /admins/{email} [`nightly/build-tooling-2026-09-19`]
  - **#3160** docs(unifier): run 91 log [`nightly/unifier-log-2026-09-19`]
  - **#3159** D5: route CatalystConfigurationModal messages through addToast [`nightly/unify-d5-toast-architecture-2026-09-19`]
  - **#3158** D3: radiogroup + roving tabindex for MaterialsWidget Typography/Title Color [`nightly/unify-d3-settings-labels-2026-09-19`]
- Comments processed: 9 total — 0 fixed, 0 explained, 9 correctly skipped. **Zero inline review threads existed on any of the 10 PRs** (`get_review_comments` returned empty for all ten), and zero formal reviews; all 9 comments were issue-level.
  - 7 were `claude[bot]` LGTM summaries reporting no findings (#3158, #3159, #3161, #3162, #3163 ×2, #3164, #3165, #3166). Not change requests, so none meets the prompt's "fix is needed" bar, and a "no fix needed" reply on an LGTM is pure noise.
  - 2 were real findings **already resolved by the PR author before this run**: #3161's `firestore.rules` `isAdmin()` analog (owner replied, logged to the debugger backlog, correctly scoped out) and #3163's `persistProject`/`updateRun` gap (owner reproduced it, pushed the `clearFields` fix plus a dedicated regression test, and a second bot pass came back clean). Re-replying would have duplicated a resolved thread.
- Fixes pushed: none. No PR carried an unaddressed actionable finding.
- Reviews posted: 10 (#3158–#3167).
- Findings worth carrying forward:
  - **#3166 — the near-miss of the night, and the reason to trace an invariant instead of reading the diff.** The clamp is `clampPoints(parsed, maxPoints)`, and `maxPoints` is `run.rubric ? (run.rubricMaxPoints ?? rubricMaxPoints(run.rubric)) : 0`. On a rubric-less run that is `0`, so `clampPoints(parsed, 0)` would silently zero every typed override — a validation fix turning into data loss. It is **not** a defect: `ProjectGrader.tsx:197` early-returns on `!run.rubric` before the grading UI renders, so `setOverride` is unreachable without a rubric. Worth recording that the invariant holding this fix up lives ~30 lines from the clamp and is invisible in the diff. The general rule: when a fix introduces a bound, find where the bound's *zero/empty* case is excluded, and don't accept "the UI wouldn't do that."
  - **#3159 — moving a toast out of a modal's own subtree has exactly two failure modes; check both.** (1) Stacking: the shared container is `fixed z-toast` (`DashboardView.tsx:142`) and `config/zIndex.ts` has `toast: 14000` vs `modalNested: 10100`, so it still paints above the modal. (2) Mount point: `AdminSettings` is lazy-loaded from `Sidebar`, inside `DashboardView`, so the renderer is always mounted — a toast dispatched into a tree with no container is a silent no-op. Five sibling admin config modals already use `addToast` under the same host, which is the strongest evidence the pattern holds.
  - **#3162 — "behavior-inert" claims are cheap to verify and worth verifying.** The three `baseWidth`/`baseHeight` edits are inert only because `WidgetRenderer.tsx:325` branches on `skipScaling` and reads `baseWidth` solely on the non-skip branch (`:339`). Also confirmed `WIDGET_SCALING_CONFIG` has no production consumer outside `WidgetRenderer.tsx` — the only other references are test mocks — so there is no second path where the numbers still matter.
  - **#3165 — a normalization helper on a write path needs two properties checked, not one.** `canonicalBuildingId` is `BUILDING_ID_ALIASES[id] ?? id`, so unknown ids pass through rather than being dropped (no silent unassignment), and `canonicalizeBuildingIds` dedupes via a `Set` (no double entry when legacy + canonical coexist). Both hold. Side effect worth naming in the PR body: `toggleBuilding` writes the canonicalized array back, so the first toggle silently migrates that activity's stored ids.
  - **#3161 — the fix is correct; the fail-open default is the part to watch.** `isSuperAdminRoleId` returns `true` for a missing `roleId` (legacy pre-Phase-4 docs). Safe today only because `/admins/{email}` is `allow write: if false`, so no client can plant a marker-less doc — that justification belongs in the PR body, not just in reviewers' heads. Recommended follow-up: backfill `roleId` on the legacy docs, then invert the default to deny, so the authz surface fails the same direction as the `APPEARANCE_CONFIG_KEYS` allowlist does.
  - **#3164 — a root-cause fix that leaves its own band-aid in place is only half landed.** `QuizEditorModal.tsx:362-370`'s `window`-level Escape listener is now genuinely dead (the new `document` listener's `stopPropagation()` stops the event before `window`). Harmless, but it reads as load-bearing to the next person touching Escape in that file. Recommended removing it in this PR rather than carrying it as a #3167 backlog row. Separately: Escape handling here now rests on two implicit ordering assumptions (Modal's bubble-phase `window` listener, and this `document` listener) — if a third lands, centralize.
  - **#3163 — a test's exact-arguments assertion can shape production code, and it did here.** `updateRun({...}, ...(clearFields ? [clearFields] : []))` exists because `toHaveBeenCalledWith(oneArg)` fails on `[arg, undefined]`. The workaround is correct but reads as a puzzle; relaxing the assertion and calling `updateRun(payload, clearFields)` plainly would be kinder to the next reader. Recorded because this is the second time this run's PRs hit the optional-trailing-parameter/arguments-array interaction (#3167 logs it as a lesson too).
- Cross-PR checks run this review:
  - **All 10 PRs share one base (`dev-paul`).** Only file overlap found: none — the ten diffs are fully file-disjoint. No merge-order constraint from conflicts.
  - **Two content-level ordering constraints do exist, both docs-narrate-code:** #3160 retires unifier exception D5-E11 for the conversion #3159 performs, and #3167 logs #3161–#3166 as shipped. Both should land after the code PRs they describe, or their rows briefly describe state not yet on the base. Flagged on each.
  - **Verified #3160's retraction rather than accepting it.** The old D5-E11 row called `CatalystConfigurationModal`'s Toast "inline within modal body"; `Toast` renders `fixed bottom-4 right-4 z-toast` at 14000 over `modalNested` 10100, so the row was genuinely mis-catalogued. The retraction is accurate, and the generalized lesson it records (JSX position in the return ≠ visual stacking) is the durable part.
  - **No PR targets `main`** tonight — all ten are `→ dev-paul`, so no push to `dev-paul` was needed or made.
- Verification standard: no code was pushed, so no pre-push gate applied. Every review claim above was verified against the local tree or the branch diff (`git fetch` + `git diff origin/dev-paul...origin/<head>`), not inferred from the PR description or from the existing bot review. Three claims that looked plausible were dropped after tracing them to source: the #3166 zero-max data loss (blocked by an early return), a suspected `canonicalizeBuildingIds` id-drop on #3165 (helper falls back to the raw id), and a suspected unused-import regression on #3159 (`useState`/`useCallback` both still used).
- Notes:
  - Branch safety: **no push to `main`**, and no `dev-*` branch touched. Only `scheduled-tasks` was written to, for this log.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt — which again heads an open PR (**#3162**) carrying a code change this run also reviewed. Seventh consecutive run recording the same tradeoff. Appended at end-of-file to stay conflict-free. Note this commit re-triggers CI on #3162 and adds a post-review commit to a PR this run marked Ready.
  - **`gh` CLI is absent from this environment**, contrary to the prompt's Pre-Task step; GitHub access was entirely via the MCP server. Every `gh` command in the prompt has an MCP equivalent, and `gh auth status` cannot be run at all.
  - **`/mnt/skills/user/` still does not exist** — eighth consecutive run recording this. Repo-local equivalents are at `.claude/skills/{new-widget,admin-widget-config}/`; widget and admin-config standards came from those plus the repo `CLAUDE.md`. Cost was low again: no PR added a new widget or an admin config panel.
  - **Local `git diff` was used as the default, not the fallback**, per the 09-16 recommendation. It paid off: the combined #3160/#3167 docs diff alone was 95 KB and blew the output cap even locally, and was read with `cut -c1-400` instead. Recommend that any docs-journal PR be read truncated by default — those files have single lines in the multi-KB range.
  - **Node version mismatch present and benign again:** container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation.
  - **The append-contention recommendation still stands, unactioned**, and is now urgent by its own terms: one file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed since 08-26, two entries have already been lost to the single-file mechanism, and the file is now ~779 KB. Reading it for context is a real cost; this run read only the last entry and the heading list rather than the file.
  - **The last logged entry before tonight was 2026-09-16** — the 09-17 and 09-18 runs left no entry. Unclear whether they did not run or did not log; worth checking, since a silent no-log looks identical to a silent no-run.

## 2026-09-21

- PRs reviewed: **2** (every open PR — the smallest queue any run has seen; the previous run reviewed 8, the one before 10).
  - **#3190** Accessibility improvements, print tests for team teachers, and bug fixes (`dev-paul` → `main`)
  - **#3191** Monday scheduled tasks — journal audits + css-scaling fix (`scheduled-tasks` → `dev-paul`, draft)
- Comments processed: **1 — 0 fixed, 0 explained, 1 no action.** Both PRs returned `totalCount: 0` inline review threads and `get_reviews` returned `[]` for both. The single PR-level comment was a `claude[bot]` summary on #3191 ending "No issues found." No reply was posted: replying "no fix needed" to an unqualified clean review is noise. **Ninth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect, and the one real defect found by independent review (#3190's failing check, below) is not fixable from inside that PR's diff.
- Reviews posted: **2** (one structured review per open PR).
- Findings that came from independent verification rather than restating the diff:
  - **#3190 — the failing `Claude PR Review` check is caused by the PR's size, not by its code, and it will not self-heal.** `mergeable_state` is `unstable`; run 35557502212 on head `0a7784b` fails at the `gh pr diff "3190" > pr.diff` step with `HTTP 406: Sorry, the diff exceeded the maximum number of lines (20000)` / `PullRequest.diff too_large`, exit 1. The follow-up step then emits `No execution log. The action skipped`, whose message blames a workflow-file drift or the actor gate — both wrong here, which is why reading the log mattered more than reading the check name. **The automated review never ran on this PR.** At 23,453 changed lines against a 20,000-line ceiling this is deterministic, not flaky, and every future push reproduces it. Two fixes proposed, neither a change to this diff: the job already checks out `refs/pull/3190/merge` in full, so `git diff origin/main...HEAD` needs no API call and has no ceiling; or fetch via `gh api -H "Accept: application/vnd.github.v3.diff"`. Not pushed — the workflow file is outside the PR's scope and `dev-paul` had no review comment to license a push under the branch rule.
  - **#3190 — the 25-widget settings-drawer migration was verified mechanically, not by eye.** At 174 files a visual pass would prove nothing, so the two registry maps were diffed as sets: every `WidgetType` removed from `WIDGET_SETTINGS_COMPONENTS` is added to `WIDGET_SETTINGS_SCHEMAS`, both directions, with no type falling out of both (which would have silently removed a widget's settings panel with no type error). All 25 new `settings.schema.ts` files exist and carry a default export. The 25 old `Settings.tsx` files are deleted rather than orphaned, and none of their named exports is referenced from any source file.
  - **#3190 — the docs still teach the pattern this PR retired.** Five deleted exports are still named in `.claude/skills/new-widget/SKILL.md` and `.agents/skills/new-widget/SKILL.md` (`ScheduleSettings`, `SpecialistScheduleSettings`), `.github/copilot-instructions.md`, `docs/routines/debugger.md` (`CountdownSettings`, `RecessGearSettings`) and `docs/plans/shipped/widget-settings-inventory.md` (`BreathingSettings`). The `new-widget` skill will now teach `lazyNamed` + `WIDGET_SETTINGS_COMPONENTS` for widgets that no longer use it — a docs pass is owed before the next widget is built from that skill.
  - **#3190 — a prior run's finding is confirmed resolved.** The 2026-09-20 entry flagged #3181's `helpReaderOk()` for ordering `isSuperAdmin()` (a `get()`) ahead of `isOrgMember()` (an `exists()`). The merged form reads `isOrgMember(...) || isSuperAdmin()`, so the read charge is gone, and the retained comment correctly explains why `isAdmin()` was the wrong predicate there (a bare `/admins/{email}` doc also mirrors org-scoped roles, letting a `building_admin` of one org read another org's help items).
  - **#3190 — the delegation path is the run's real risk surface and it holds up.** `authorizeDelegatedPrint` runs the whole §4 ladder server-side before any admin read or write; both kill switches are re-read server-side; `firestore.rules` gains no PLC branch into `users/{uid}/**` and `paper_batches` stays owner-only; `toPrintStudent` rebuilds rows from three fields so `pin`/`email` cannot ride along; the caller sends selections and never a seat map, so seats are re-derived from the target's real rosters; the copy claim is transactional on the `sync.groupId` query with the loser trashing its own Drive file, and a failed sync-join rolls back both the doc and the file. Two minor notes raised: `unnamedSpares += roster.studentCount` is unbounded while the caller's `spareCount` is capped at 20 (not a DoS — `planPaperBatch` throws past `MAX_SEAT = 1023` — but the teacher sees a generic "split it across batches" rather than an error naming the roster), and `memberName()` can fall back to a teacher's email, which then persists as `printedByName` in another user's document and prints on the sheet.
  - **#3191 — a `cqmin` fix is only a fix if a container exists, so that was checked rather than assumed.** `min(12px, 4.5cqmin)` matches the `components/widgets/CLAUDE.md:88` small-label formula and the file's own neighbours (lines 703–718 already use `min(12px, 4cqmin)` and `min(14px, 5cqmin)`), but none of that proves the units resolve. `WidgetRenderer.tsx:330` and `components/common/DraggableWindow.tsx:2291` both set `containerType: 'size'`, so the widget shell supplies the query context. Without it a `cqmin` value degrades silently to viewport-relative sizing and the change would have been a no-op that still reviews as correct.
- Cross-PR checks run this review:
  - **The two PRs are stacked, not parallel:** #3191's base is `dev-paul`, which is #3190's head. #3191 must land before #3190 for its content to reach `main` in this cycle, or it waits for the next one. No file overlap between the two diffs beyond that relationship.
  - **#3190 is `dev-*`-headed and therefore push-restricted.** It carried no review comment, so the "may push to `dev-paul` when there are PR comments on a PR merging `dev-paul` into `main`" clause never activated and no push was made.
- Verification standard: no code was pushed, so no pre-push gate applied. Every claim above was checked against the branch (`git diff origin/main...origin/dev-paul`, `git show origin/dev-paul:<file>`) or against CI job logs, not inferred from the PR body or the existing bot review. Two suspicions were dropped after tracing them: unbounded `unnamedSpares` as a resource-exhaustion vector (capped downstream by `MAX_SEAT`), and the 25 migrated widgets leaving dead named exports behind (the source files are deleted, and only docs still name them).
- Notes:
  - Branch safety: **no push to `main`**, and no `dev-*` branch touched. Only `scheduled-tasks` was written to, for this log.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt — which again heads an open PR (**#3191**) carrying a code change this run also reviewed. Eighth consecutive run recording the same tradeoff. Appended at end-of-file to stay conflict-free. This commit re-triggers CI on #3191 and adds a post-review commit to a PR this run marked Ready.
  - **`gh` CLI is absent from this environment**, contrary to the prompt's Pre-Task step; GitHub access was entirely via the MCP server. `gh auth status` cannot be run at all. Ninth consecutive run recording this — worth editing the prompt, since every `gh` command in it has an MCP equivalent and the Pre-Task step as written cannot be followed.
  - **`/mnt/skills/user/` still does not exist** — ninth consecutive run recording this. Repo-local equivalents at `.claude/skills/{new-widget,admin-widget-config}/` plus `components/widgets/CLAUDE.md` supplied the widget and admin-config standards, and both were load-bearing this run (the small-label formula and the registry checklist came from them).
  - **The 20,000-line API diff ceiling now applies to this routine too, not just to CI.** `gh pr diff` is the prompt's Phase 2 step 1; on #3190 it would have failed exactly as the workflow's did. Local `git diff` against the fetched refs was used instead and had no such limit. Recommend the prompt drop `gh pr diff` in favour of `git diff <base>...<head>` outright rather than as a fallback.
  - **Node version mismatch present and benign again:** container runs Node v22.22.2 against `engines: >=24.0.0`, printing an unsupported-engine warning on every `pnpm` invocation.
  - **The append-contention recommendation still stands, unactioned.** One file per run under `docs/scheduled-tasks/pr-review-log/` has been proposed since 08-26; two entries have already been lost to the single-file mechanism and the file has passed ~780 KB. This run again read only the heading list and the final entry rather than the file.

## 2026-09-22

- PRs reviewed: **8** (every open PR). All eight target `dev-paul`, all eight are drafts. Seven head branches are `nightly/*`, `claude/*` or `scheduled-tasks`; none is `main`- or `dev-*`-headed, so all were writable and the push-restriction clause never activated.
  - **#3254** docs(routines): log Run 78 — 5 bugs shipped, 1 security (`nightly/debugger-log-2026-09-22`)
  - **#3253** fix(dashboard): recover AssignStudentPicker when the active roster disappears (`nightly/dashboard-layout-2026-09-22`)
  - **#3252** docs(skills): fix stale SpecialistSchedule `Settings.tsx` references (`scheduled-tasks`)
  - **#3251** fix(admin): give unlabeled config Toggles an accessible name (`nightly/admin-config-2026-09-22`)
  - **#3250** fix(flashcards): don't drop a lone header-like card on import (`nightly/widgets-2026-09-22`)
  - **#3249** fix(quiz): stop grade-inflating mixed dedup conventions in Classroom push (`nightly/state-data-2026-09-22`)
  - **#3248** fix(rules): SECURITY — close a cross-org admin data leak on announcements (`nightly/build-tooling-2026-09-22`)
  - **#3247** fix(admin): finish D3 exclusive-choice radiogroup retrofit in TimeToolConfigurationPanel (`claude/admiring-gates-25th74`)
- Comments processed: **7 — 0 fixed, 0 explained, 7 no action.** All eight PRs returned `totalCount: 0` inline review threads and `get_reviews` returned `[]` for every one. Seven PR-level `claude[bot]` summaries existed (#3247 ×2, #3248, #3249, #3250, #3251, #3253), every one of them closing with an explicit no-findings verdict ("Nothing to flag", "Looks good to merge", "No correctness, security, or performance concerns"). #3252 and #3254 had no comments at all. No reply was posted to any of the seven: replying "no fix needed" to an unqualified clean review is noise. **Tenth consecutive run with nothing unresolved to act on.**
- Fixes pushed: **none.** No comment identified an unaddressed defect, and every finding produced by independent review below is either out of the relevant PR's scope or a one-line change on another agent's open PR, raised in the review for its author rather than pushed.
- Reviews posted: **8** (one structured review per open PR).
- Findings that came from independent verification rather than restating the diff:
  - **#3248 — measured the rules-size headroom this security fix consumes, rather than trusting the green `check:rules-size` gate.** Running `stripRulesComments` over both refs: `origin/dev-paul` is 181,275 stripped bytes, this PR is 181,754. The empirical compiled-ruleset cliff in `scripts/checkRulesSize.mjs` is 182,349, so headroom falls from 1,074 bytes to **595** — 45% of the remaining budget, one night after #3241 had to reclaim it by refactoring 49 call sites. CI passes, so nothing flags this. Note that `stripRulesComments` removes full-line `//` comments *before* measuring, so the +479 is entirely rule logic (the new `announcementWriteScopeOk()` helper plus three `allow` rules replacing one) — trimming the new comment blocks would buy none of it back. Any further `firestore.rules` change this cycle needs `check:rules-size` re-run against the merged result, not against `dev-paul`.
  - **#3248 — the client half of the fix has no test, which is the exact gap that cost #3195 two review rounds.** `tests/rules/announcementCrossOrgAdmin.test.ts` (419 lines) proves the rule against the real emulator, but nothing exercises `AnnouncementsManager`'s new `isSuperAdmin ? collection(…) : query(…, where('orgId','==',orgId))` branch or the org-less-admin closed-listener branch. A rules suite cannot prove the component sends the query the rule expects — #3195 shipped a correct rule with a still-unfiltered client query and only a human reviewer caught it. `tests/components/admin/AnnouncementsPollExport.test.tsx` already establishes a harness for this component.
  - **#3248 — checked every consumer of the collection before accepting "no reader regression".** `components/announcements/AnnouncementOverlay.tsx:421` (the teacher/student-facing reader) was already org-scoped via `where('orgId','==',orgId)` / `== null`, so tightening the read rule doesn't break it; `PollWidget` and the manager's poll panel touch only the untouched `pollVotes` subcollection. Two consequences worth recording: a non-super org admin can no longer see or manage legacy (missing-`orgId`) or explicit `orgId: null` announcements, and `pollVotes` stays authed-user-readable while its parent is now org-gated.
  - **#3254 → a live instance of the run's own new standing rule, in the collection #3248 was modeled on.** Run 78's notes derive the rule "check that write access isn't *narrower* than the matching read access for any role the fix's read-side change was meant to preserve." Applying it to `help_resources` turns up the same gap `4ee96d6` had just closed in `announcements`: `helpReaderOk()` (`firestore.rules:4074-4078`) grants read via `… || isSuperAdmin()`, but `helpScopeOk()` (`:4048-4051`) is `(d.orgId == null && isSuperAdmin()) || (d.orgId is string && isOrgMember(d.orgId))` — **no `isSuperAdmin()` fallback on the org-scoped branch**. `hooks/useHelpResources.ts` has an `allOrgs` browse mode (`:91`, `:117`) gated on exactly `isSuperAdmin`, so the manager surfaces foreign-org rows whose edit/delete will fail. Raised as a backlog candidate on #3254 rather than pushed — it is outside every open PR's diff.
  - **#3252 — the journal's claim that the two skill mirrors differ only cosmetically is wrong, and that claim is what would keep the drift hidden.** The 2026-09-22 `skill-freshness.md` entry says the `.claude/` and `.agents/` copies differ by "only cosmetic Prettier/naming/'SPART Board' vs 'SpartBoard'". Diffing them: `.claude/skills/new-widget/SKILL.md` carries a **Step 3.5** row in the 8-file checklist (`pnpm run new-widget-settings`) and three `WIDGET_SETTINGS_SCHEMAS` mentions; `.agents/` has **no Step 3.5 row at all** and one mention. Conversely `.agents/` carries the "clock/time-tool/text/embed/lunchCount are migrated — don't use them as legacy `Settings.tsx` references" note (this journal's own 2026-09-08 action) and `.claude/` does not. An agent reading the `.agents/` mirror gets a widget-creation checklist that omits the schema-driven settings path entirely. Pre-existing on `dev-paul`, so not this PR's defect — but the one-line correction belongs in a file this PR already touches. **Nothing enforces `.claude/skills/**` ≡ `.agents/skills/**`;** a `diff -r` in `validate` would make the weekly manual read deterministic.
  - **#3252 — verified all four factual claims in the skill rewrite against source rather than the diff.** `components/widgets/SpecialistSchedule/` contains no `Settings.tsx`; `index.ts` is exactly `export * from './SpecialistScheduleWidget';`; `WidgetRegistry.ts:308` registers `settings.schema` into `WIDGET_SETTINGS_SCHEMAS` while `:183-184` is the only `lazyNamed` for the widget; `settingsFields.tsx:34-36` genuinely reads `featurePermissions` via `useAuth()`, so the repointed example is correct. All four hold. One omission: `Settings.test.tsx` still exists (importing from `./settingsFields`) and isn't in the rewritten file list — the same stale-filename trap one level down.
  - **#3251 — re-derived the remaining inventory independently instead of trusting the PR body's backlog list.** Scanning `components/admin/**/*.tsx` on the PR head for `<Toggle>` elements with no `label` prop returns **15 instances across 9 files**, matching the description's list exactly, file for file and count for count. Spot-checking `GlobalPermissionsManager.tsx:1140` and `:1382` confirms they are genuine (`role="switch"` with only an unassociated sibling `<span>`) — and those two are the per-feature master enable switches, the highest-stakes toggles in the admin panel, so they belong at the head of the next batch. `DockDefaultsPanel.tsx:58` will need a dynamic label (`Enable ${building.name}`).
  - **#3247 — the index-mapping assumption in `handleRadioGroupKeyDown` was checked, not assumed.** The helper collects `[role="radio"], [role="tab"]` descendants of `e.currentTarget` and indexes `options` by **DOM position**, so any extra focusable child silently desynchronises selection from focus. Both converted containers render exactly one `role="radio"` per option and nothing else, and the Accent Color reset button sits outside the radiogroup `<div>`. No keyboard trap either: `TRAFFIC_COLORS[0]` is `{ value: null, label: 'None' }`, so `(cfg.timerEndTrafficColor ?? null) === value` always matches exactly one option and `tabIndex={checked ? 0 : -1}` always leaves one tabbable button — which is why Accent Color (no null option) needs the `hasThemeColorSelected` fallback and Traffic Light does not.
  - **#3253 — verified the adjust-during-render guard terminates and no-ops correctly, since that pattern fails silently in both directions.** After `setActiveRosterId(rosters[0].id)` the predicate `!rosters.some(r => r.id === activeRosterId)` is false on the re-run, so it fires at most once per roster change — no render loop. With `rosters.length === 0` the guard is skipped by short-circuit ordering and `activeRoster` resolves to `null` through the existing `find(...) ?? null`, so the missing reset branch the comment reasons about is genuinely unnecessary. That empty-rosters branch is the one the new test doesn't cover and the one a refactor is most likely to break.
  - **#3249 — traced the dedup convention rather than accepting "dedupe once" at face value.** `dedupeQuestionsById` (`utils/quizMaxPoints.ts:45`) keeps the **first** occurrence, so the denominator's existing convention is preserved and only the numerator's `Map`-keeps-last behaviour changes — the safe direction. No circular import (`quizMaxPoints.ts` imports only `@/types`). `canScoreResponse` and `isResponseAwaitingGrade` both key off question id via Map/Set lookups, so pre-deduped input is behaviour-preserving for already-clean callers.
  - **CLAUDE.md's one-line comment rule is the most-repeated finding of the run — three of five code PRs breach it.** #3249 grows an existing 7-line block to **16 lines** of pure root-cause narrative (the `Map`-last vs `Set`-first story, already in the PR body); #3250 adds a fresh 3-line block to a file with no verbose-comment convention to inherit; #3253 grows a 3-line block to 6. The rule's exception covers *matching* an existing convention, not doubling a block's length. Raised on each PR.
- Cross-PR checks run this review:
  - **All seven `nightly/*`+`claude/*` PRs share the same base commit** (`bc0a5356` on `dev-paul`, except #3247 at `88ef714a`), and their changed-file sets are fully disjoint — no two touch the same file, so they can land in any order without conflict.
  - **#3252's head is `scheduled-tasks`**, the branch this log is appended to, so this run's own log commit lands on a PR this run also reviewed. Ninth consecutive run recording that tradeoff (see Notes).
  - **CI absence on #3252 and #3254 is intentional, not a missing run.** Both returned `total_count: 0` check runs; `pr-validation.yml` `paths-ignore`s `**/*.md` and `docs/**`, and both PRs are 100% Markdown. Verified the workflow file rather than reporting a red/absent gate. The other six are green, 14/14 checks each.
- Verification standard: no code was pushed, so no pre-push gate applied. Every claim above was checked against the branch (`git diff <merge-base>...<head>`, `git show <ref>:<file>`), against a measurement (`stripRulesComments` over both refs), or against the workflow/check-run data — none inferred from a PR body or an existing bot review. Two suspicions were dropped after tracing them: that #3254's `announcementWriteScopeOk()` backlog row described a still-open defect (it is correctly filed as "open (process note, not a bug)" — `4ee96d6` does carry the `|| isSuperAdmin()` fallback), and that #3248's read-rule tightening might regress the announcement overlay (it was already org-scoped).
- Notes:
  - Branch safety: **no push to `main`**, and no `dev-*` branch touched. Only `scheduled-tasks` was written to, for this log.
  - **Log placement: appended to `scheduled-tasks`**, per the task prompt — which again heads an open PR (**#3252**) carrying a change this run also reviewed. Ninth consecutive run recording the same tradeoff. Appended at end-of-file to stay conflict-free. Because the log is Markdown-only and `pr-validation.yml` `paths-ignore`s `**/*.md`, this commit does **not** re-trigger CI on #3252 — the first run where that side effect is absent.
  - **`gh` CLI is absent from this environment**, contrary to the prompt's Pre-Task step; GitHub access was entirely via the MCP server. `gh auth status` cannot be run at all. Tenth consecutive run recording this — every `gh` command in the prompt has an MCP equivalent and the Pre-Task step as written cannot be followed.
  - **`/mnt/skills/user/` still does not exist** — tenth consecutive run recording this. Repo-local equivalents at `.claude/skills/{new-widget,admin-widget-config}/` supplied the widget and admin-config standards, and were load-bearing this run: #3252's entire review is a verification of those two files' accuracy.
