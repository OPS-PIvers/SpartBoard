...
- Findings that came from independent verification rather than restating the diff:
  ...
  - **#3190 — the docs still teach the pattern this PR retired.** Five deleted exports are still named in `.claude/skills/new-widget/SKILL.md` and `.agents/skills/new-widget/SKILL.md` (`ScheduleSettings`, `SpecialistScheduleSettings`), `.github/copilot-instructions.md`, `docs/routines/debugger.md` (`CountdownSettings`, `RecessGearSettings`) and `docs/plans/widget-settings-inventory.md` (`BreathingSettings`). The `new-widget` skill will now teach `lazyNamed` + `WIDGET_SETTINGS_COMPONENTS` for widgets that no longer use it — a docs pass is owed before the next widget is built from that skill.
