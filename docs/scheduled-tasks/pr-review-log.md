...
- Local verification run for this review (Node 22; CI on Node 24 remains authoritative). All type-checks clean, all suites green:
  ...
  - Merge safety: all 12 distinct branch tips verified to merge cleanly into `dev-paul` **individually and in sequence** (`git merge` simulation on a scratch branch, not just `merge-tree`). CI green on every code-touching PR head checked, #2395 included (7/7 via `get_check_runs`); docs-only heads have zero checks by design (see below).
- New findings this run:
  ...
  - **Methodology correction — `get_status` is the wrong endpoint for CI in this repo, and it produced one false finding this run.** I initially reported "#2395 has no CI on its current head" and added "restore CI" to its pre-merge gates. That was wrong: `get_status` returns the legacy *commit status* API, which is empty for **every** PR in this repo because results are published as GitHub Actions **check runs**. Querying check runs directly returns 7/7 green on `5552a62`. Retracted on the PR. Use `get_check_runs`, never `get_status`, for CI state here.
  - **Docs-only PRs correctly have zero checks, and that is not a missing signal.** `.github/workflows/pr-validation.yml` carries `paths-ignore: ['**/*.md', 'docs/**']`, added deliberately so the nightly fleet's journal-only PRs don't spend Actions minutes on Markdown. This affects #2522, #2507, the review-log stack, and this run's own log PR. I had claimed "CI green" on #2522 without verifying; corrected on the PR.
