...
- PRs reviewed: 16 open PRs (all authored by OPS-PIvers, all drafts, all targeting `dev-paul`). None had a head of `main` or `dev-*`, so all 16 were in scope. Eight are new since the last run (the 2026-08-15 nightly batch plus the prior run's own log PR); the other eight all carry commits landed *after* their last review, so each got a delta-scoped follow-up rather than a re-review of unchanged code.
  - #2464 — docs(routines): debugger run 39 log (head `nightly/debugger-log-2026-08-15`) — docs-only.
  - #2463 — fix(lti): dedup OneRoster roster emails before scoring class-link overlap (head `nightly/build-tooling-2026-08-15`) — `string[]` → `Set<string>` in `ltiSuggestClassLinkMatchV1`; new test case.
  - #2462 — fix(i18n): unify DE meeting-agenda terminology to "Tagesordnung" (head `nightly/admin-config-2026-08-15`) — one `de.json` value; new 4-case test.
  - #2461 — fix(rosters): sync `pin_index` sidecar for students migrated off Firestore (head `nightly/state-data-2026-08-15`) — adds the missing `syncRosterPinIndex()` call to `runMigrationIfNeeded`; new test.
  - #2460 — fix(dashboard): resolve widget-keyboard-action target through settings-panel portals (head `nightly/dashboard-layout-2026-08-15`) — `data-widget-id` on `SettingsPanel`'s portal + widened `closest()` selector in `DashboardView`; 3 new tests.
  - #2459 — fix(widgets): stop Escape leaking from RandomGroups color picker (head `nightly/widgets-2026-08-15`) — duplicate of #2452, see below.
  - #2458 — docs(routines): unifier run 56 log (head `nightly/unifier-log-2026-08-15`) — docs-only.
  - #2457 — pr-review log for 2026-08-14 (head `claude/pensive-bell-lpgj1j`) — docs-only, the prior run's own output.
  - #2456 — chore(scheduled-tasks) batch (head `scheduled-tasks`) — 2 new commits since last review: Saturday audit journals + `EmbedWidget` zoom-toolbar `vmin` conversion.
  - #2455 / #2454 / #2453 / #2452 / #2451 / #2450 — the 2026-08-14 nightly batch, each with one follow-up commit landed since its 2026-08-14 review.
  - #2395 — feat(ai): move Gemini to Vertex AI (head `claude/quirky-ritchie-wghdl3`) — 4 commits since its 2026-08-08 review. Open 10 days.
- Comments processed: 3 unresolved inline threads, all on #2395 — 0 fixed, 0 newly explained. All three already carry an owner reply and are verification notes gated on the preview-deploy smoke test, not open change requests; 10 of the 13 threads have since been resolved (up from 0 last run). No re-reply was added under the frugality directive. The other 15 PRs carried **zero** inline review comments.
- Fixes pushed: **0**. No comment on any PR was left unaddressed, so Phase 1 had nothing to fix. No push was made to any PR head branch this run. All findings below were delivered as review comments instead — several are cross-PR conflicts that no single-branch commit could resolve.
- Reviews posted: 16 — one structured review per open PR (each carries the automated-review disclaimer + Claude Code attribution footer). Merge-readiness calls:
  - Ready: #2463, #2462, #2461, #2454, #2453, #2452, #2450, #2457 (docs).
  - Ready with minor notes: #2464 (docs), #2458 (docs), #2455 (docs), #2451 (docs), #2456.
  - Needs changes: #2460, #2459.
  - Needs changes / hold (draft): #2395 — every code finding across nine review rounds now closed; blocked only on 4 operational gates.
- Principal findings this run (all cross-checked against the tree, not diff-reading alone):
  - **#2459 duplicates #2452 and is the weaker implementation.** Same bug, same file, same `useEffect` slot in `RandomGroups.tsx` — the 2026-08-15 nightly re-found it because #2452 is still unmerged. #2459 omits the `isEscapeFromWidgetInput(e)` guard that protects the group-rename input's own Escape handler (`RandomGroups.tsx:256-264`). Verified they cannot both merge: `git merge-tree` across the two heads yields **6 conflict markers**. Recommended closing #2459 in favour of #2452.
  - **#2460's widened selector silently disables the top-widget fallback for nine portals.** `closest('.widget, [data-widget-portal]')` matches any `[data-widget-portal]`, but only `SettingsPanel` carries `data-widget-id`; swept the other nine (`DrawingWidget` ×3, `GuidedLearningEditor` ×3, `ScreenCaptureModal`, `ConfirmDialog`, `PromptDialog`, `TextWidget` ×2, `DraggableWindow`'s title input) — none do. Confirmed empirically in the repo's jsdom setup that `getAttribute('data-widget-id')` then returns `null`, so `targetId` is `null` and Delete/Alt+P no-op where they previously fell back to `topWidget.id`. All 3 new tests use a synthetic fixture that always carries the id, so none covers the failing branch. Suggested `'.widget[data-widget-id], [data-widget-portal][data-widget-id]'` or an explicit `?? topWidget.id`.
  - **#2456's `vmin` coefficients are ~25% tighter than the LunchCount precedent they cite.** `min(16px, 1.6vmin)` breaks even at a 1000px viewport minimum dimension; LunchCount's `min(20px, 2.5vmin)` breaks even at 800px. Worked the arithmetic across real resolutions: no-op at 1920×1080, but at 1366×768 the zoom buttons drop 32px → 24.6px and the zoom-% readout 12px → 9.2px; at 1280×720 the buttons reach 23.0px, under the WCAG 2.2 SC 2.5.8 24×24 minimum. `min()` can only shrink relative to its cap, so it addresses the "doesn't grow on projectors" finding only above break-even and regresses below it. Suggested LunchCount-matched ratios, or `clamp()` if growth is the actual goal.
  - **Nightly run numbers and backlog attributions are colliding across nights.** #2455 and #2464 both bump `docs/routines/debugger.md` from run 38 → **39**; #2451 and #2458 both bump `docs/routines/unifier.md` from 55 → **56**. Each pair rewrites the same counter line, so they conflict and the later merge leaves the counter under-stated. Worse, #2455 and #2464 strike the *same* `RandomGroups` backlog row with contradictory attributions (**Fixed #2452** vs **Fixed #2459**). Structural cause: nightly branches are cut from `dev-paul` while the prior night's PRs are still open, so each run reads a stale counter and re-discovers unmerged fixes. Recommended merging log PRs same-day or rebasing each night's branches onto the previous night's merged state.
  - **#2395: one new code item** — `KNOWN_GEMINI_MODELS` and `isDeprecatedGeminiModelId` are now independent lists with nothing asserting they agree, and the deprecation warning fires only on the `Custom...` branch. Consistent today, but a future picker entry the predicate rejects would be offered as a first-class option and silently discarded server-side. Suggested a one-line consistency test.
- Verification performed this run (reviews were evidence-based, not diff-reading alone):
  - `pnpm run type-check` run on every code-carrying branch (#2456/#2459/#2460/#2461/#2462/#2463, plus the #2450/#2452/#2453/#2454 follow-up heads) — all clean.
  - Affected suites run per branch: #2460 `DashboardView.test.tsx` **34/34** ✓, #2461 `useRosters.test.ts` **36/36** ✓, #2459 `RandomGroups.test.tsx` **9/9** ✓, #2462 full `tests/i18n/` **40 files / 1563 tests** ✓, #2463 `courseLinkEndpoints.test.ts` **18/18** ✓.
  - **#2460's regression claim confirmed empirically**, not inferred: ran a throwaway jsdom probe asserting `btn.closest('.widget')` → `null` (old: falls back to top widget) vs `btn.closest('.widget, [data-widget-portal]')` → the portal element with `getAttribute('data-widget-id')` → `null` (new: `targetId` null, dispatch skipped). Probe file removed afterwards; working tree clean.
  - **#2459/#2452 conflict confirmed** via `git merge-tree` across the two heads (6 conflict markers), rather than assumed from the diffs touching the same file.
  - **#2462's premise re-checked against the tree**: `de.json` contains exactly two `"agenda"` leaves (`:623`, `:922`), both now `"Tagesordnung"` — no third occurrence left behind.
  - **#2456's shrink quantified** from the `min()` arithmetic across 1920×1080 / 1366×768 / 1280×720 and cross-referenced against the actual LunchCount ratios in `LunchCount/Widget.tsx:1086-1113`, rather than asserted qualitatively.
  - **#2451's correction of a prior finding verified and accepted**: last run flagged `DockSection.tsx:69` as a live WCAG 2.5.3 divergence; #2451's follow-up found both strings resolve through the same i18n key in all 4 locales, so the divergence exists only between hardcoded `defaultValue` fallbacks. Narrower than originally stated — recorded here so the next run doesn't restate the stronger claim.
- Notes:
  - Branch-safety: no push to `main` or any `dev-*` branch. No push to any PR head branch at all this run.
  - This log commit is on the designated `claude/pensive-bell-1yqsp3` branch, **stacked on `claude/pensive-bell-lpgj1j`** (the still-open #2457) rather than cut fresh from `dev-paul` — deliberately, since cutting fresh would append to the same file tail and conflict, which is the exact pattern flagged on #2455/#2464 and #2451/#2458 this run. Merging this PR carries #2457's entry with it, after which #2457 can be closed as superseded. Continues the standing precedent of keeping the log off `scheduled-tasks` (head of actively-open PR #2456); that divergence from the literal POST-TASK instruction has now held for four consecutive runs and should be written into the task definition.
  - Tooling: this environment exposes GitHub via the MCP server (no `gh` CLI); all PR list/diff/comment/review operations used `mcp__github__*` equivalents of the prescribed `gh` commands.
  - Verification env runs Node 22 (repo pins 24, "Unsupported engine" warning); CI on Node 24 remains the authoritative gate.

## 2026-08-15 (evening)

- PRs reviewed: 16 open PRs — the same set as this morning's run (#2450–#2458, #2460–#2465, #2395), since none merged in between and none opened. #2459 was closed as a duplicate by the morning run and is correctly absent. This run was scoped to unresolved comments rather than to fresh reviews: the morning run had already posted a structured review on all 16, so re-reviewing unchanged code would have been pure noise.
- Comments processed: **26** across all 16 PRs (3 unresolved inline threads, 9 issue comments, 21 review bodies) — **1 fixed, 3 confirmed already fixed, 4 explained, the rest already answered.**
  - The morning run posted 16 reviews between 06:17 and 06:23 and then ended without answering the five that carried findings. Four of those five turned out to be already-handled by commits that crossed with the review; one was genuinely open. That gap is the whole of this run's work.
- Fixes pushed: **1**
  - **#2461** — branch `nightly/state-data-2026-08-15`, commit `bea5639`. The 06:19 review's two non-blocking notes were still open on a head last touched at 04:07. (1) `runMigrationIfNeeded`'s `if (rosterName)` skip was the only early-out in `hooks/useRosters.ts` with no breadcrumb — a roster doc with no `name` and no matching meta entry silently got no `pin_index` sidecar, indistinguishable in the console from a roster that was never ClassLink-linked. Now warns with the roster id and the recovery step (`re-save the roster`), matching the file's existing convention. (2) Added the negative test the review asked for — a migrated roster with no ClassLink students must complete without a `commitRosterPinIndexV1` round trip — plus one covering the new warn branch, since an unasserted `console.warn` is exactly what a later refactor drops silently. Both confirmed non-tautological: removing the `else` branch fails the warn case (`1 failed | 37 passed`); removing `syncRosterPinIndex`'s `entries.length === 0` early return fails the negative case alongside the pre-existing local-roster one (`2 failed | 36 passed`). Verified locally: type-check ✓, repo-wide lint (`--max-warnings 0`) ✓, Prettier ✓, `useRosters.test.ts` **38/38** (was 36).
- Replies posted: 5 (#2461, #2456, #2462, #2463, #2464). No new reviews posted — the morning run's 16 stand.
- Findings confirmed already fixed (reviews that crossed with their own fix and were never answered):
  - **#2456 — the `vmin` finding was valid and had already landed.** Commit `bad06a4` at 06:22:11 applied exactly the reviewer's suggested coefficients (`gap 0.5vmin`, `padding 1vmin`, icon `2vmin`, text `1.5vmin`), three minutes after the 06:19:08 review. Break-even moves 1000px → 800px, so the zoom buttons hold at 32px on 1366×768 and 1280×720 instead of dropping to 24.6px/23.0px and crossing the WCAG 2.2 SC 2.5.8 floor. Recorded the outcome on the PR, and noted explicitly that the *original* audit finding (toolbar doesn't grow on projectors) remains open — `min()` caps, so this change only guarantees no regression below the cap; growth needs the `clamp()` shape and its own commit.
  - **#2464 — the run-number collision was already resolved.** Verified against the diff, not the conversation: counter reads **40**, the `RandomGroups` backlog row is attributed `superseded by #2452`, and the PR now targets `nightly/debugger-log-2026-08-14` so the two log PRs stack instead of both rewriting the same trailing line. The reviewer's structural recommendation was also taken up as a Notes & Gotchas entry.
  - **#2458 / #2451** — the Unifier run-number collision was settled in the morning (#2458 renumbered to 57, #2451 keeps 56); both carry replies.
- Findings explained without a code change:
  - **#2462** (DE `"Tagesordnung"`) — Ready, no changes requested. The string-length note is moot (the sibling key already renders the same 13 characters on the same surface). On the suggested generic drift-check: agreed the bug class recurs, but the invariant as stated — every EN key sharing a leaf value must agree per locale — would be noisy, since EN collides on generic leaves (`"Name"`, `"Title"`, `"Close"`) where a target language legitimately varies by grammatical context. A useful version needs a curated domain glossary, not value-equality; filed as that rather than as the general rule, which would get disabled in a week.
  - **#2463** (OneRoster dedup) — Ready, no issues found. The one open item (is the merged/cross-listed-enrollment premise real for this district's feed?) is a live-data question code review can't settle. Added the bounding facts from the code side: `ltiSuggestClassLinkMatchV1` is a suggester, not a writer, so a pre-fix inflated score could surface a wrong section but never silently persist a link. Flagged the specific failure mode worth auditing if anyone checks saved links — `ambiguous` is `best.overlap - secondOverlap <= 1`, so duplicate-inflation could have *suppressed* the ambiguity warning, turning an uncertain suggestion into a confident wrong one.
- Verification performed this run:
  - Re-read all 13 inline threads on #2395: unchanged since the morning run — 10 resolved, 3 deliberately open as pre-merge gates, every one carrying an owner reply. Nothing to re-raise, no reply added under the frugality directive.
  - Checked `get_reviews` (not just `get_comments`) on every PR. This is what surfaced the gap: on this repo the `claude[bot]`/automated findings live in **review bodies**, so a run that only reads issue comments and inline threads sees five PRs as "quiet" when they are in fact carrying unanswered findings. Worth making explicit in the task definition.
  - Confirmed both #2461 fixes fail-before/pass-after by hand (file-swap), rather than trusting that a new test tests something.
- Notes:
  - Branch-safety: no push to `main` or any `dev-*` branch. The one fix landed on a nightly feature-branch PR head (`nightly/state-data-2026-08-15`), which is fair game per the critical rule.
