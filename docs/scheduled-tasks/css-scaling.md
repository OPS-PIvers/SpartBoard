...
### MEDIUM SpecialistScheduleWidget uses `border-[min()]` Tailwind arbitrary value — CSS `min()` not evaluated at build time

- **Detected:** 2026-06-25
- **File:** components/widgets/SpecialistSchedule/SpecialistScheduleWidget.tsx:395
- **Detail:** The widget uses `className="... border-[min(6px,1.5cqmin)] ..."` (Tailwind JIT arbitrary value syntax). Tailwind does not evaluate CSS `min()` expressions inside `[]` — the class resolves to a literal `border-[min(6px,1.5cqmin)]` property string that browsers cannot parse, meaning the border either falls back to the initial value or does not render at all. Widget has `skipScaling: true`. This silently breaks the intended scaled border and is invisible in development because browsers skip unrecognized values without error.
- **Fix:** Remove `border-[min(6px,1.5cqmin)]` from `className` and apply a `style` prop instead: `style={{ borderWidth: 'min(6px, 1.5cqmin)' }}`. Keep any other border-style/color Tailwind classes on `className` — only the `borderWidth` needs to move to a style.
