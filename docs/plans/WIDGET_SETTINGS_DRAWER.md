...
| D8  | **Settings tab groups**                | Fixed order **Content → Behavior → Display**. Empty groups omitted. Style is its own tab; the `display` group renders there (first section, per-board), above the universal Content tier.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| D11 | **Cruft cleanup in bounds**            | Remove dead/duplicate controls; rename/normalize config keys with a **dual-write** migration (new key added, old key kept until D25 retires it); reorganize into the standard groups with i18n'd labels.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| D25 | **One Firebase project**               | The dev preview channels and production deploy to the **same** Firebase project (`--project spartboard` in both `firebase-dev-deploy.yml` and `firebase-deploy.yml`; the dev branch differs only by hosting channel). Rules, indexes, functions, the `global_permissions` flag doc **and every teacher's Firestore data** are shared. Consequences: (a) this series needs **no** `firestore.rules` change, since the flag doc, the user-profile width field and the `configVersion` widget field all land under existing rules, and none may be added without a product-owner ruling; (b) a board saved from the preview bundle is read by the production bundle minutes later, so every config-key rename is **dual-write**: the migration adds the new key and keeps the old one, front faces read new-then-old, and the old key is deleted only by a later migration step that ships **after** the reading code has been on `main` for one full release (§4.4); (c) once wave 1b merges, admins see the drawer on production too, which is accepted for an admin-gated flag. **Rev 8, from the go/no-go grading.** |
| D26 | **Wave-2 reassessment gate**           | Waves 0–2 are the committed scope (shell, flag, field kit, top-5 migrations). Before wave 3 starts, the product owner reviews: count of `schema-gap` Custom fields in the five schemas, the cost of the 0.4 snapshot harness against what it caught, and teacher feedback on the drawer surface. Waves 3–10 proceed only on an explicit go recorded in §2. **Rev 8.**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| D27 | **Scope split (2026-09-16)**           | Inline toolbars act on the current selection only; the drawer owns whole-widget properties. Text widget: note color and vertical alignment move from the toolbar to the Style tab (toolbar trim flag-gated). Settings tab = content/behavior, Style tab = this widget's appearance, for every widget.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| D28 | **Explicit defaults (2026-09-16)**     | Three levels: building default (admin) → my default (teacher) → this widget. "My default" is saved only from a **Save as my default** / **Reset to my default** footer on the Style tab (appearance allowlist only) with a "differs from your default" hint. Automatic last-used saving stops for flag-on users; flag-off users keep it. Storage stays `savedWidgetConfigs`, so no migration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| D29 | **Drawer style controls (2026-09-16)** | Font = dropdown previewing each face (Content and Window tiers share the same 11 fonts + Default). Text size = stepped slider; the live widget is the preview. Font color and window background = 5 high-utility presets + custom (`FONT_COLOR_PRESETS`, `WINDOW_BACKGROUND_PRESETS`). Transparency drops "(Global)" for a "Board default" state and Reset.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| D30 | **Drawer-only presentation**           | New presentations key off `FieldCtx.surface === 'drawer'` (set only by `SettingsDrawer`); legacy palettes and chip grids stay frozen for flag-off users and the legacy snapshots, and are deleted in wave 10.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| D31 | **No redundant Window controls**       | Window font / window text size hide (and drop from the filter index) when the Content tier declares `fontFamily`, or `textSizePreset` / `fontSize` / `scaleMultiplier`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- Key renames (D11) are **dual-write** (D25). Step N of a rename copies old → new and **keeps
  old**; the front face and the schema read `config.new ?? config.old`. Step N+1, which deletes
  the old key, is authored in the same PR but registered in `WIDGET_CONFIG_MIGRATIONS` only in
  a later wave, after the reading code has shipped to `main` for one full release. Rationale:
  the whole-board `setDoc` in `hooks/useFirestore.ts` writes the migrated board back, and the
  production bundle on the same Firebase project reads it next. Tests assert old-shape fixtures
  gain the new key with no data loss and that step N leaves the old key in place; the deletion
  step gets its own fixture test when it is registered. `savedWidgetConfigs` follows the same
  two-step rule.
- **The stamp can desync from the config through the save merge (rev 9).** `mergeWidget` in
  `utils/dashboardSaveMerge.ts` spreads the local widget and then, when the local config was
  not edited since the baseline, takes `server.config` and `server.version`. On the shared
  Firebase project (D25) the server copy may be the un-migrated shape written by the production
  bundle, so the merged result is a **new `configVersion` on an old-shape config**. Later loads
  skip the migration because the stamp says it ran. Dual-read hides this until a deletion step
  runs, which would then delete the old key with no new key present. Two rules close it:
  1. When the merge takes `server.config` it also takes `server.configVersion`. `configVersion`
     joins the config group in `mergeWidget`, and is deliberately **absent** from
     `LAYOUT_FIELDS` / `STYLE_FIELDS` in `utils/widgetMergeFields.ts`; a comment there records
     why, and `utils/dashboardSaveMerge.test.ts` gains a case asserting the pair travels together.
  2. Every deletion step is **self-healing**: it first re-applies the copy (`new ??= old`) and
     only then deletes the old key, so the stamp is a hint, never a proof. The fixture test for
     each deletion step includes an old-shape input with the wrong stamp.
...
- Add `'settings-drawer'` to `GlobalFeature` in `types.ts`. In `config/featureDefaults.ts` the
  ...
  **before** `if (isAdmin) return true;`, so with these defaults and no
  ...
  global-permissions init scripts) that writes the doc with `accessLevel: 'admin'`. The doc is
  written to production **before** the wave-1b code merges, and the `GlobalPermissionsManager`
  on `main` lists only features it knows, so until the merge the flag can be switched off only
  by script or console; the script therefore also takes `--disable` (sets `enabled: false`) and
  `--delete`. The
  wave-1b exit criterion is "doc written to the shared Firebase project; an admin sees the
  drawer in the dev preview", not "default is admin". There is no separate dev project (D25),
  so the same doc gates production once the wave-1b code merges; admin-only is the accepted
  exposure. Wave 4.1 flips **both** `defaultAccessLevel: 'public'`
...
| 1b.6 Migration plumbing        | `utils/migration.ts`, `hooks/useFirestore.ts`, `hooks/useStarterPacks.ts`, `utils/googleDriveService.ts`, `utils/widgetConfigPersistence.ts`, `utils/dashboardSaveMerge.ts` + test, `utils/widgetMergeFields.ts`, `context/DashboardContext.tsx` (orchestrator-owned)                 | `configVersion` on `WidgetData`, `WIDGET_CONFIG_MIGRATIONS`, all load paths per §4.4, `flipped` normalization, `configVersion` travelling with `config` in the save merge (§4.4 rev 9).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
Exit: `global_permissions/settings-drawer` doc written to the shared Firebase project (D25) by
the init script and an admin sees the drawer in the dev preview (on in the default E2E project, off in
...
### Reassessment gate (D26)

Wave 2 is the last committed wave. The orchestrator writes a short report into §6 (schema-gap
count per migrated widget, snapshot-harness findings, teacher feedback) and stops. Waves 3–10
start only after the product owner records a go in §2.
...
- 4.3 Remove the flag after one release cycle (separate tiny PR; noted here so it isn't
  forgotten). The same PR registers the pending old-key deletion steps from waves 2–3 (D25),
  since by then the dual-read code has been on `main` for a release.
...
- **Wave-2 go/no-go** (D26): the product owner decides after wave 2 whether waves 3–10 run.
...
## 16. Revision 8 — independent go/no-go grading

Grade given: plan quality A-, idea B+ (worth doing, scope is the risk). Changes:

| Finding                                                                                                                                                                                   | Change                                                                      |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- |
| Dev preview and production deploy to one Firebase project; rev 7 spoke of a "dev project". Rules, flag doc and teacher data are shared.                                                   | D25; §4.5 and wave-1b exit reworded.                                        |
| No `firestore.rules` change is needed (flag doc, profile width and `configVersion` all sit under existing rules), so the product owner's "rules go live on prod" constraint is satisfied. | Recorded in D25; any rule change now needs a ruling.                        |
| "Copy old → new and delete old" renames would be written back by the preview bundle and read by the production bundle, regressing widgets to defaults in prod during testing.             | D25, §4.4 dual-write two-step; deletion steps scheduled in 4.3 and wave 10. |
| Committing to 59 migrations before one has landed is where large refactors stall.                                                                                                         | D26 reassessment gate after wave 2; §5 gate section; §8 item.               |

Accepted risks, unchanged from §15, plus: admins see the drawer on production from wave 1b.

## 17. Revision 9 — pre-implementation safety review

Question asked: is it safe to begin on `dev-paul` with no regression for teachers on `main`?
Answer: yes. Verified against the tree: the flag is off for everyone with no doc and off for
non-admins with the admin doc (`resolvePermissionAccess`); dashboards, user profile and
`global_permissions` rules need no change; migrated boards reach Firestore only through the
owner's own save transaction, so cross-user exposure is limited to shared boards and templates
published from the preview, which dual-write covers; and the `main` bundle preserves unknown
widget fields on every path. Changes:

| Finding                                                                                                                                                                 | Change                                                                |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `mergeWidget` keeps the local `configVersion` while taking `server.config`, so a stamp can sit on an un-migrated config; a later deletion step would then lose the key. | §4.4 rev 9 bullet: stamp travels with config; deletion steps heal.    |
| The plan never named `utils/dashboardSaveMerge.ts` or `utils/widgetMergeFields.ts`, whose own comment requires new `WidgetData` fields to be tracked deliberately.      | 1b.6 files and deliverable.                                           |
| The flag doc lands on production before the code, and the `main` admin panel cannot list or disable it.                                                                 | §4.5 and 1b.4: init script gains `--disable` / `--delete`.            |
| `dev-paul` pushes deploy rules, indexes and functions to the shared project (D25).                                                                                      | Standing rule: no unrelated backend change on `dev-paul` this series. |

This revision is final. Further changes require a new product-owner ruling recorded in §2.

## 18. Revision 10 — Text widget paper cuts and explicit defaults (product owner, 2026-09-16)

Rulings D27–D31. Delivery, each PR to `dev-paul`:

1. **Style controls** — D29, D30, D31, plus the Text toolbar paper cuts (5 font/highlight presets + custom; toolbar menus layer above the widget chrome bar).
2. **Text drawer alignment** — note color and vertical alignment in the Text Style tab; flag-gated removal of both from the toolbar (D27).
3. **Explicit defaults** — Style-tab footer and the flag-gated switch away from automatic saving (D28).
