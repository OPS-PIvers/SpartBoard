...
2. **Find-a-setting filter** directly under the header, above the tab bar: a single text input
   ...
   form field closes the drawer. This is the existing `SettingsPanel.tsx` "Escape inside a field"
   rule applied to the filter; the prototype showed Escape being swallowed there. Rationale: the
...
- `WidgetRenderer.tsx`: if the flag is on, pass no settings to `DraggableWindow` and let
  ...
  Escape priority chain in `handleCustomKeyboard` reads; the drawer's close path must set it
  through the same host callback, or the next Escape after closing the drawer is swallowed.
  Add this to the 1b.9 ported-behaviors list.
