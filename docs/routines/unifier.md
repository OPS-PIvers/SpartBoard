...
| LOW      | SHIPPED (run 10) | `components/widgets/SoundboardWidget/Widget.tsx:384`              | "Select sounds below" hand-rolled state → ScaledEmptyState; Music + ScaledEmptyState already imported; no subtitle — PR #1888                                                                  |
...
| LOW      | SHIPPED (run 10) | `utils/` — 22 relative imports across 16 files                                                            | `'../types'`, `'../config/...'`, `'../hooks/useQuizSession'`, `'../components/...'` → @/ alias; 2 non-import occurrences left (inline type annotation + URL test data) — PR #1889 |
| LOW      | OPEN             | `components/admin/Organization/views/` — 8 files, ~16 imports                                             | `'../types'` (Organization/types) and `'../components/primitives'` — single-level up within Organization module; mechanical; next D4 target                          |
...
| ---------- | ------------------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| 2026-06-07 | `nightly/unify-empty-states-2026-06-07`                 | D1 Empty States    | Shipped — SoundboardWidget "Select sounds below" hand-rolled state → ScaledEmptyState; Music + ScaledEmptyState already imported; no subtitle; validate ✅                                                                                 | #1888 |
| 2026-06-07 | `nightly/unify-import-paths-utils-2026-06-07`           | D4 Import Paths    | Shipped — utils/ directory complete (16 files, 22 imports); 2 non-import `'../'` occurrences correctly left (inline type annotation + URL test data); validate ✅                                                                         | #1889 |
| 2026-06-07 | —                                                       | D2 Brand Colors    | Aligned — only MusicWidget (D2-E5, known exception) in scan; no new inline brand-color instances                                                                                                                                           | —     |
| 2026-06-07 | —                                                       | D3 Settings Labels | Aligned — no new anti-pattern in admin/ or new dirs; LunchCount/SubmitReportModal.tsx remains NEEDS REVIEW (human decision on scope); D3 is exhausted for in-scope items                                                                   | —     |
| 2026-06-07 | —                                                       | D5 Toast Arch      | Aligned — OrganizationPanel, LinkShortenerManager, GraphicOrganizerModal all known exceptions; no new admin components with local toast                                                                                                    | —     |
...
- **D3 effectively aligned (run 10):** No new anti-pattern found in admin/, new dirs (classroomAddon/, legal/, lti/), or widget Settings files. LunchCount/SubmitReportModal.tsx labels are in a submit confirmation modal (not a settings back-face or admin config panel) — a human must decide if D3 scope extends there. Do not unify without that decision.
- **D4 hooks/ complete (run 9):** All 16 files, 43 imports converted. Bonus Prettier fix in useImageUpload.ts.
- **D4 utils/ complete (run 10):** All 16 files, 22 imports converted. Two non-import `'../'` occurrences correctly left: `quizDriveService.ts:657` (inline dynamic type annotation) and `studentJoinRouting.test.ts:36` (URL path test data). Next D4 target: `components/admin/Organization/views/` (~8 files, ~16 imports — `'../types'` and `'../components/primitives'`).
- **D4 Organization/views/ gray-zone note (run 10 scan):** `components/admin/Organization/views/` imports `'../types'` and `'../components/primitives'` — these reference Organization-module-local types/primitives, not global app types. Converting to `@/components/admin/Organization/types` is mechanical and correct. Also found within-module gray zones: `common/library` → `'../Modal'`, `GuidedLearning/components` → `'../utils/imageUtils'`, plc root gray-zone (D4-E2). Only Organization/views/ is clearly actionable.
