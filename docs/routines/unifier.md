...
| LOW      | SHIPPED (run 8) | `components/widgets/NextUp/Settings.tsx`              | 3 instances (Session Status, Integration & Logic, Visual Style) → SettingsLabel — PR #1850                                                                                                                                                                   |
| LOW      | SHIPPED (run 8) | `components/widgets/MathTools/Settings.tsx`           | 1 instance (Palette DPI Calibration) → SettingsLabel — PR #1850                                                                                                                                                                                              |
| LOW      | SHIPPED (run 8) | `components/widgets/RecessGear/Settings.tsx`          | 1 instance (Source Weather Widget, had extra px-1 via className prop) → SettingsLabel — PR #1850                                                                                                                                                             |
...
| LOW      | SHIPPED (run 8) | `components/plc/authoring/` — 2 files                                                                     | `PlcAuthorQuizModal.tsx` and `PlcAuthorVideoActivityModal.tsx`: `'../assignments/PlcAssignmentConfigModal'` → `@/components/plc/assignments/...` — PR #1851  |
| LOW      | SHIPPED (run 8) | `components/plc/tabs/PlcAssignmentsInProgressSubTab.tsx`                                                  | `'../assignments/PlcAssignmentSessionModal'` → `@/components/plc/assignments/PlcAssignmentSessionModal`; tabs→root gray-zone import preserved — PR #1851    |
...
| ---------- | ------------------------------------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- |
| 2026-06-04 | —                                                        | D1 Empty States    | Aligned — 462-hit scan; all loading states (D1-E2), non-empty-state layout patterns, or pre-existing cqmin-styled front-face states already at equivalent quality to ScaledEmptyState. No new snowflakes. ShuffleList DnD-reactive drop zone noted as DnD-exception-adjacent. | —     |
| 2026-06-04 | —                                                        | D2 Brand Colors    | Aligned — no new inline brand color instances found.                                                                                                                                                                                                                           | —     |
| 2026-06-04 | `nightly/unify-settings-labels-2026-06-04`               | D3 Settings Labels | Shipped — NextUp/Settings.tsx (3 labels), MathTools/Settings.tsx (1), RecessGear/Settings.tsx (1, px-1 via className) → SettingsLabel; 5 total; validate ✅; rebase on c902187 before push                                                                                    | #1850 |
| 2026-06-04 | `nightly/unify-import-paths-plc-authoring-2026-06-04`    | D4 Import Paths    | Shipped — authoring→assignments (2 files) + tabs→assignments (1 file): 3 cross-subdir imports → @/components/plc/assignments/...; tabs→plc-root gray-zone import preserved; validate ✅                                                                                        | #1851 |
| 2026-06-04 | —                                                        | D5 Toast Arch      | Aligned — 13 local Toast imports in admin/ all map to D5-E2 through D5-E13; no new admin components found.                                                                                                                                                                    | —     |
...
- **D1 cqmin-styled front-face states (run 8):** Several pre-existing front-face empty states (NeedDoPutThen x3, WorkSymbols:115, RandomGroups:594, ExpectationsWidget:428, SoundboardWidget:384, Embed:507 "not embeddable") use cqmin inline styles already — they're functionally equivalent in quality to ScaledEmptyState. Do not flag these in future scans. `ShuffleList.tsx:149` drop zone has DnD-reactive styling (`isOver` state changes background/border/text) and CANNOT use ScaledEmptyState — analogous to D1-E3 (ConceptWeb blank canvas); do not re-flag.
...
- **D3 next targets (run 9):** All OPEN items are now NEEDS REVIEW — ExpectationsWidget/Settings.tsx (1 instance, missing `font-black`), SoundWidget/Settings.tsx (2 instances, missing `font-black`). These may be intentional lighter-weight variants. LunchCount/SubmitReportModal labels need human decision on whether a submit modal is in D3 scope. Scan for newly added widget settings files before picking: `grep -rn "text-xxs font-black text-slate-400 uppercase tracking-widest" components/widgets/ --include="*Settings*" | grep -v SettingsLabel`.
- **D4 plc/ remaining (run 8 update):** authoring→assignments and tabs→assignments clusters shipped (PR #1851). The `plc/` cross-subdir backlog is now exhausted. Next D4 target is the large pass: `hooks/`, `context/`, `utils/` relative root-sibling imports (~43/54/22 files) — requires care around test tsconfig path resolution; confirm test imports resolve correctly before fixing.
