...
| D3-E4  | `LunchCount/Settings.tsx` `<div className="text-xxs uppercase text-slate-400 tracking-widest">` placeholder | Visual placeholder text inside a dashed-border rosterMode indicator — not a form control label                                |
...
- **D3 next target:** `components/widgets/SpecialistSchedule/Settings.tsx` — 3 instances (simplest remaining conversion after LunchCount is shipped).
- **D4 plc/ re-audit (run 4):** The `overview/` and `grid/` subdirectories were deleted from `components/plc/` on dev-paul. Before running D4 on plc/ again, audit which files remain and which cross-subdirectory imports actually exist. 26 relative imports found in run 4 — many may be within-plc-subtree and acceptable.
- **D1 backlog exhausted (run 5):** All known hand-rolled empty states are now shipped or classified as intentional exceptions. D1 future runs should scan for new widgets only.
