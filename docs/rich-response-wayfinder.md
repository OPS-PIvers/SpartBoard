...
_Rebuilt 2026-08-07 after RR-06 closed. **The quiz side has no design questions
left.** Response model, serialization, persistence, privacy, timing, completeness,
recording controls, the AI boundary and now grading are all settled. What remains
splits cleanly in three: **two human errands** (RR-A5, RR-09), **the B track**,
and **the C track, which nobody has touched since it was charted.**_

⚠️ **The character of this board changed today.** For four sessions the answer to
"what's next" was a keystone ticket with consequences pooling in it. There is no
such ticket now — **RR-B2 is the nearest thing**, and it is a keystone for one
track rather than for the map. The map's remaining risk has moved out of
decisions and into **two measurements and an email.**

**Takeable now:**

- 🔥 **RR-A5** — Verify format round-trip and capture policy on district hardware _(task, HITL)_ — **the only thing on the board no agent can do, and the only thing blocking anything.** Fourth session running as the tail dependency of a **student-facing number**: RR-08 sub-decision 6 blocks Submit on an in-flight upload and explicitly refused to invent the threshold, deferring to RR-A6, which waits on this. RR-A2 sharpened it — with takes appending and `takeLimit` unlimited, a student generates **back-to-back uploads on one connection**. Also: does district hardware encode **480p / 500 kbps** (RR-A1 sub-decision 4). **Needs a student Chromebook.** Harness: `docs/rich-response/rr-a5-capture-harness.html`
- 🔥 **RR-09** — the questions only district counsel and Google can answer _(task, HITL, unclaimed)_ — **question 7 is still the one item on the map that can stop a capability from shipping**, and it is answered by sending an email. RR-06 added **question 9** today, a much smaller one: Classroom has an "excused" state and Schoology's AGS apparently doesn't, which decides how honestly the grade push can report itself. Nine items now; two of them (7, 3) are worth more than the rest combined
- **RR-B2** — Is the audio synchronized to the strokes, or attached alongside? _(grilling)_ — **the B track's keystone and now the largest open decision on the map.** A three-way fork thanks to RR-B1, and it has collected **four riders** that all resolve by resolving it: RR-A2's `takeIndex` (does a whiteboard even have takes?), RR-05's transcription (one artifact or three kinds?), and 🔵 **new today** — RR-06's time-anchored comments, which land on the _work_ if the timelines are one and on nothing in particular if they're two. **Note the sharpest thing RR-06 handed it:** RR-06 dropped `gradingSnapshot` for media because a take is immutable, and a command-stack whiteboard is the one artifact on this map that might not be
- **RR-07** — Alternate-format policy _(grilling)_ — **narrowed three times in three days.** RR-08 removed substitution; RR-A2 removed refusal; **RR-06 gave the device-blocked student a grading-side excuse** (sub-decision 3) — so what's left is an **authoring-time** question: how a teacher learns, while building, that some of their class will hit a wall. RR-06's excuse is a backstop and it is discretionary; if this ticket wants that student protected rather than merely visible, it still has to say so
...
**Still blocked:** RR-B3 (RR-B2 only — **RR-06 cleared today**) · RR-B4 (RR-B2) ·
RR-A6 (RR-A5 only).
...
⚡ **No ticket on this board is waiting on another ticket except RR-B3 and RR-B4,
which both wait on RR-B2, and RR-A6, which waits on a measurement.** The map's
first two-way dependency **resolved rather than persisted**: RR-06 permitted
grading an earlier take, RR-05 sub-decision 3 was reopened and amended in place,
and the arrow points one way again. One backward thread survives — **RR-B3's
prototype is now the only path that can reopen RR-05's AI menu**, because RR-06
answered the queue-cost question by reasoning and RR-B3 is where it gets counted.

**Both human errands should be run now.** RR-A5 has been the tail dependency of a
student-facing number for four sessions. RR-09's question 7 should go out ahead of
any transcription work — it is copy the district has to approve, which is fast to
obtain and cheap to wait for.

**The next agent session should take RR-B2.** It is the last keystone left, four
tickets' worth of consequence resolve by resolving it, and it is the only thing
standing between the map and the B track's other three tickets. **RR-C3 is the
better pick if you want something small and self-contained** — it is independent
of everything and has been the recommended warm-up since the map was charted.
...
**Type:** prototype (HITL) · **Status:** Open · **Blocked by:** RR-B2 · ~~RR-06~~ (closed 2026-08-07)
