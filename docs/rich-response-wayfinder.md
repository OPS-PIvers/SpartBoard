...
_Rebuilt 2026-08-06 after RR-05 closed. **RR-06 is unblocked, and it is now the
biggest thing on the map.** Every quiz-side design ticket except grading is
settled: response model, serialization, persistence, privacy, timing,
completeness, recording controls, and the AI boundary. **Nothing on this board is
blocked by another ticket any more except RR-A6, which waits on a measurement.**_

**Takeable now:**

- 🔥 **RR-06** — How do media responses grade, and how do they reach the gradebook? _(grilling)_ — **unblocked today and immediately the largest open question on the map.** Three closed tickets have been pooling consequences here for two days: RR-08 gave it a state `GradeResult` cannot express, RR-A2 gave it N takes and two grading questions it explicitly declined, and **RR-05 just removed the possibility of machine help** — no AI scoring, so every judgment is human and the 30 × N wall-clock cost is a fixed input. It also holds a thread that runs backwards: **if it permits grading an earlier take, RR-05 sub-decision 3 reopens**, and if it finds the human queue unworkable, that is the finding that reopens RR-05's menu. **The best agent work on the board by a wide margin**
- 🔥 **RR-A5** — Verify format round-trip and capture policy on district hardware _(task, HITL)_ — **still the only thing on the board no agent can do, and now the only thing blocking anything.** It is the tail dependency of a **student-facing timeout**: RR-08 sub-decision 6 blocks Submit on an in-flight upload and explicitly refused to invent the threshold, deferring to RR-A6, which is blocked on this. RR-A2 sharpened it — with takes appending and `takeLimit` unlimited, a student generates **back-to-back uploads on one connection**, so the measurement feeds a queueing decision too. Also: does district hardware encode **480p / 500 kbps** (RR-A1 sub-decision 4). **Needs a student Chromebook.** Harness: `docs/rich-response/rr-a5-capture-harness.html`
- **RR-07** — Alternate-format policy _(grilling)_ — **narrowed twice in one day and now nearly halved.** RR-08 removed substitution for a required addendum; **RR-A2 removed refusal entirely** — the block stands with no decline exit, so _"this student will not record"_ has no design left in it. What remains is **degradation and device-blocked**, where the mandatory alternative on the **primary mode set** stands on RR-A4 finding 5. RR-05 made its population visible to the teacher as device-blocked rather than as a blank. No compliance thread runs through it any more
- **RR-B2** — Is the audio synchronized to the strokes, or attached alongside? _(grilling)_ — a **three-way** fork thanks to RR-B1, and **RR-A2 handed it `takeIndex` for free** plus the question of whether a whiteboard has takes at all. **RR-05 added a third rider:** if the audio syncs to the strokes the combined artifact is a recording and inherits transcription; if it doesn't, the slot holds strokes, audio and a transcript as three kinds at once. The B track's keystone
...
- **RR-09** — task: the questions only district counsel and Google can answer _(unclaimed)_ — ⚠️ **urgency reversed today, after two days of softening.** RR-05 added **question 7**, and unlike every other item on this list it **gates a shipping decision**: if a recording may be machine-transcribed, RR-04's Tennessen notice has to say so, and the clause needs the district's review rather than in-product drafting. Its stale **Blocks: RR-05** claim was retired the same day — RR-05 closed without it

**Still blocked:** RR-B3 (RR-B2, RR-06) · RR-B4 (RR-B2) · RR-A6 (RR-A5 only).
...
⚡ **The board has one ticket left that other tickets are waiting on, and it is
RR-06.** Everything else is **measurement** (RR-A5, RR-09) or **an untouched
track** (B, C). ⚠️ **But RR-05 introduced the map's first two-way dependency**,
which is new in kind: RR-06 can reopen RR-05 from either direction — by
permitting an earlier take to be graded, or by finding the unassisted queue
unworkable. Every prior dependency on this map pointed one way. **Whoever takes
RR-06 should read RR-05's resolution first, not just its gist.**

**RR-A5 should be run by a human now** — it has been the tail dependency of a
student-facing number for three sessions running, and it is the last thing
blocking anything. **RR-09 has quietly become urgent again**: its new question 7
is the one item on the map that can stop a capability from shipping, and it is
answered by sending an email.

**The next agent session should take RR-06.** It is the only remaining ticket
with three closed tickets' consequences pooled in it, it is the last quiz-side
design decision, and it is the gate to the entire B track's grading half.
...
**Type:** grilling (HITL) · **Status:** Open — **unblocked 2026-08-06** · **Blocked by:** ~~RR-02, RR-03, RR-04, RR-05~~ (all closed)
