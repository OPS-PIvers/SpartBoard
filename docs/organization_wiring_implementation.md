...
- **User counters.** ~~Phase 4's responsibility.~~ **Landed 2026-04-20 in Phase 4.1** as `organizationMemberCounters` (see Phase 4.1 backlog entry). The `users` field on `/organizations/{orgId}`, `/buildings/{id}`, `/domains/{id}` is now maintained atomically via `FieldValue.increment()` deltas on every `members/{email}` write.
