...
- **Counter-maintenance Cloud Function.** `users` on `/organizations/{orgId}`, `/buildings/{id}`, `/domains/{id}` is a denormalized count. Needs an onWrite trigger on `members/{emailLower}` that increments/decrements these counters atomically. **Why deferred:** stale display isn't a regression; keeps Phase 4 scope tighter. **Size:** ~half day (trigger + unit test + idempotency for the catch-up run). **Stopgap:** [scripts/recount-org-members.js](scripts/recount-org-members.js) rebuilds all three counters from `/organizations/{orgId}/members/*` in one pass. Safe to re-run; counts drift again as invites/role-changes/removals land until the trigger ships. Ran 2026-04-20 after the A2 backfill landed — org=92, buildings (high 29 / schumann 21 / middle 18 / intermediate 18), domain primary=92. The script's `tallyMembers()` helper is deliberately shaped so the future trigger can share the counting logic verbatim. **Trigger:** Users view looking too obviously broken, or counters drifting far enough from reality that admins stop trusting them.
