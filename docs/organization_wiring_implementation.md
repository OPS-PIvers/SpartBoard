...
- ~~**Functions-scoped test runner.**~~ **Landed 2026-04-21** alongside the counter-maintenance CF on branch `claude/fix-building-user-count-F7BqF`. `functions/package.json` now has a `vitest` devDep and `"test": "vitest run"` script; the root `package.json` added a `test:all` script (`pnpm test && pnpm -C functions test`) and wired it into `validate`. `.github/workflows/pr-validation.yml` Unit Tests job now runs `pnpm run test:all`, so the previously-executable-documentation suites (`organizationInvites.test.ts` 47 tests, `organizationMembersSync.test.ts` 18 tests, `sanitize.test.ts`, and the new `organizationMemberCounters.test.ts` 21 tests) all gate merge to `main`. Net: 105 passing, 3 skipped. The 3 skipped tests all live in `index.test.ts > adminAnalytics` — the handler's engagement model shifted from `user.lastLogin` → `dashboard.updatedAt`, `topUsers` now filters by `authUsersMap`, and `totalDashboards` is tallied before the anonymous-owner filter. Fixtures need rewriting to match; tracked as a follow-up. Mock fixes that WERE made while landing the runner: `firebase-admin` mock now exports `apps: []` (needed because `organizationMemberCounters.ts` / `organizationMembersSync.ts` / `organizationInvites.ts` all `if (!admin.apps.length) admin.initializeApp()` at module load); `mockFirestore` now has `doc()` and `getAll()` for the userProfile batch-read path; and the `fetchExternalProxy` allowed-URL error-message assertion was refreshed to include `https://orono.api.nutrislice.com`.
