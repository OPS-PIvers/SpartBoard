...
## RR-B4 closed 2026-08-07 — the same lesson twice in a row, and the first time an audit made a ticket _smaller_ than the person asking for it expected

RR-B2's entry ends with a recommendation: write up the audit's **conclusions**,
not just its findings, before question one. RR-B4 was the first session run that
way, and it changed the shape of the questions rather than just their framing —
three of the ticket's four charted bullets were struck before anything was asked,
because the audit's conclusions said they were already answered by shipped code.

- 🔴 **RR-B1 missed the same class of thing a third time, and this one was live.** It reported that geometry is stored in raw canvas pixels and framed that as a student-response problem. The call sites say it is not: `canvasSize` is the ResizeObserver'd wrapper box and objects are never rescaled when it changes, so **the teacher's own whiteboard already slides its artwork on every resize.** A defect that ships today was sitting inside a finding filed as a hypothetical about a feature that doesn't exist yet. Same directory, same research ticket, same root cause as RR-B2's finding: **the type said where coordinates live; only the call site said what happens to them.**
- ✅ **Two audits in a row moved the estimate in the cheap direction, and the mechanism was identical both times.** RR-B2 found the capture layer already built. RR-B4 found the fixed page already implemented three separate ways — pointer input divides by `canvas.width / rect.width` per axis, `ScalableWidget`'s `canSpread: false` branch is uniform-fit letterboxing in production, and `exportCanvas.ts` renders a page to PNG at an arbitrary size. **The B track's two most expensive-looking tickets were both mostly built.** That is now a pattern worth stating as a prior: on this codebase, a wayfinder ticket that sounds like infrastructure is likely to be a constant.
- 📌 **The sharpest finding was one nobody asked for.** `utils/proportionalLayout.ts` + `migrateProportionalLayout.ts` already migrated every widget's **bounds** from absolute pixels to proportions of a fixed `REFERENCE_VIEWPORT`, with an aspect field and a defensive re-derivation detector. The drawing widget's **contents** were the last absolute-pixel coordinate space left in the app. The ticket's question — "what is a whiteboard's canonical coordinate space?" — had a house answer one level up, and no charted bullet pointed at it. It was found by asking _"has this app solved this shape of problem before?"_, which is a question neither RR-05's rule nor RR-B2's amendment tells you to ask.
- ⚠️ **And it arrived one question too late, which is the second time in two sessions.** I priced the "fix the teacher widget too" option as _"a migration guessing at a canvas size that was never recorded."_ Paul took it anyway. The precedent finding then showed the migration is read-time, pure, needs no backfill, and derives the source canvas from persisted data. **The option text was wrong in the direction that argues against the answer Paul gave** — recorded in the ticket as a correction rather than quietly fixed. RR-B2's amendment (assemble conclusions before question one) was followed for the _findings_; this one came from a follow-up query run after the last question, which is exactly the gap that amendment was meant to close.
- 🔵 **First on this map: a decision that changes shipped behaviour.** Every prior ticket added net-new response-capture surface, so nothing could regress for an existing user. Sub-decision 6 rescales drawings teachers already made. The map's standing "plan, don't build" posture means nobody has to act on it today — but it puts the first item on the scheduled-work list whose failure mode is **a teacher's saved work moving**, and the ticket says it wants its own PR and its own before/after screenshots for exactly that reason.

## RR-C1 closed 2026-08-07 — the C track opened the way the other two did, and then an audit made a ticket _harder_ for the first time

Two sessions ran in parallel: RR-C1 as grilling, and RR-C2's audit half in a
subagent. The pairing is worth recording on its own — the audit half of a
grilling ticket parallelizes cleanly because it asks nobody anything, and RR-C2's
next session is now pure grilling against verified ground.

- ✅ **The "has this app solved this shape of problem before?" question paid off a second time, and it was asked deliberately this time.** RR-B4's entry recorded that question as the source of its sharpest finding and noted no rule tells you to ask it. RR-C1 asked it first, and the answer was GuidedLearning: per-step `audioUrl`/`videoUrl` with non-destructive playback trims, a full player for each, shipped and student-facing. The ticket's framing — "which formats are in, and are they rendered in-app or handed off?" — had a house answer for four of its five formats. **Promoting that question from an accident to a step is the concrete process change from this session.**
- 🔴 **The audit found a contradiction the codebase is already living with, which is a new category on this map.** Prior audits found things that were wrong (RR-05's four premises) or things already built (RR-B2, RR-B4). This one found `AudioInteraction` shipping **no scrubbing at all** and `VideoInteraction` shipping **full native controls**, in the same feature, in the same directory, neither deliberate. The ticket asked whether restricting replays is a legitimate assessment lever; the app had already answered **both ways** without noticing. The resolution fixes it for stimuli and explicitly declines to fix GuidedLearning — scope discipline that is easy to lose when the audit hands you a defect.
- 🔴 **RR-C2's audit inverted the map's own rule, and this is the important entry.** The standing rule was "read the call sites and the ticket gets cheaper" — twice running. RR-C2's audit read the call sites and the ticket got **harder**: the option its text called "probably disqualifying" is the shipped house style, and the proxy precedent RR-03 believed it left behind **does not exist**, because SpartBoard holds only the `drive.file` scope. **The rule was always "the call sites are where the truth is."** Two cheap results in a row had quietly rewritten it into "the call sites make things cheaper," and it took a ticket going the other way to notice.
- ⚠️ **My option text was wrong again, in the same direction, for the third session running.** I priced pdf.js as "a substantial new dependency and bundle cost." `vite.config.ts` already isolates `@imgly/background-removal` into its own manual chunk and widgets already lazy-load — the cost is a chunk entry and a lazy boundary, both house pattern. **Three sessions in a row I have overpriced the option Paul then chose, each time by not checking whether the repo already has a pattern for it.** RR-B2's amendment says to assemble conclusions before question one; the pattern here is narrower and worth stating separately: **before writing a cost into an option, check whether the repo has already paid it once.**
- 📌 **A decision was made against my recommendation whose cost I described wrongly in both directions.** Silent stimulus failure: I framed it as a student trapped behind a spinner. Nothing blocks the input, so the student answers anyway — the real cost is a wrong answer **indistinguishable from not knowing**, unrecorded anywhere. Less dramatic, more insidious. It is the first failure mode on this map that RR-07 cannot help with, because it is unknowable at authoring time, and it produced the map's first fog patch that exists only to catch evidence that a rate is non-zero.

## RR-C2 closed 2026-08-07 — the audit's hardest finding was undone by one more query, and it was mine to undo

Same pairing as the previous session, one track along: RR-C2 as grilling, RR-C3's
audit half in a subagent. Both audits have now made their tickets easier _and_
harder, which is the entry below.

- 🔴 **I asserted a capability was impossible, checked before building on it, and was wrong — and that single check is what resolved the ticket.** Reasoning toward the question about gate strength, I concluded Firebase Storage rules cannot consult Firestore, which would have made a real gate impossible without a Cloud Function. `storage.rules` performs five cross-service lookups, and `:127-139` gates activity-wall photos on `firestore.get()` against the parent session doc — within one line of exactly the predicate RR-C2 needed. **The grounding brief's entire §6 — concurrency 80, the 512 MiB fully-buffering `driveArchive.ts:249`, cold starts, no caching, no signed URLs — was the carefully-priced cost of a component that turned out to be unnecessary.** The brief was not wrong; it priced a proxy accurately. Nobody asked whether a proxy was required.
- ✅ **The process rule from RR-C1 has a sibling now, and it is the more useful of the two.** RR-C1's retrospective produced _"before writing a cost into an option, check whether the repo has already paid it once."_ This session produces: **before ruling an option out, check whether the repo has already built the capability it needs.** The first rule catches overpriced options; the second catches options that get **deleted from the menu entirely**, which is worse because nobody ever sees them to argue about. The Cloud Function proxy was on RR-C2's menu for three days as the "decided precedent," and the correct answer was that the whole category was misconceived.
- ⚠️ **The audit's headline was right and its framing was load-bearing in a way that nearly cost the session.** "Both premises are wrong in opposite directions" was accurate. But the brief then reasoned _within_ the ticket's three-option fork — treating it as a choice between public URLs, a proxy, and copy-into-Storage — and every one of its six questions inherited that frame. **An audit that refutes a ticket's premises can still inherit its option set**, which is a subtler failure than getting a fact wrong and much harder to notice from inside. The frontier's own caution for this ticket had warned against treating the six questions as a menu; the actual risk was treating them as _exhaustive_.
- 🔵 **Three of six questions dissolved without being asked, and that is the grilling working rather than shortcuts.** Once the gate was `request.auth != null`, the pre-join-preview question had no content (there is no join to precede), the "where does the reference live" question had no content (a Storage path is not a credential), and the AV-storage question was forced by construction. **Each earlier answer should shrink the remaining question set** — a grilling session that asks all its prepared questions probably wasn't listening to the answers.
- 📌 **Paul answered "I'm not sure" once, and that was diagnostic rather than a stall.** The question — does the product tell a teacher who can reach their stimulus file — turned out to be mis-homed rather than hard: its audience is the district, not the teacher, so RR-09 already owns it, and it is the only decision in the ticket reversible at near-zero cost. Reframing from _what to decide_ to _where it gets decided_ resolved it in one exchange. **When a decision-maker stalls on a question the other three didn't, check whether it belongs to this ticket at all before pushing for an answer.**
- 🔴 **The resolution says a thing none of its sub-decisions say.** Each sub-decision is individually reasonable; together they mean SpartBoard does not meaningfully protect copyrighted stimulus material — the thing the ticket opened by calling "probably disqualifying." Nothing in the four answers states that, and it would have been easy to close without writing it down. **A resolution assembled from defensible parts can carry an undefended whole**, and the map's job is to say so out loud rather than let it be discovered later by someone reading the sub-decisions in order.

## RR-C3 closed 2026-08-07 — the precedent was real, transferable, and right for the wrong reason

The last design ticket on the map, and the first whose audit ran as a separate
session rather than in parallel with another ticket's grilling. The pairing
pattern the previous two sessions established paid off exactly as intended: the
expensive half was already on disk, so this session asked six questions and
nothing else.

- 🔴 **"The app has already built this" is a candidate, not an answer — and the reason it was built has to be checked separately from the shape.** The audit's headline was that VideoActivity _is_ this ticket, shipped, with the media on the parent and a pointer on the child. That was true. But both precedents put media on the parent because **the media supplies the ordering** — a video's timeline, a slide sequence — and a quiz stimulus supplies none, so the precedent's own justification does not transfer. The shape was adopted anyway, on a reason neither precedent knows about (**the pointer is the grouping concept**, which is what shuffle needed), and departed from on the one point where the ordering semantics were doing real work: a stable id rather than GL's `imageIndex`. **Adopting a shipped shape for a reason the shipped code does not have is fine; not noticing that you have done so is not.**
- ⚠️ **The frontier's caution for this ticket was the only prediction this map made about its own next session, and it landed on the exact line it named.** It said the risk was treating the precedent as the answer, and that it would stop applying precisely where the media supplies the ordering. It stopped there. **That is worth recording as evidence the frontier is doing work rather than summarizing** — a handoff that predicts where the next session will go wrong, and is right, is a different artifact from one that lists what is left.
- 🔴 **The audit inherited a mechanism inference from a comment it did not read.** Its Q7 rested on `key={currentImageUrl}` (`GuidedLearningPlayer.tsx:727`), read as an idiom for making a shared asset **persist** across items. The comment four lines above says the key exists so the element **reloads** when the slide changes. Same line, opposite purpose. The inferred consequence survived; the mechanism was backwards, and the real house idiom turned out to be **simpler** — the persistent element is unkeyed. **New process rule, sibling to the previous two: read the comment next to the mechanism before inferring what the mechanism is for.** RR-C1's rule catches overpriced options; RR-C2's catches deleted ones; this one catches options priced against a mechanism that does something else.
- ✅ **Two premises were verified _before_ question one, and both changed the questions.** The keying finding made pinning and attachment separable, so the attachment question could be decided on authoring grounds alone. And `toPublicQuestion` turning out to be a hand-written **allowlist** — nothing reaches a student by default — meant storage and exposure are two decisions, which **dissolved RR-C2's two-shapes wrinkle outright** rather than answering it. RR-B2's amendment (assemble conclusions, not just findings, before question one) has now paid on three consecutive sessions, and this is the first time it paid by making a handed-down constraint disappear.
- 📌 **The audit refuted the ticket's other premises and still inherited its framing on shuffle.** RR-C2's retrospective named this exact failure one session earlier — _"an audit that refutes a ticket's premises can still inherit its option set"_ — and it recurred immediately. The charted worry (_"a passage attached to a group is meaningless if shuffling scatters it"_) assumed the passage is displayed **once above a block**; under per-question pinned rendering each question carries its own display, so scattering costs read-once **flow**, not validity. Nobody caught it in the audit because nobody re-derived what "scatter" would actually do. **Naming a failure mode in a retrospective does not inoculate the next session against it.**
- 🔵 **The decision against my recommendation is the one with an open verdict, and there is now a measurement scheduled to judge it.** Paul took the pointer **array** — a question may stack two stimuli — which created the overlapping-set problem that shaped the shuffle answer and the label cost. Three prior against-recommendation calls have since been vindicated; this one lands on a 768px Chromebook nobody has looked at, and RR-A5 now carries a fifth item to look at it. **That is the right shape for an open verdict: not an argument in a document, a line item on an errand.**
