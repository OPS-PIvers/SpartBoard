...
## 2026-03-05 - [CI Cache Drift] **Bottleneck:** GitHub actions using `setup-node` with pnpm in a monorepo were not correctly caching the `functions/pnpm-lock.yaml`. **Fix:** Added explicit `cache-dependency-path` to all jobs in `firebase-dev-deploy.yml` to ensure deterministic caching across both the root and functions directories.
