...
## 2026-04-06 - [Strict CI Runners and Node Version] **Bottleneck:** GitHub Actions used `ubuntu-latest` and package.json used `node: >=20.0.0` which can cause "works on my machine" issues or breaking changes if versions bump unexpectedly. **Fix:** Pinned CI runners to `ubuntu-24.04` and node engine strictly to `20.x` for deterministic builds.
