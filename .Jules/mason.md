...
## 2026-03-28 - [Dynamic pnpm Versioning in CI] **Bottleneck:** The CI setup action hardcoded or left the pnpm version unspecified, risking `ERR_PNPM_BAD_PM_VERSION` errors and pipeline breakages if versions drift from `package.json`. **Fix:** Updated `.github/actions/setup/action.yml` to dynamically extract the exact pnpm version from the `packageManager` field in `package.json` using bash utilities and pass it to `pnpm/action-setup@v4`.
