/**
 * ShareLinkCreatorModal — host-side share link creator for peer teachers:
 * Synced, View-Only or Copy. Writes a `/shared_boards/{shareId}` doc with
 * `intendedMode` set; the recipient lands at `/share/{shareId}`.
 *
 * Handing a board to a substitute lives in `ShareWithSubModal` — a sub share is
 * written as a one-board collection share, so update, extend and end-now work
 * the same for one board as for a whole collection.
 */
...
type ShareMode = SharedBoardImportMode;
