...
const widget = { id: 'pdf-test-1', type: 'pdf' } as unknown as WidgetData;
const makeCtx = (config: PdfConfig, updateConfig: (patch: Record<string, unknown>) => void) =>
  ({
    config,
    widget,
    isAdmin: false,
...
describe('PDF settings drawer field', () => {
  it('returns an active document to the library with a sparse reset', () => {
    ...
    render(<PdfCurrentDocumentField ctx={makeCtx({
      activePdfId: 'pdf-1',
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Switch to another PDF' }));
