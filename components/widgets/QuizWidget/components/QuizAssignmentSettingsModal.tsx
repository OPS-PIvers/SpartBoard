...
> = ({ assignment, rosters, onClose, onSave }) => {
  ...
  // "Auto-Generated PLC Sheet" toggle — initial state mirrors whether a
  // URL is already attached. Legacy assignments (cached PLC sheet or
  // manual paste) open with the toggle OFF and the URL pre-filled so
  // the user can keep using or edit it. Edit mode never triggers
  // auto-create on save — toggling ON simply hides the field; the
  // existing `assignment.plc.sheetUrl` is preserved by the no-op branch
  // in `handleAssign` when `plcSheetUrl` saves as empty.
  const [useAutoGenerated, setUseAutoGenerated] = useState(
    !assignment.plc?.sheetUrl
  );
