...
export const RubricScoringPanel: React.FC<RubricScoringPanelProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col divide-y divide-slate-200">
        {rubric.criteria.map((criterion) => {
          ...
          return (
            <fieldset
              key={criterion.id}
              aria-labelledby={`rubric-${rubric.id}-${criterion.id}-name`}
              className="px-4 py-4"
            >
              <div className="mb-3 flex items-start justify-between gap-2">
                <div className="min-w-0">
                  <p
                    id={`rubric-${rubric.id}-${criterion.id}-name`}
                ...
                {selected && (
                  <button
                    ...
                    className="-mr-1 -mt-1 shrink-0 p-1 rounded text-slate-500 hover:bg-slate-200 hover:text-slate-700 transition-colors"
                  >
                    {noteOpen ? (
                      <X className="w-4 h-4" />
                    ) : (
      ...
      <footer className="px-4 py-3 border-t border-slate-200 bg-white rounded-b-lg">
        <div className="flex items-baseline justify-between gap-3">
          <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
            Rubric total
          </span>
          <span className="font-mono text-sm font-bold text-slate-900 whitespace-nowrap">
