...
/**
 * Two-pane shared notebook for the PLC. Reusable body extracted from the
 * legacy `PlcNotesTab` so the Phase 1+ live-tile renderer (and any future
 * fullscreen surface) can mount the same editor without duplicating the
 * collaborative-write debounce logic.
 *
...
export const NotesBody: React.FC<NotesBodyProps> = ({ plc }) => {
  ...
  const handleCreate = async () => {
    try {
      ...
      logError('NotesBody.createNote', err, { plcId: plc.id });
    }
  };
