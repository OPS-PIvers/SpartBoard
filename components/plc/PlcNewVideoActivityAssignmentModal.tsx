...
> = ({ plc, assignmentMode = 'submissions', onClose, onCreated }) => {
  ...
  // Stable ids for label↔control association on the rewind / penalty /
  // score-visibility controls in the trailing scoring block. The visible
  // labels are siblings (not <label htmlFor>) because the row layout
  // splits label-left / control-right; `aria-labelledby` ties them
  // together for screen readers.
  const penaltyLabelId = useId();
                ...
                <div>
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <input
                      ...
                      step={1}
                      value={penalty}
                      aria-labelledby={penaltyLabelId}
                      onChange={(e) => {
