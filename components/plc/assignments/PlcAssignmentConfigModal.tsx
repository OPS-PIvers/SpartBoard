...
> = ({
  ...
  const { user, ensureGoogleScope, getAssignmentMode } = useAuth();
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Build PLC sheet (best-effort). Acquire the Sheets scope on demand
      // (Path B): silent for already-granted users, a one-time consent popup
      // for never-granted (this Save click is a user gesture). If the token is
      // null (declined / no Google access) we skip sheet creation, exactly as
      // before — the assignment still proceeds without a sheet.
