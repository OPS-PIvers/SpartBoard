...
- [ ] **M12 — Written-response rubrics (Phase 3)** (`docs/specs/M12-written-response-rubrics-spec.md` + design doc `docs/written-response-quiz-questions.md`): decisions locked (spec §3a); phases 3-A..3-I: types, `/users/{uid}/rubrics` rules + tests, RubricBuilderPanel, RubricScoringPanel, useRubrics, CSV export, link + full PLC sharing, `GradeResult.state` (RR-06 interleave), student-facing rubric views. Carry-overs from Phase 1: rules tests for student-rejection on grading writes; Playwright pause/resume-next-day E2E. Phase 4 (AI-assisted grading) deferred. (L)
