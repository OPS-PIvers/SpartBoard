...
// ── Regression: smart paste claimed success on a view-only board ───────────
//
// Bug: addWidget returns early when the active board was joined as a viewer,
// but handlePaste toasted "Added Embed widget!" regardless. The teacher was
// told the paste worked and no widget ever appeared.

describe('Dock smart-paste – view-only board', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('says the board is view-only instead of claiming a widget was added', () => {
    const addWidgetSpy = vi.fn();
    const addToastSpy = vi.fn();
    ...
    expect(addToastSpy.mock.calls[0][0]).toMatch(/view-only/i);
    expect(addToastSpy.mock.calls[0][1]).not.toBe('success');
