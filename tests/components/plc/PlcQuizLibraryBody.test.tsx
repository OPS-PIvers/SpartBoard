...
// Sheet auto-create is exercised via its result shape only.
vi.mock('@/utils/plcLinkage', () => ({
  buildPlcLinkage: vi
    .fn()
    .mockResolvedValue({ linkage: undefined, error: null }),
}));
...
describe('PlcQuizLibraryBody', () => {
  ...
  describe('C1 — assign-pickup lifecycle', () => {
    ...
    it('creates a PLC sheet for the pickup and links it so it registers with the PLC', async () => {
      const linkage = {
        id: 'plc-1',
        name: 'Test PLC',
        sheetUrl: 'https://docs.google.com/spreadsheets/d/pickup-sheet',
      ...
      await waitFor(() => expect(createAssignment).toHaveBeenCalledTimes(1));
      expect(buildPlcLinkage).toHaveBeenCalledWith(
        expect.objectContaining({
          plc,
          selfUid: 'teacher-1',
          googleAccessToken: 'sheets-token',
        })
      );
      expect(createAssignment).toHaveBeenCalledWith(
        expect.anything(),
        expect.objectContaining({ plc: linkage }),
        expect.anything()
      );
    });
